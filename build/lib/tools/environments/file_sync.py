...
_FORCE_SYNC_ENV = "EDEN_OE_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.eden") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.eden to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``EDEN_OE_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, eden_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.eden/`` directory as a tar archive,
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="eden-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.eden/skills/a.md`` →
        ``~/.eden/skills/a.md``, a new remote file at
        ``/root/.eden/skills/b.md`` maps to ``~/.eden/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
