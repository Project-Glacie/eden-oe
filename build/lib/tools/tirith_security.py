...
it is automatically downloaded from GitHub releases to $EDEN_HOME/bin/tirith.
...
def _get_eden_home() -> str:
    """Return the Eden OE home directory, respecting EDEN_HOME env var."""
    return str(get_eden_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_eden_home(), ".tirith-install-failed")
...
def _eden_bin_dir() -> str:
    """Return $EDEN_HOME/bin, creating it if needed."""
    d = os.path.join(_get_eden_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $EDEN_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_eden_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $EDEN_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $EDEN_HOME/bin/tirith
    ...
    eden_bin = os.path.join(_eden_bin_dir(), "tirith")
    if os.path.isfile(eden_bin) and os.access(eden_bin, os.X_OK):
        _resolved_path = eden_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return eden_bin
