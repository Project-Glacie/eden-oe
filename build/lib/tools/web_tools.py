...
Backend is selected during ``eden tools`` setup (web.backend in config.yaml).
When available, Eden OE can route Firecrawl calls through a Eden-hosted tool-gateway
for Eden Subscribers only.
...
- Firecrawl: https://docs.firecrawl.dev/introduction (search, extract; direct or derived firecrawl-gateway.<domain> for Eden Subscribers)
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Eden OE config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Eden OE' config/.env layer (``eden config set``, ``eden tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.eden/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``eden tools``).
    ...
    # pre-empted by a Eden OAuth token whose subscription tier may not
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `eden tools` repaint.
        try:
...
def _web_requires_env() -> list[str]:
    ...
    refresh against the Eden portal on every CLI startup (invoked at
...
def _store_full_text(url: str, content: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_eden_dir("cache/web", "web_cache")
            ...
            if disabled_key:
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        f"Re-enable it with `eden plugins enable {disabled_key}` "
                        ...
                        return json.dumps(
                            {
                                ...
                                "error": (
                                    ...
                                    f"`eden plugins enable {disabled_key}` "
                                    "(or remove it from plugins.disabled)."
                                ),
                            },
                            ensure_ascii=False,
...
if __name__ == "__main__":
    ...
    from eden_cli.config import get_env_value as _gev
