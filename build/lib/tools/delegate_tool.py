...
# Must match eden_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _get_child_timeout() -> Optional[float]:
    ...
    Eden OE (Phase 2): Also checks ``EDEN_SUBAGENT_TIMEOUT`` env var as a
    fallback. When running in an Eden context (parent has _eden_agent_name),
    the child timeout defaults to 300s (5 minutes) unless explicitly
    configured otherwise via delegation.child_timeout_seconds.
    """
    cfg = _load_config()
    ...
    if env_val:
        try:
            ...
            return None if parsed <= 0 else max(30.0, parsed)
    # ── Eden OE (Phase 2): Default to 300s in Eden context ────────
    eden_timeout = os.getenv("EDEN_SUBAGENT_TIMEOUT")
    if eden_timeout:
        try:
            parsed = float(eden_timeout)
            return None if parsed <= 0 else max(30.0, parsed)
        except (TypeError, ValueError):
            pass
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``eden-cli`` (which bundles
...
def _build_child_agent(
    ...
    role: str = "leaf",
    # ── Eden OE (Phase 2) — agent identity & tier gating ───────────
    # When _eden_agent_name is provided (non-None), Eden identity is injected
    # onto the child AIAgent and tier-based toolset restriction is applied.
    # _eden_agent_tier is the authoritative tier from agents.db → agent_delta.
    _eden_agent_name: Optional[str] = None,
    _eden_agent_tier: Optional[str] = None,
):
    """
    ...
    model on OpenRouter while the parent runs on Eden Portal).
    """
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. eden-cli) so that individual
    ...
    # ── Eden OE (Phase 2): Tier-based toolset restriction ─────────────
    # Defense layer 2: restrict the child's toolset based on its
    # AGENT_DELTA tier BEFORE the child is constructed. If _eden_agent_name
    # or _eden_agent_tier is set, apply the tier gate. The model never sees
    # tools it cannot use. The Governor's BOUNDARY check (layer 1) still
    # fires at execution time for defense in depth.
    if _eden_agent_name or _eden_agent_tier:
        try:
            from eden.agents.subagent import apply_tier_toolset_restriction

            _tier = _eden_agent_tier or "B"
            _pre_restrict = list(child_toolsets)
            child_toolsets = apply_tier_toolset_restriction(
                child_toolsets,
                agent_name=_eden_agent_name or "unknown",
                tier=_tier,
            )
            if child_toolsets != _pre_restrict:
                logger.debug(
                    "Eden tier gate applied: agent=%s tier=%s "
                    "toolsets: %s → %s",
                    _eden_agent_name or "(inherited)", _tier,
                    sorted(_pre_restrict), sorted(child_toolsets),
                )
        except Exception as exc:
            logger.debug(
                "Eden tier gate failed (non-fatal, using unrestricted): %s",
                exc,
            )
    ...
    # Extract parent's API key so subagents inherit auth (e.g. Eden Portal).
    ...
    # ── Eden OE (Phase 2): Inject Eden identity onto the child ────────────
    # _eden_agent_name and _eden_agent_tier are the DB-backed canonical
    # identity. The Governor reads these at execution time for BOUNDARY
    # and ACCORDS checks. The mutable Python attributes are convenience
    # fallbacks — the DB (agents.db → agent_delta) is authoritative.
    if _eden_agent_name:
        child._eden_agent_name = _eden_agent_name
    if _eden_agent_tier:
        child._eden_agent_tier = _eden_agent_tier
    # Inherit parent's Eden identity when the parent itself has one
    # (nested orchestration — parent subagent spawning its own worker).
    if not _eden_agent_name and hasattr(parent_agent, "_eden_agent_name"):
        child._eden_agent_name = parent_agent._eden_agent_name
    if not _eden_agent_tier and hasattr(parent_agent, "_eden_agent_tier"):
        child._eden_agent_tier = parent_agent._eden_agent_tier
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.eden/logs/subagent-<sid>-<ts>.log``
...
def _spill_summary_to_file(task_index: int, summary: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_eden_dir("cache/delegation", "delegation_cache")
...
def _run_single_child(
    ...
    **_kwargs,
    ...
    try:
        ...
        # ── Eden OE (Phase 2): Publish vine.complete to Event Bus ────────
        # The Eden Persistence Engine (eden-persistence daemon) consumes
        # vine.complete events for intent tracking and agent DELTA scoring.
        if hasattr(child, "_eden_agent_name") and child._eden_agent_name:
            try:
                from eden.agents.subagent import publish_vine_complete

                publish_vine_complete(
                    vine_id=_subagent_id or "",
                    agent_name=child._eden_agent_name,
                ...
                logger.debug("vine.complete publish failed (non-fatal): %s", exc)
...
def delegate_task(
    ...
    # ── Eden OE (Phase 2): Resolve parent's Eden identity ─────────────
    # propagate _eden_agent_name / _eden_agent_tier from parent to all
    # children. When the parent itself is an Eden agent, children inherit
    # the parent's identity for Governor BOUNDARY/ACCORDS checks.
    _parent_eden_name: Optional[str] = getattr(parent_agent, "_eden_agent_name", None)
    _parent_eden_tier: Optional[str] = getattr(parent_agent, "_eden_agent_tier", None)

    # ── Eden OE (Phase 2): Governor ACCORDS check before spawn ─────────
    # Before any child is created, verify this agent is authorized to
    # delegate. An agent at tier C trying to spawn a subagent (which
    # requires tier B+) will be denied here before any work starts.
    if _parent_eden_name:
        try:
            from eden.agents.subagent import check_governor_spawn_authorization

            # Determine the effective child tier — if parent tier is known,
            # children inherit it unless explicitly overridden per task.
            _child_tier = _parent_eden_tier or "B"
            if not check_governor_spawn_authorization(
                parent_agent, _parent_eden_name, _child_tier,
            ):
                return json.dumps({
                    "error": (
                        f"Governor DENIED subagent spawn: agent "
                        f"'{_parent_eden_name}' (tier={_child_tier}) is not "
                        f"authorized to delegate tasks. Per 22-BOUNDARY.rule "
                        f"and Eden Accords Article IV, tier escalation is a "
                        f"governance violation."
                    ),
                    "error_type": "eden_governor_denied",
                })
        except PermissionError:
            raise  # re-raise explicit denials
        except Exception as exc:
            logger.debug(
    ...
    try:
        for i, t in enumerate(task_list):
            ...
            child = _build_child_agent(
                ...
                role=effective_role,
                # ── Eden OE (Phase 2): Propagate identity ────────
                _eden_agent_name=t.get("_eden_agent_name") or _parent_eden_name,
                _eden_agent_tier=t.get("_eden_agent_tier") or _parent_eden_tier,
            )
    ...
    if background:
        ...
        try:
            ...
            _source = get_session_env("EDEN_OE_SESSION_SOURCE", "")
            _origin_ui_session_id = get_session_env("EDEN_OE_UI_SESSION_ID", "")
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Available providers: openrouter, eden, zai, kimi-coding, minimax."
        ) from exc
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'eden auth'."
        )
...
def _load_config() -> dict:
    """Load delegation config from the active Eden OE config.

    Prefer the shared persistent loader because it follows the active
    EDEN_HOME/profile. ``cli.CLI_CONFIG`` is a legacy fallback for entry
    ...
    ``EDEN_OE_IGNORE_USER_CONFIG=1`` (``eden chat --ignore-user-config``) is
    ...
    """
    prefer_legacy = os.environ.get("EDEN_OE_IGNORE_USER_CONFIG") == "1"
    if not prefer_legacy:
        try:
            from eden_cli.config import load_config_readonly
