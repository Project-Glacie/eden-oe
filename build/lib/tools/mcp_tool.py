...
transport, discovers their tools, and registers them into the eden-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.eden/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.eden/logs/mcp-stderr.log), tagged with the
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach Eden OE' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            eden_home = os.path.expanduser(
                os.getenv(
                    "EDEN_HOME", os.path.join(os.path.expanduser("~"), ".eden")
                )
            )
            candidates = [
                os.path.join(eden_home, "node", "bin", resolved_command),
                ...
                # image (which the Eden OE Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Eden OE MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Eden OE gateways know how to render.
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Eden OE' existing approval
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (EDEN_OE_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit EDEN_OE_SESSION_PLATFORM, so the
    ...
    def _make_logging_callback(self):
        ...
        server into Eden OE' logging (agent.log via eden_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "  pip install 'eden-agent[mcp]'\n"
                "or (full install):\n"
                "  pip install 'eden-agent[all]'"
            )
        ...
        # ungraceful exit of this Eden OE process (kill -9, crash, force-quit)
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Eden OE' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Eden OE is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `eden mcp login {server_name}` (or delete the tokens "
            f"file under ~/.eden/mcp-tokens/ and restart). Do NOT retry "
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local EDEN_HOME override into ``coro``.
    ...
    try:
        ...
        home_override = get_eden_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_eden_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local EDEN_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_eden_home()
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Eden OE config file.
    ...
    ``os.environ`` (which includes ``~/.eden/.env`` loaded at startup).
    """
    try:
        from eden_cli.config import load_config
        from utils import env_var_enabled as _env_enabled

        if _env_enabled("EDEN_OE_SAFE_MODE"):
            return {}
        config = load_config()
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Eden OE' MEDIA: tag convention and out to messaging
            ...
            # Eden OE' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Eden OE's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. Eden OE uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Eden OE registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``eden tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Eden OE registry.
...
def _kill_orphaned_mcp_children(
    ...
    survivors, avoiding shared-resource collisions when multiple eden
