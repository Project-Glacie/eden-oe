...
def managed_nous_tools_enabled(*, force_fresh: bool = False) -> bool:
    """Return True when the user is entitled to the Eden Tool Gateway.

    Entitlement is paid Eden Portal service access OR a live free tool pool
...
def nous_tool_gateway_unavailable_message(
    ...
    *,
    ...
    """Return account-aware guidance for an unavailable Eden Tool Gateway path."""
    try:
        ...
        pass
    return (
        f"{capability} is unavailable. Run `eden model` to refresh your "
        "Eden Portal login and billing status."
    )
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.eden/.env`` (via
    ``eden_cli.config.get_env_value`` when available) so tool-side
