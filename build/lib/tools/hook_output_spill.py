...
Config (``config.yaml``)::
    ...
    hooks:
      output_spill:
        ...
        directory: null        # default: <EDEN_HOME>/hook_outputs
...
* Never raises. Any I/O error (disk full, permission denied, missing
  EDEN_HOME, etc.) falls back to a byte-length truncation with an
...
def _resolve_spill_dir(directory_override: Optional[str], session_id: Optional[str]) -> Path:
    ...
    if directory_override:
        ...
        try:
            ...
            base = Path(get_eden_home()) / "hook_outputs"
        except Exception:
            # Last-resort fallback: EDEN_HOME env var, then ~/.eden
            home = os.environ.get("EDEN_HOME") or os.path.expanduser("~/.eden")
