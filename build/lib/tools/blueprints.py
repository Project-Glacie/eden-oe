...
`eden skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.eden.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.eden.blueprint`` is a mapping containing
    ...
    eden = meta.get("eden") if isinstance(meta, dict) else None
    blueprint = eden.get("blueprint") if isinstance(eden, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.eden.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.eden.blueprint`` block) they can hand
    to ``eden skills publish`` to share. ``body`` is the plain-language
