...
_UNSET = object()
_EDEN_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_EDEN_HOME_OVERRIDE", default=_UNSET
)


def set_eden_home_override(path: str | Path | None) -> Token:
    """Set a context-local Eden OE home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _EDEN_HOME_OVERRIDE.set(value)


def reset_eden_home_override(token: Token) -> None:
    """Restore the previous context-local Eden OE home override."""
    _EDEN_HOME_OVERRIDE.reset(token)


def get_eden_home_override() -> str | None:
    """Return the active context-local Eden OE home override, if any."""
    override = _EDEN_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_eden_home() -> Path:
    """Return the platform-native default Eden OE home path."""
...
def get_eden_home() -> Path:
    """Return the Eden OE home directory (default: platform-native path).

    Reads EDEN_HOME env var, falls back to the platform-native default.
    This is the single source of truth — all other copies should import this.

    When ``EDEN_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``EDEN_HOME`` explicitly (see the systemd
    template in ``eden_cli/gateway.py`` and the kanban dispatcher in
    ``eden_cli/kanban_db.py``).  See https://github.com/ProjectGlacie/eden-agent/issues/18594.
    """
    override = get_eden_home_override()
    if override:
        return Path(override)

    val = os.environ.get("EDEN_HOME", "").strip()
    if val:
        return Path(val)
    ...
    if not _profile_fallback_warned:
        try:
            fallback_home = _get_platform_default_eden_home()
        ...
        if active and active != "default":
            ...
            msg = (
                f"[EDEN_HOME fallback] EDEN_HOME is unset but active "
                f"profile is {active!r}. Falling back to {fallback_home}, which "
                f"is the DEFAULT profile — not {active!r}. Any data this "
                f"process writes will land in the wrong profile. The "
                f"subprocess spawner should pass EDEN_HOME explicitly "
                f"(see issue #18594)."
            )
            try:
    ...
    return _get_platform_default_eden_home()


def get_default_eden_root() -> Path:
    """Return the root Eden OE directory for profile-level operations.

    In standard deployments this is the platform-native Eden OE home
    (``~/.eden`` on POSIX, ``%LOCALAPPDATA%\\eden`` on native Windows).

    In Docker or custom deployments where ``EDEN_HOME`` points outside
    ``~/.eden`` (e.g. ``/opt/data``), returns ``EDEN_HOME`` directly
    — that IS the root.

    In profile mode where ``EDEN_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.eden/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = _get_platform_default_eden_home()
    ...
    try:
        ...
        # EDEN_HOME is under ~/.eden (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — EDEN_HOME itself is the root
    return env_path
...
def _get_packaged_data_dir(name: str) -> Path | None:
    ...
    Used to discover bundled skills/optional-skills when Eden OE is installed
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``EDEN_OE_OPTIONAL_SKILLS``.
    """
    override = os.getenv("EDEN_OE_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_eden_home() / "optional-skills"


def get_optional_mcps_dir(default: Path | None = None) -> Path:
    """Return the optional-mcps directory, honoring package-manager wrappers.

    Mirrors :func:`get_optional_skills_dir` for the MCP catalog (Eden-approved
    Model Context Protocol servers shipped with the repo but disabled by
    default). Packaged installs may ship ``optional-mcps`` outside the Python
    package tree and expose it via ``EDEN_OE_OPTIONAL_MCPS``.
    """
    override = os.getenv("EDEN_OE_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_eden_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``EDEN_OE_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Wheel-installed ``<sysconfig data>/skills`` (pip install path)
        3. Caller-supplied ``default`` (typically the source-checkout path)
        4. ``<EDEN_HOME>/skills`` last-resort
    """
    override = os.getenv("EDEN_OE_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_eden_home() / "skills"


def get_eden_dir(new_subpath: str, old_name: str) -> Path:
    """Resolve a Eden OE subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to EDEN_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to EDEN_HOME (e.g. ``"image_cache"``).
...
def iter_eden_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Eden-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\eden\\node``. POSIX installs use
    ``$EDEN_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_eden_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with edenManagedNodePathEntries() in
...
_EDEN_OE_NODE_TARGET_MAJOR = int(os.environ.get("EDEN_OE_NODE_TARGET_MAJOR", "22"))
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Eden-managed Node trees live under ``$EDEN_HOME/node`` (or a profile's
    ``EDEN_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_eden_node_executable``
    used to trust file presence alone, so ``eden update`` would pick that
...
def eden_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Eden-managed node/npm/npx shim exists on disk."""
    names = set()
    for command in ("node", "npm", "npx"):
        names.update(_candidate_node_command_names(command))
    for directory in iter_eden_node_dirs(home):
        for name in names:
...
def _heal_managed_node_windows() -> bool:
    """Redownload the portable Node zip into ``%EDEN_HOME%\\node`` on Windows."""
    import re
    import tempfile
    ...
    home = get_eden_home()
    index_url = f"https://nodejs.org/dist/latest-v{_EDEN_OE_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_EDEN_OE_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def heal_eden_managed_node() -> bool:
    """Redownload Eden-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not eden_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "EDEN_HOME": str(get_eden_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def find_eden_node_executable(command: str) -> str | None:
    """Return a Eden-managed Node/npm executable path, healing broken trees."""
    names = _candidate_node_command_names(command)
    broken_present = False
    for directory in iter_eden_node_dirs():
        for name in names:
            candidate = directory / name
            if candidate.is_file() and (
                sys.platform == "win32" or os.access(candidate, os.X_OK)
                ...
                if node_tool_runnable(resolved):
                    return resolved
                broken_present = True
    if broken_present and heal_eden_managed_node():
        for directory in iter_eden_node_dirs():
            for name in names:
                candidate = directory / name
                if candidate.is_file() and (
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Eden-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Eden-managed installs.

    This is for Eden-owned subprocesses that should not be broken by a bad,
    ...
    """
    managed = find_eden_node_executable(command)
    if managed:
        return managed
    if eden_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_eden_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Eden-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_eden_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``eden update`` — leaving a **dangling symlink** that ``which`` still
...
def display_eden_home() -> str:
    """Return a user-friendly display string for the current EDEN_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.eden``
        profile:  ``~/.eden/profiles/coder``
        custom:   ``/opt/eden-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.eden``.  For code that needs a real ``Path``, use
    :func:`get_eden_home` instead.
    """
    home = get_eden_home()
    try:
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``EDEN_HOME`` or other path
    env vars resolve to an unexpected location.

    See https://github.com/ProjectGlacie/eden-agent/issues/25821.
    """
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{EDEN_HOME}/home`` when the profile-home directory exists."""
    eden_home = get_eden_home_override() or (env or {}).get("EDEN_HOME") or os.getenv("EDEN_HOME")
    if not eden_home:
        return None
    profile_home = os.path.join(eden_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("EDEN_OE_REAL_HOME") or os.getenv("EDEN_OE_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Eden OE profile HOME.

    ``EDEN_HOME`` scopes Eden OE state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={EDEN_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{EDEN_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{EDEN_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Eden OE' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["EDEN_OE_REAL_HOME"] = real_home
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when Eden OE itself runs inside WSL.
...
def is_container() -> bool:
    ...
    See: ProjectGlacie/eden-agent#47111
    """
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under EDEN_HOME.

    Replaces the ``get_eden_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, eden_logging.py, eden_time.py, etc.).
    """
    return get_eden_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under EDEN_HOME."""
    return get_eden_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under EDEN_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_eden_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._eden_ipv4_patched = True  # type: ignore[attr-defined]
