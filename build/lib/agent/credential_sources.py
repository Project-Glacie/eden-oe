"""Unified removal contract for every credential source Eden OE reads from.

Eden seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.eden/.env
    claude_code   — ~/.claude/.credentials.json
    eden_pkce   — ~/.eden/.anthropic_oauth.json
    device_code   — auth.json providers.<provider> (eden, openai-codex, ...)
    qwen-cli      — ~/.qwen/oauth_creds.json
    gh_cli        — gh auth token
    config:<name> — custom_providers config entry
    model_config  — model.api_key when model.provider == "custom"
    manual        — user ran `eden auth add`
...
unify here is **removal**:
    ...
    ``eden auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, eden device_code (partial), eden_pkce, copilot gh_cli, and
...
class RemovalStep:
    ...
    Attributes:
        provider: Provider pool key (``"xai"``, ``"anthropic"``, ``"eden"``, ...).
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.eden/.env  → clear it
    ...
    """
    from eden_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to Eden OE.",
            f"  The pool entry is now suppressed — Eden OE will ignore "
            f"{env_var} until you run `eden auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Eden OE stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `eden auth add anthropic` to re-enable if needed.",
    ])


def _remove_eden_pkce(provider: str, removed) -> RemovalResult:
    """~/.eden/.anthropic_oauth.json is ours — delete it outright."""
    from eden_constants import get_eden_home

    result = RemovalResult()
    oauth_file = get_eden_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Eden OE Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    """Eden OAuth lives in auth.json providers.eden — clear it and suppress.

    We suppress in addition to clearing because nothing else stops a future
    `eden auth add eden` (or any other path that writes providers.eden)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `eden auth add eden` to re-engage, which is the
...
def _remove_minimax_oauth(provider: str, removed) -> RemovalResult:
    ...
    Same pattern as Eden: single-source OAuth state with refresh tokens.
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``eden auth remove xai-oauth <N>`` silently undoes
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `eden model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Eden OE auth store is not enough — _seed_from_singletons() would
    ...
    via ``eden auth add openai-codex``), but in both cases the re-seed
    ...
    """
    from eden_cli.auth import suppress_credential_source
    ...
    result.hints.extend([
        ...
        "Run `eden auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `eden auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Eden OE stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `eden auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="eden_pkce",
        remove_fn=_remove_eden_pkce,
        description="~/.eden/.anthropic_oauth.json",
    ))
    register(RemovalStep(
        provider="eden", source_id="device_code",
        remove_fn=_remove_nous_device_code,
        description="auth.json providers.eden",
    ))
    register(RemovalStep(
