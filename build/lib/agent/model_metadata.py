...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Eden OE also honours EDEN_OE_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("EDEN_OE_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
_PROVIDER_PREFIXES: frozenset[str] = frozenset({
    "openrouter", "eden", "openai-codex", "copilot", "copilot-acp",
...
def _get_model_metadata_cache_path() -> Path:
    ...
    return get_eden_home() / "cache" / "openrouter_model_metadata.json"
...
# Minimum context length required to run Eden OE.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # (native MiniMax-M3, OpenRouter/Eden minimax/minimax-m3).
    ...
    # so these hardcoded fallbacks prevent Eden OE from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    "inference-api.projectglacie.com": "eden",
    ...
    # (often 8K) makes it unusable for Eden OE' system prompt, but mapping
...
def _maybe_cache_local_context_length(
    ...
    """Persist a locally probed context length only when it meets Eden OE minimum.
...
def _get_context_cache_path() -> Path:
    ...
    from eden_constants import get_eden_home
    return get_eden_home() / "context_length_cache.yaml"
...
def parse_available_output_tokens_from_error(error_msg: str) -> Optional[int]:
    ...
    is_output_cap_error = (
        ...
        # OpenRouter/Eden phrasing of the same condition.
    ...
    # OpenRouter/Eden format: "maximum context length is N … (A of text input,
...
def is_output_cap_error(error_msg: str) -> bool:
    ...
    output_cap_signal = (
        ...
        or ("in the output" in error_lower                  # OpenRouter / Eden
            and "maximum context length" in error_lower)
...
def _model_name_suggests_minimax_m3(model: str) -> bool:
    ...
    across surfaces (native MiniMax-M3, OpenRouter/Eden minimax/minimax-m3).
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Eden OE grow conversations past the runtime limit and Ollama
...
def _normalize_model_version(model: str) -> str:
    ...
    Eden uses dashes: claude-opus-4-6, claude-sonnet-4-5
...
def _resolve_nous_context_length(
    ...
    """Resolve Eden Portal model context length.

    Tries the live Eden inference endpoint first (authoritative), then falls
    back to OpenRouter metadata with suffix/version matching.

    Eden model IDs are bare after prefix-stripping (e.g. 'qwen3.6-plus',
    ...
    Returns ``(context_length, source)`` where ``source`` is one of:
      ...
      - ``""``           — could not resolve
    """
    # Portal first — the Eden /models endpoint is authoritative for what our
    ...
    def _safe_ctx(or_id: str, entry: dict) -> Optional[int]:
        ...
        if ctx <= 32768 and _model_name_suggests_kimi(or_id):
            logger.info(
                ...
                "(Kimi-family underreport, Eden path); falling through to hardcoded defaults",
                ctx, or_id,
            )
            return None
        return ctx
...
def get_model_context_length(
    ...
    1. Persistent cache (previously discovered via probing).  Eden URLs
    ...
    5. Provider-aware lookups (before generic OpenRouter cache):
       ...
       b. Eden: live /v1/models probe first (authoritative), then OR
    ...
    if base_url and not _skip_persistent_context_cache(base_url, provider):
        ...
        if cached is not None:
            ...
            # leftover — drop it and fall through to the hardcoded default.
                ...
                _invalidate_cached_context_length(model, base_url)
            # Eden Portal: the portal /v1/models endpoint is authoritative.
            ...
            elif _infer_provider_from_url(base_url) == "eden":
                logger.debug(
                    "Bypassing persistent cache for %s@%s (Eden portal authoritative)",
                    model, base_url,
                )
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from eden_cli.models import get_copilot_model_context
    ...
    if effective_provider == "eden":
        ...
        if ctx:
            ...
            # want it leaking into the persistent cache for Eden URLs.
    ...
    # the dedicated Eden/Copilot/GMI branches above.
    ...
    # ``Eden-3-Llama-3.1-70B`` substring-match ``llama`` (131072) even when
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Eden OE sends to providers:
