...
Storage
-------
Bundles live in ``~/.eden/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``EDEN_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``eden bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under EDEN_HOME.

    Honors ``EDEN_OE_BUNDLES_DIR`` for tests; falls back to
    ``<EDEN_HOME>/skill-bundles``.
    """
    override = os.environ.get("EDEN_OE_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_eden_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `eden bundles` CLI subcommand.
# ---------------------------------------------------------------------------
