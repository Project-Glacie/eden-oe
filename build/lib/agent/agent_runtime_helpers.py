...
def try_recover_primary_transport(
    ...
    Skipped for proxy/aggregator providers (OpenRouter, Eden) which
    ...
    if provider_lower in {"eden", "project-glacie"}:
        return False
...
def dump_api_request_debug(
    ...
    *,
    ...
    try:
        ...
        # originate from untrusted input (X-Eden-Session-Id header), and an
        ...
        if env_var_enabled("EDEN_OE_DUMP_REQUEST_STDOUT"):
...
def anthropic_prompt_cache_policy(
    ...
    *,
    ...
    if eff_provider.strip().lower() == "moa":
        try:
            from eden_cli.config import load_config as _load_moa_cfg
    ...
    # Eden Portal proxies to OpenRouter behind the scenes — identical
    # OpenAI-wire envelope cache_control semantics. Treat it as an
    # OpenRouter-equivalent endpoint for caching layout purposes.
    is_nous_portal = "projectglacie" in eff_base_url.lower()
    ...
    if (is_openrouter or is_nous_portal) and (is_claude or is_kimi):
        return True, False
    # Eden Portal Qwen (e.g. qwen3.6-plus) takes the same envelope-layout
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    # Delegate all rate-limit / 5xx retry to eden's outer conversation loop,
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    turn-scoped).
    """
    from eden_cli.providers import determine_api_mode
    ...
    if hasattr(agent, "context_compressor") and agent.context_compressor:
        ...
        try:
            from eden_cli.config import load_config, get_compatible_custom_providers
            _sm_cfg = load_config()
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    try:
        from eden_cli.middleware import apply_tool_request_middleware
    ...
    if not pre_tool_block_checked:
        try:
            from eden_cli.plugins import resolve_pre_tool_block
    ...
    # ── Eden Governor pre-execution constitutional gate ──────────────
    # Phase 1c: All 7 constitutional checks must pass before any tool
    # executes. The Governor runs as an in-process Python module;
    # a bridge to the Rust Governor daemon (eden-governor.service) is
    # planned when the HTTP endpoint becomes available.
    # See: eden/governor.py, PLAYBOOK-EDEN-OE-COMPLETION Phase 1c
    try:
        from eden.governor import eden_check_tool as _eden_check
    except ImportError as _eden_import_error:
        import logging as _eden_logging
        _eden_log = _eden_logging.getLogger("eden_governor")
        _eden_log.critical(
            "EDEN GOVERNOR FAILED TO LOAD — ALL CONSTITUTIONAL CHECKS DISABLED. "
            "ImportError: %s. "
            "This is a CRITICAL security condition. No tool calls are being "
            "constitutionally gated. The Governor module at eden/governor.py "
            "may be corrupted, missing, or have a syntax error. "
            "Restore the Governor module immediately.",
            _eden_import_error,
        )
        _eden_check = None

    if _eden_check is not None:
        _eden_agent_name = getattr(agent, "_eden_agent_name", "")
        _eden_agent_tier = getattr(agent, "_eden_agent_tier", "B")
        _eden_session = getattr(agent, "session_id", "") or ""
        _governor_decision = _eden_check(
            function_name,
            function_args,
            agent_name=_eden_agent_name,
            agent_tier=_eden_agent_tier,
            session_id=_eden_session,
        )
        if not _governor_decision.permitted:
            denied_result = json.dumps(
                {"error": _governor_decision.reason, "governor_denied": True},
                ensure_ascii=False,
            )
            try:
                from model_tools import _emit_post_tool_call_hook
                _emit_post_tool_call_hook(
                    ...
                    api_request_id=getattr(agent, "_current_api_request_id", "") or "",
                    status="blocked",
                    error_type="eden_governor_deny",
                    error_message=_governor_decision.reason,
                    middleware_trace=list(_tool_middleware_trace),
                )
            except Exception:
                pass
    ...
    from eden_cli.middleware import run_tool_execution_middleware
...
def sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    #
    # We do NOT drop the call: eden' own dispatch loop intentionally keeps an
