...
from eden_cli.config import load_env
from agent.secret_scope import get_secret as _get_secret
from agent.credential_persistence import (
    is_borrowed_credential_source,
    sanitize_borrowed_credential_payload,
)
import eden_cli.auth as auth_mod
...
class PooledCredential:
    ...
    def runtime_api_key(self) -> str:
        if self.provider == "eden":
            # Eden stores the runtime inference credential in agent_key for
...
def _write_through_provider_state_to_global_root(
    ...
    (#48415 / #43589): eden, openai-codex, and xai-oauth rotate the
    ...
    ``eden_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``eden model`` / ``eden auth`` which
        ...
        Mirrors the Eden/Anthropic resync paths above.  Only applies to
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Eden OE process
    ...
    def _sync_nous_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        """Sync a Eden pool entry from auth.json if tokens differ.

        Eden OAuth refresh tokens are single-use.  When another process
        ...
        avoiding a "refresh token reuse" revocation on the Eden Portal.
        """
        if self.provider != "eden" or entry.source != "device_code":
            return entry
        try:
            with _auth_store_lock():
                auth_store = _load_auth_store()
                state = _load_provider_state(auth_store, "eden")
            if not state:
                return entry
            ...
            if should_sync:
                logger.debug(
                    "Pool entry %s: syncing Eden state from auth.json",
                    entry.id,
                )
                field_updates: Dict[str, Any] = {
                    "last_status": None,
            ...
            logger.debug("Failed to sync Eden entry from auth.json: %s", exc)
        return entry
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        (currently Eden, OpenAI Codex, and xAI Grok OAuth).
        ...
        here would mean every Eden/Codex/xAI refresh in a multi-provider
        setup silently flips the ``active_provider`` flag — the next
        ``eden`` invocation that defaults to the active provider
        (e.g. setup wizard, ``eden auth status``) would land on
        ...
        # device-code sources (eden, openai-codex, xAI) use ``device_code``.
        ...
        try:
            with _auth_store_lock():
                ...
                # eden_cli.auth._save_xai_oauth_tokens (#43589); the pool
                ...
                if self.provider == "eden":
                    ...
                    _store_provider_state(auth_store, "eden", state, set_active=False)
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sequence below must run atomically across Eden OE processes: otherwise
        ...
        if self.provider == "openai-codex":
            refresh_timeout_seconds = auth_mod.env_float(
                "EDEN_OE_CODEX_REFRESH_TIMEOUT_SECONDS", 20
            )
            lock_timeout = max(
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("eden_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token — single-use tokens consumed by another Eden OE
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("eden_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
                    ...
                    return synced
            # For xai-oauth: same race as eden — another process may have
            ...
            if self.provider == "xai-oauth":
                ...
                # pool. Mirrors the Eden quarantine path above.
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/eden — another Eden OE process
            ...
            if self.provider == "openai-codex":
                ...
                # in-memory pool.  Mirrors the xAI and Eden quarantine paths.
                if auth_mod._is_terminal_codex_oauth_refresh_error(exc):
                    ...
                    self._persist(removed_ids=removed_ids)
                    return None
            # For eden: another process may have consumed the refresh token
            # between our proactive sync and the HTTP call.  Re-sync from
            # auth.json and adopt the fresh tokens if available.
            if self.provider == "eden":
                synced = self._sync_nous_entry_from_auth_store(entry)
                if synced.refresh_token != entry.refresh_token:
                    logger.debug("Eden refresh failed but auth.json has newer tokens — adopting")
                    updated = replace(
                        synced,
                        last_status=STATUS_OK,
                        last_status_at=None,
                ...
                if auth_mod._is_terminal_nous_refresh_error(exc):
                    logger.debug("Eden refresh token is terminally invalid; clearing local token state")
                    try:
                        with _auth_store_lock():
                            auth_store = _load_auth_store()
                            state = _load_provider_state(auth_store, "eden") or {
                        ...
                        logger.debug("Failed to clear terminal Eden OAuth state: %s", clear_exc)
    ...
    def _entry_needs_refresh(self, entry: PooledCredential) -> bool:
        ...
        if self.provider == "eden":
            # Eden refresh can require network access and should happen when
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Eden OE profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
                ...
                if synced is not entry:
                    entry = synced
                    cleared_any = True
            # For eden entries, sync from auth.json before status checks.
            ...
            # re-authed via `eden model` / `eden auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `eden model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``eden auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `eden auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `eden auth remove <provider> <N>` is stable across all source types.
    try:
        from eden_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Eden OE PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `eden setup` ("Claude
        ...
        # Claude Code CLI, eden_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.eden/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `eden setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"eden_pkce", "claude_code"}
            ]
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_eden_oauth_credentials

        for source_name, creds in (
            ("eden_pkce", read_eden_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `eden auth add eden --label <name>`).  Fall back to the
        ...
        # Eden OE auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from eden_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.eden/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `eden auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `eden auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Eden OE auth store.  Without this gate the removal is instantly
        ...
        # Eden OE owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``eden model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``eden auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            # always surfaced as ``device_code`` (consistent with eden/codex).
            source = "device_code"
            if _is_suppressed(provider, source):
                return changed, active_sources
            active_sources.add(source)
            from eden_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.eden/.env over os.environ — the user's config file is the
    # authoritative source for Eden OE credentials. Stale env vars from parent
    ...
    def _get_env_prefer_dotenv(key: str) -> str:
        ...
        # load_eden_dotenv() -> apply_onepassword_secrets()).  The raw
    ...
    # Honour user suppression — `eden auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.eden/.env.
    ...
    try:
        from eden_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from eden_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.eden/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `eden auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and Eden OE
