...
cache problem must never block Eden OE startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Eden OE home used for cache paths.

    ``home_path`` is whatever ``load_eden_dotenv()`` already resolved;
    falling back to ``$EDEN_HOME`` / ``~/.eden`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        home_path = Path(os.getenv("EDEN_HOME", Path.home() / ".eden"))
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<eden_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if EDEN_HOME is ever made traversable.
            try:
