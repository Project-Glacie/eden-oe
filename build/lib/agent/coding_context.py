"""Coding-context awareness — base Eden OE, every interactive surface.

When the user runs Eden OE inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), Eden OE shifts into a **coding posture**. This module is the
...
logger = logging.getLogger("eden.coding_context")
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         "eden", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
...
# in _EDEN_CORE_TOOLS, so they're present on every surface this fires on.
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``EDEN_OE_TUI_TOOLSETS``, …); they never
...
def is_coding_context(
    *,
    ...
    """Whether Eden OE should operate in its coding posture right now."""
