...
from eden_cli.auth import AuthError, _read_codex_tokens, resolve_codex_runtime_credentials
...
def build_nous_credits_snapshot(account_info) -> Optional[AccountUsageSnapshot]:
    ...
    """
    try:
        from eden_cli.eden_account import nous_portal_topup_url
...
def nous_credits_lines(*, markdown: bool = False, timeout: float = 10.0) -> list[str]:
    """Return rendered Eden-credits /usage lines, or [] when there's nothing to show.

    Account-independent of any live agent: gated on "a Eden account is logged in"
    ...
    Dev override: when EDEN_OE_DEV_CREDITS_FIXTURE selects a fixture state, /usage
    ...
    try:
        ...
        tok = (get_provider_auth_state("eden") or {}).get("access_token")
...
def _snapshot_from_credits_state(state) -> Optional[AccountUsageSnapshot]:
    ...
    so EDEN_OE_DEV_CREDITS_FIXTURE can exercise /usage without a live account. The
    *_usd strings are mock display values here (not server balance to compute on);
    ...
    try:
        ...
        details.append("(dev fixture — EDEN_OE_DEV_CREDITS_FIXTURE)")
        return AccountUsageSnapshot(
            provider="eden",
            source="dev-fixture",
            fetched_at=_utc_now(),
...
def _resolve_codex_usage_credentials(
    ...
    state, then the credential pool.  Eden OE's native OAuth setup now stores
