...
def _resolve_concurrent_tool_timeout() -> float | None:
    raw = os.getenv("EDEN_OE_CONCURRENT_TOOL_TIMEOUT_S", "").strip()
    if not raw:
    ...
    try:
        ...
        logger.warning(
            "invalid EDEN_OE_CONCURRENT_TOOL_TIMEOUT_S=%r; using %.0fs",
            raw,
...
def _flush_session_db_after_tool_progress(
    ...
    *,
    ...
    current Eden OE process before the normal turn-end persistence path runs.
...
def execute_tool_calls_concurrent(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        if _ts_scope_block is not None:
            ...
            if block_message is not None:
                ...
                if not guardrail_decision.allows_execution:
                    ...
                    # ── Eden Governor — Insertion 2: DURING-TURN ──────
                    # Per governance spec §2.2: EdenToolPolicy check before
                    # tool dispatch.  Blocks tools that violate lane, tier,
                    # delegation, covenant, or playbook constraints.
                    try:
                        from eden.governor.policy import EdenToolPolicy
                        _eden_policy = EdenToolPolicy()
                        _eden_verdict = _eden_policy.check(
                            tool_name=function_name,
                            agent_lane=getattr(agent, "_eden_agent_lane",
                                getattr(agent, "lane", "DEV")),
                            agent_tier=getattr(agent, "_eden_agent_tier",
                                getattr(agent, "tier", "B")),
                            has_delegation=getattr(agent, "_eden_has_delegation", False),
                            have_notified=getattr(agent, "_eden_haven_notified", False),
                            active_playbook=getattr(agent, "_eden_active_playbook", False),
                        )
                        if _eden_verdict.get("blocked"):
                            block_result = json.dumps({
                                "error": f"Governor: DENIED — {_eden_verdict.get('reason', 'policy violation')}",
                            }, ensure_ascii=False)
                            blocked_by_guardrail = True
                            _emit_terminal_post_tool_call(
                                agent,
                                ...
                                tool_call_id=getattr(tool_call, "id", "") or "",
                                status="blocked",
                                error_type="eden_governor_block",
                                error_message=_eden_verdict.get("reason", "Blocked by Eden Governor policy"),
                                middleware_trace=list(middleware_trace),
                            )
                    except ImportError:
                        pass  # Eden Governor not installed
                    except Exception as _eden_tool_err:
                        logger.debug("Eden Governor tool check failed (non-fatal): %s", _eden_tool_err)
    ...
    for i, (tc, name, args, middleware_trace, block_result, blocked_by_guardrail) in enumerate(parsed_calls):
        ...
        # ── Eden Scratchpad — Insertion 3.5: TOOL OUTPUT SUMMARIZATION ──
        # Per governance spec Phase 3.5: summarize tool output locally
        # before injecting into cloud context.  500-line build outputs
        # → 3-line summaries.  Prevents prefix cache thrashing (July 12
        # spike: 12.6M cache misses from raw injection).
        try:
            from eden.scratchpad import summarize
            function_result = summarize(name, function_result)
        except ImportError:
            pass  # Eden Scratchpad not installed
        except Exception as _eden_scratchpad_err:
            logger.debug("Eden Scratchpad summarization failed (non-fatal): %s", _eden_scratchpad_err)
    ...
    for i, tool_call in enumerate(assistant_message.tool_calls, 1):
        ...
        if _block_msg is None:
            ...
            if not guardrail_decision.allows_execution:
                ...
                # ── Eden Governor — Insertion 2: DURING-TURN ──────────
                # Per governance spec §2.2: EdenToolPolicy check before
                # tool dispatch in the sequential execution path.
                try:
                    from eden.governor.policy import EdenToolPolicy
                    _eden_policy = EdenToolPolicy()
                    _eden_verdict = _eden_policy.check(
                    ...
                    if _eden_verdict.get("blocked"):
                        _eden_block_reason = _eden_verdict.get("reason", "policy violation")
                        _eden_block_msg = f"Governor: DENIED — {_eden_block_reason}"
                        _block_msg = _eden_block_msg
                        _block_error_type = "eden_governor_block"
                except ImportError:
                    pass  # Eden Governor not installed
                except Exception as _eden_tool_err:
                    logger.debug("Eden Governor tool check failed (non-fatal): %s", _eden_tool_err)
        ...
        try:
            from eden.scratchpad import summarize
            function_result = summarize(function_name, function_result)
        except ImportError:
            pass  # Eden Scratchpad not installed
        except Exception as _eden_scratchpad_err:
            logger.debug("Eden Scratchpad summarization failed (non-fatal): %s", _eden_scratchpad_err)
