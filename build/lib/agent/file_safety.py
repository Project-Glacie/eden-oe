...
def _eden_home_path() -> Path:
    """Resolve the active EDEN_HOME (profile-aware) without circular imports."""
    try:
        from eden_constants import get_eden_home  # local import to avoid cycles
        return get_eden_home()
    except Exception:
        return Path(os.path.expanduser("~/.eden"))


def _eden_root_path() -> Path:
    """Resolve the Eden OE root dir (always the parent of any profile, never per-profile)."""
    try:
        from eden_constants import get_default_eden_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    eden_home = _eden_home_path()
    eden_root = _eden_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(eden_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(eden_root / ".anthropic_oauth.json"),
...
def get_safe_write_roots() -> set[str]:
    """Return resolved EDEN_OE_WRITE_SAFE_ROOT paths. Supports multiple directories
    separated by ``os.pathsep`` (``:`` on Unix, ``;`` on Windows).
    E.g., ``/opt/data:/var/www/html`` on Unix, ``C:\\data;D:\\www`` on Windows."""
    env = os.getenv("EDEN_OE_WRITE_SAFE_ROOT", "")
    if not env:
        return set()
    roots: set[str] = set()
...
def is_write_denied(path: str) -> bool:
    ...
    eden_dirs = []
    for base in (_eden_home_path(), _eden_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in eden_dirs:
                eden_dirs.append(real)
        except Exception:
            continue

    for base_real in eden_dirs:
        try:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Eden OE path.

    Three categories are blocked:

      * Internal Eden OE cache files under ``EDEN_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under EDEN_HOME and the global Eden OE
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.eden/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active EDEN_HOME (profile-aware) AND the global
    # Eden OE root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (EDEN_HOME points at
    ...
    for hd in eden_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Eden OE cache file "
    ...
    for hd in eden_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Eden OE credential store "
    ...
    for hd in eden_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Eden OE MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Eden OE MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Eden OE read (see
...
#
# Eden OE profiles are separate EDEN_HOME dirs under
...
#
# Reference: May 2026 incident where a eden-security profile session
# edited skills under both ``~/.eden/profiles/eden-security/skills/``
# AND ``~/.eden/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under EDEN_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from EDEN_HOME.
    ...
    """
    try:
        home_real = _eden_home_path().resolve()
        root_real = _eden_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Eden OE scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Eden OE entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Eden OE profile {info['target_profile']!r}, but the "
...
#
#   <EDEN_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.eden/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.eden/…`` segment and warns
# regardless of which Eden OE profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.eden/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.eden`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.eden/…`` and returns the
    index where the inner Eden-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .eden / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".eden":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Eden OE state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.eden``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.eden`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Eden OE resolver to
    succeed, so it works correctly even when called from contexts where
    EDEN_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Eden OE process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real EDEN_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.eden/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.eden/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host EDEN_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Eden OE state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Eden OE "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real EDEN_HOME. Use the "
