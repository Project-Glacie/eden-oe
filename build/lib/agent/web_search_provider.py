...
``kind: backend``) or ``~/.eden/plugins/web/<name>/`` (user, opt-in via
``plugins.enabled``).
...
def get_provider_env(name: str) -> str:
    ...
    Resolves *name* via :func:`eden_cli.config.get_env_value` (checks
    ``os.environ`` first, then ``~/.eden/.env``) so credentials set
    through Eden OE' config layer are visible even when they were never
...
class WebSearchProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``eden tools``. Defaults to ``name``."""
        return self.name
    ...
    def is_available(self) -> bool:
        ...
        runs at tool-registration time and on every ``eden tools`` paint.
        """
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``eden tools`` picker.

        Used by ``eden_cli/tools_config.py`` to inject this provider as a
