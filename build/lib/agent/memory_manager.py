...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_EDEN_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Eden OE expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``eden_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_eden_home()`` themselves.
        """
        if "eden_home" not in kwargs:
            from eden_constants import get_eden_home
            kwargs["eden_home"] = str(get_eden_home())
        for provider in self._providers:
            try:
