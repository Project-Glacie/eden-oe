...
A pre-run snapshot of ``~/.eden/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.eden/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.eden/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def _backups_dir() -> Path:
    return get_eden_home() / "skills" / ".curator_backups"
...
def _cron_jobs_file() -> Path:
    """Source path for the live cron jobs store (``~/.eden/cron/jobs.json``)."""
    return get_eden_home() / "cron" / "jobs.json"
...
def snapshot_skills(reason: str = "manual", *, protect_ids: Optional[Set[str]] = None) -> Optional[Path]:
    """Create a tar.gz snapshot of ``~/.eden/skills/`` and prune old ones.
    ...
    if not skills.exists():
        logger.debug("No ~/.eden/skills/ directory — nothing to back up")
        return None
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.eden/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.eden/skills/``.
    ...
    if target is None:
        return (
            ...
            + " (use `eden curator rollback --list` to see available snapshots)",
            None,
        )
