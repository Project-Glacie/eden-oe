...
projection into Eden OE' display, approval bridging, transcript projection into
...
"codex_app_server"`. Eden OE' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Eden OE secret — gateway
        ...
        spawn_env = eden_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("EDEN_OE_KANBAN_TASK"):
            kanban_db = spawn_env.get("EDEN_OE_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "EDEN_OE_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("EDEN_HOME", os.path.expanduser("~/.eden")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "eden",
        client_title: str = "Eden OE",
