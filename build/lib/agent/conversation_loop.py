...
from eden_constants import PARTIAL_STREAM_STUB_ID
from eden_logging import set_session_context
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Eden OE tool use: "
    ...
    return (
        ...
        f"context, but Eden OE needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In Eden OE config, set `model.ollama_num_ctx: 65536` "
...
def _is_nous_inference_route(provider: str, base_url: str) -> bool:
    ...
    return (
        base_url_host_matches(base, "inference-api.projectglacie.com")
        or base_url_host_matches(base, "inference.projectglacie.com")
    )
...
def _try_refresh_nous_paid_entitlement_credentials(agent) -> bool:
    """Refresh Eden runtime credentials after a fresh paid-entitlement check."""
    try:
...
_CONTENT_POLICY_RECOVERY_HINT = (
    ...
    "adding a fallback provider with `eden fallback add`."
)
...
def run_conversation(
    ...
    if moa_config is None:
        try:
            from eden_cli.moa_config import decode_moa_turn
    ...
    # ── Eden Governor — Insertion 1: PRE-TURN ──────────────────────────
    # Per governance spec §2.2: runs after build_turn_context(), before the
    # main API loop.  Blocks the turn if any governance check fails.
    try:
        from eden.governor.pre_turn import eden_check_turn

        _eden_rejection = eden_check_turn(
            agent,
            user_message,
        ...
        if _eden_rejection:
            logger.warning(
                "Eden Governor pre-turn blocked agent '%s': %s",
                getattr(agent, "_eden_agent_name", getattr(agent, "callsign", "unknown")),
                _eden_rejection[:120],
            )
            return {
                "final_response": _eden_rejection,
                "messages": messages,
                "api_calls": 0,
                "completed": False,
                "failed": True,
                "error": f"eden_governor_pre_turn: {_eden_rejection}",
            }
    except ImportError:
        pass  # Eden Governor not installed — continue without governance
    except Exception as _eden_pre_err:
        logger.debug("Eden Governor pre-turn check failed (non-fatal): %s", _eden_pre_err)
    ...
    # all run inside Codex). Default Eden OE path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for Eden OE internals.
        #
        # Eden OE invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Eden OE tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            # ── Eden Portal rate limit guard ──────────────────────
            # If another session already recorded that Eden is rate-
            # limited, skip the API call entirely.  Each attempt
            # (including SDK-level retries) counts against RPH and
            # deepens the rate limit hole.
            if agent.provider == "eden":
                try:
                    ...
                    if _nous_remaining is not None and _nous_remaining > 0:
                        _nous_msg = (
                            f"Eden Portal rate limit active — "
            ...
            try:
                ...
                try:
                    from eden_cli.middleware import apply_llm_request_middleware
                ...
                if env_var_enabled("EDEN_OE_DUMP_REQUESTS"):
                ...
                from eden_cli.middleware import run_llm_execution_middleware
                ...
                if finish_reason == "content_filter":
                    ...
                    _refusal_response = (
                        ...
                        "(safety refusal — not a Eden/gateway failure).\n\n"
                        f"{_refusal_detail}\n\n"
                ...
                # Clear Eden rate limit state on successful request —
                ...
                if (
                    ...
                    if _try_refresh_nous_paid_entitlement_credentials(agent):
                        agent._vprint(
                            f"{agent.log_prefix}🔐 Eden paid access verified — "
                ...
                if (
                    ...
                    if agent._try_refresh_nous_client_credentials(force=True):
                        print(f"{agent.log_prefix}🔐 Eden agent key refreshed after 401. Retrying request...")
                        continue
                    ...
                    try:
                        ...
                        pass
                    print(f"{agent.log_prefix}🔐 Eden 401 — Portal authentication failed.")
                    if _body_text:
                        print(f"{agent.log_prefix}   Response: {_body_text}")
                    if not _print_nous_entitlement_guidance(agent, "Eden model access"):
                        print(f"{agent.log_prefix}   Most likely: Portal OAuth expired, account out of credits, or agent key revoked.")
                    print(f"{agent.log_prefix}   Troubleshooting:")
                    print(f"{agent.log_prefix}     • Re-authenticate: eden auth add eden")
                    print(f"{agent.log_prefix}     • Check credits / billing: https://portal.projectglacie.com")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `eden doctor` for credential-chain diagnostics, or")
                    ...
                    from eden_constants import display_eden_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Eden-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: eden config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: eden config set ANTHROPIC_API_KEY \"\"")
                ...
                # ── Eden Portal: record rate limit & skip retries ─────
                # When Eden returns a 429 that is a genuine account-
                ...
                #
                # IMPORTANT: Eden Portal multiplexes multiple upstream
                # providers (DeepSeek, Kimi, MiMo, Eden OE).  A 429 can
                # also mean an UPSTREAM provider is out of capacity
                # for one specific model -- transient, clears in
                # seconds, nothing to do with the caller's quota.
                # Tripping the cross-session breaker on that would
                # block every Eden model for minutes.  We use
                ...
                if (
                    ...
                    try:
                        ...
                        if _genuine_nous_rate_limit:
                            ...
                            logger.info(
                                "Eden 429 looks like upstream capacity "
                    ...
                    if _genuine_nous_rate_limit:
                        ...
                        # top-of-loop Eden guard handles fallback or
                ...
                # which Eden OE' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Eden OE' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`eden",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            pass
                        elif _provider == "eden" and _print_nous_entitlement_guidance(
                            agent,
                            "Eden model access",
                        ):
                            pass
                        elif _provider in {"openai-codex", "xai-oauth", "eden"} and status_code == 401:
                            if _provider == "openai-codex":
                                agent._vprint(f"{agent.log_prefix}   💡 Codex OAuth token was rejected (HTTP 401). Your token may have been", force=True)
                                agent._vprint(f"{agent.log_prefix}      refreshed by another client (Codex CLI, VS Code). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Run `codex` in your terminal to generate fresh tokens.", force=True)
                                agent._vprint(f"{agent.log_prefix}      2. Then run `eden auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `eden model`.", force=True)
                            else:  # eden
                                agent._vprint(f"{agent.log_prefix}   💡 Eden Portal OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: eden portal", force=True)
                                agent._vprint(f"{agent.log_prefix}      2. Check your portal account: https://portal.projectglacie.com", force=True)
                                # ``:free`` is OpenRouter slug syntax; Eden Portal will reject
                                ...
                                if isinstance(_model, str) and _model.endswith(":free"):
                                    ...
                                    agent._vprint(f"{agent.log_prefix}         Eden Portal won't recognize that model name. Either switch to a", force=True)
                                    agent._vprint(f"{agent.log_prefix}         Eden catalog model, or run `/model openrouter:{_model}` to use OpenRouter.", force=True)
                        else:
                            agent._vprint(f"{agent.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: eden setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        eden fallback add   (interactive picker — same as `eden model`)",
                            force=True,
                        )
                ...
                if retry_count >= max_retries:
                    ...
                    if _is_thinking_timeout:
                        ...
                        agent._vprint(
                            ...
                            f"in `~/.eden/config.yaml` to extend the per-call "
                            f"timeout. (Eden OE's built-in floor is 600s for "
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                try:
                    ...
                    # terminates Eden OE mid-turn, resume logic still sees the
                ...
                try:
                    ...
                    from eden_cli.plugins import get_pre_verify_continue_message, has_hook
    ...
    # ── Eden Governor — Insertion 3: POST-TURN ─────────────────────────
    # Per governance spec §2.2: runs after the main loop exits, before
    # finalize_turn().  Failures are logged but never block the turn.
    try:
        ...
        eden_post_turn(
            agent,
            final_response,
            list(messages),  # pass a snapshot of messages for tool-call analysis
            getattr(agent, "session_id", ""),
        )
    except ImportError:
        pass  # Eden Governor not installed
    except Exception as _eden_post_err:
        logger.debug("Eden Governor post-turn hook failed (non-fatal): %s", _eden_post_err)
