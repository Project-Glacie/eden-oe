"""ACP auth helpers — detect and advertise Eden OE authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "eden-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Eden OE runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Eden OE can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Eden OE.
    ...
    not have Eden OE provider credentials configured yet, so Eden OE always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Eden OE using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Eden OE provider",
            description=(
                "Open Eden OE' interactive model/provider setup in a terminal. "
                "Use this when Eden OE has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
