"""CLI entry point for the eden-agent ACP adapter.

Loads environment variables from ``~/.eden/.env``, configures logging
...
try:
    ...
    # shadowing Eden OE's own modules — ``eden acp`` can be started from any
    # cwd, including a project that has same-named packages on its path.
    eden_bootstrap.harden_import_path()
...
def _load_env() -> None:
    """Load .env from EDEN_HOME (default ``~/.eden``)."""
    from eden_cli.env_loader import load_eden_dotenv

    eden_home = get_eden_home()
    loaded = load_eden_dotenv(eden_home=eden_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", eden_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="eden-acp",
        description="Run Eden OE as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Eden OE version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Eden OE provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.eden/node/ "
...
def _run_check() -> None:
    ...
    from acp_adapter.server import EdenACPAgent  # noqa: F401

    print("Eden OE ACP check OK")


def _run_setup() -> None:
    from eden_cli.main import main as eden_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "eden", "model"]
        eden_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``eden postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting eden-agent ACP adapter")
    ...
    import acp
    from .server import EdenACPAgent
