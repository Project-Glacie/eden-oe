"""
Eden MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Eden-specific extra)

Usage:
    eden mcp serve
    eden mcp serve --verbose
...
logger = logging.getLogger("eden.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using EDEN_HOME."""
    try:
        from eden_constants import get_eden_home
        return get_eden_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("EDEN_HOME", Path.home() / ".eden")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from eden_constants import get_eden_home
        directory_file = get_eden_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("EDEN_HOME", Path.home() / ".eden")
...
class EventBridge:
    ...
    This is the Eden OE equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from eden_constants import get_eden_home
            db_file = get_eden_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("EDEN_HOME", Path.home() / ".eden")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Eden OE MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "eden",
        instructions=(
            "Eden OE messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Eden OE MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
