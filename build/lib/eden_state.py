#!/usr/bin/env python3
"""
SQLite State Store for Eden OE.
...
DEFAULT_DB_PATH = get_eden_home() / "state.db"
...
# eden_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``eden_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see eden_cli/kanban_db.py) would
...
def _db_opens_cleanly(db_path: Path) -> Optional[str]:
    ...
    try:
        ...
        probe_session_id = f"_eden_fts_health_probe_{time.time_ns()}"
        try:
...
class SessionDB:
    ...
    # With multiple eden processes (gateway + CLI sessions + worktree agents)
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            # ``eden_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `eden update` to rebuild the venv with a "
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._eden_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._eden_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
    ...
    def get_messages_as_conversation(
        ...
        for row in rows:
            ...
            # that replay reasoning (OpenRouter, OpenAI, Eden) receive
    ...
    def search_sessions_by_id(
        ...
        a session id from logs, CLI output, or another Eden OE surface and jump
    ...
    def delete_session_if_empty(
        ...
        and ``eden sessions list`` output. (Pattern ported from
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Eden OE, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Eden OE session.

        A Eden OE session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Eden OE session is already bound to any Telegram DM topic.
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``EDEN_OE_DISABLE_FTS_TRIGRAM`` or not yet created), so
