"""Windows UTF-8 bootstrap for Eden OE entry points.
...
should be imported at the very top of every Eden OE entry point
(``eden``, ``eden-agent``, ``eden-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Eden OE modules.

    Eden OE ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Eden OE source root
    ...
    """
    root = src_root or os.environ.get("EDEN_OE_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``EDEN_OE_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("EDEN_OE_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import eden_bootstrap``
# (or ``from eden_bootstrap import apply_windows_utf8_bootstrap``) at
