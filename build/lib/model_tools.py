...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        # async tools resolve get_eden_home() under the active profile.
...
#   - cli.py, eden_cli/*      -> inline on startup (no event loop)
...
def get_tool_definitions(
    ...
    if quiet_mode:
        ...
        cache_key = (
            ...
            cfg_fp,
            bool(os.environ.get("EDEN_OE_KANBAN_TASK")),
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if os.environ.get("EDEN_OE_KANBAN_TASK") and "kanban" not in effective_enabled_toolsets:
            # Dispatcher-spawned workers are scoped by EDEN_OE_KANBAN_TASK and
    ...
    # This ensures that even if a composite toolset (like eden-cli)
    ...
    if disabled_toolsets:
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                ...
                if toolset_name.startswith("eden-") or (get_toolset(toolset_name) or {}).get("posture"):
                    # Platform bundles (eden-*) include _EDEN_CORE_TOOLS, and
                    ...
                    if (not quiet_mode and toolset_name.startswith("eden-")
    ...
    # window). Core Eden OE tools (toolsets._EDEN_CORE_TOOLS) are NEVER
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from eden_cli.config import load_config as _load
        cfg = _load() or {}
...
def _normalize_json_strings_for_schema(value: Any, schema: Any) -> Any:
    ...
    Ported from cline/cline#11803, adapted to eden-agent's coercion layer.
...
def _emit_post_tool_call_hook(
    *,
    ...
    """
    try:
        from eden_cli.plugins import has_hook, invoke_hook
...
def handle_function_call(
    ...
    # ── Eden Governor — Insertion 2: DURING-TURN TOOL GATE ──────────
    # Per governance spec §2.2: every tool call passes through the 7
    # constitutional checks before execution.  Rejections are returned
    # as JSON error strings so the model sees why the tool was blocked
    # and can agent-correct (escalate, delegate, or change approach).
    try:
        from eden.governor import eden_check_tool

        # Resolve agent identity from the active identity store
        # (set by pre_turn hook via eden.governor.set_active_identity).
        # Falls back to empty/B if pre_turn hasn't run (non-Eden sessions).
        try:
            from eden.governor import get_active_agent_name, get_active_agent_tier
            _gov_agent_name = get_active_agent_name()
            _gov_agent_tier = get_active_agent_tier()
        except ImportError:
            _gov_agent_name = ""
            _gov_agent_tier = "B"

        _gov_decision = eden_check_tool(
            tool_name=function_name,
            tool_args=function_args,
            agent_name=_gov_agent_name,
            agent_tier=_gov_agent_tier,
            session_id=session_id or "",
        )
        if not _gov_decision.permitted:
            logger.warning(
                "Eden Governor BLOCKED tool '%s': %s",
                function_name,
                _gov_decision.reason[:120],
            )
            return json.dumps(
                {
                    "error": f"Governor denied: {_gov_decision.reason}",
                    "tool": function_name,
                    "governor": True,
                },
                ensure_ascii=False,
        ...
        pass  # Eden Governor not installed — tools execute without governance
    except Exception as _gov_err:
        logger.debug(
            "Eden Governor tool check failed (non-fatal, tool allowed): %s",
            _gov_err,
        )
