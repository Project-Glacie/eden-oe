...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_eden_home() / "sessions")
    ...
    # gateway behaves exactly as before — single EDEN_HOME, no profile stamping.
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        if multiplex_profiles is None and isinstance(nested_gateway, dict):
            ...
            # ``eden config set gateway.multiplex_profiles true``.
            multiplex_profiles = nested_gateway.get("multiplex_profiles")
        # Operator override: GATEWAY_MULTIPLEX_PROFILES wins over config.yaml when
        # set to a recognized value. Hosted deployments (Eden Portal / Fly) stamp
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.eden/config.yaml (primary user-facing config)
    3. ~/.eden/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_eden_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through eden_cli.config.load_config, so without this a managed
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `eden config set gateway.multiplex_profiles true`
            ...
            if not isinstance(streaming_cfg, dict):
                ...
                # ``eden config set gateway.streaming.*``
                streaming_cfg = (
