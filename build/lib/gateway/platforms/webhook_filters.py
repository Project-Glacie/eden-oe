...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.eden to the active profile home."""
    ...
    if raw == "~/.eden":
        return eden_home
    if raw.startswith("~/.eden/"):
        return eden_home / raw.removeprefix("~/.eden/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return eden_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under EDEN_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from eden_constants import get_eden_home

    scripts_root = (get_eden_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.eden" or raw_text.startswith("~/.eden/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__eden_ignore__") is True
        ):
            return False, None
