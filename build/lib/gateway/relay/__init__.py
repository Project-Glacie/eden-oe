"""Relay/connector support package for the Eden OE Gateway.
...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    Both come from enrollment (``eden gateway enroll`` writes them to
    ``~/.eden/.env``): ``GATEWAY_RELAY_ID`` identifies the enrolled instance,
...
def relay_endpoint() -> Optional[str]:
    ...
    ``EDEN_DASHBOARD_PUBLIC_URL``); a hosted/NAS container has the same var
...
def relay_wake_url() -> Optional[str]:
    ...
    sets it explicitly (or passes ``--wake-url`` to ``eden gateway enroll``).
...
def _resolve_relay_identity_token() -> str:
    ...
    ``eden gateway enroll`` CLI. Two modes, in precedence order:

      1. **Generic OIDC client-credentials** (air-gapped / self-hosted-IdP, NO
         Eden Portal): when ``gateway.idp.token_url`` (or
      ...
      2. **Eden Portal** (default): ``resolve_nous_access_token()`` — existing
    ...
    if not token_url:
        # Mode 2 — Eden Portal (default, unchanged behaviour).
        from eden_cli.auth import resolve_nous_access_token
...
def self_provision_relay() -> bool:
    ...
    is already present, AND the agent can resolve its own Eden access token. In
    that case the runtime resolves the agent's own Eden access token (the same
    ...
    up. The creds live ONLY in process memory — never written to ``~/.eden/.env``.
    ...
    sets neither ``EDEN_OE_MANAGED`` nor a ``.managed`` marker), so gating on it
    ...
    self-guarding:
      ...
      - A self-hosted operator who ran ``eden gateway enroll``: has a PINNED
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'eden'}"
