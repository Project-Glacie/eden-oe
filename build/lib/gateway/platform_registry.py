...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``eden setup`` display).
    ...
    # built-ins).  Used by ``eden gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `eden`
        # invocation -- including plain `eden chat`, which never touches any
        ...
        # `eden setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``eden setup``/``gateway status``, channel
