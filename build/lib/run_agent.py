...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    ...
    launched from is meaningful for ``eden -c`` / ``--resume`` (relaunch
...
def _session_source_for_agent(platform: Optional[str]) -> str:
    try:
        ...
        source = get_session_env("EDEN_OE_SESSION_SOURCE", "")
    except Exception:
        source = os.environ.get("EDEN_OE_SESSION_SOURCE", "")
...
_loaded_env_paths = load_eden_dotenv(eden_home=_eden_home, project_env=_project_env)
...
def _routermint_headers() -> dict:
    ...
    return {
        "User-Agent": f"EdenOS/{_EDEN_OE_VERSION}",
    }
...
def _safe_session_filename_component(session_id: str) -> str:
    ...
    ``X-Eden-Session-Id`` API header) and are otherwise interpolated raw
    into on-disk artifact filenames under ``~/.eden/sessions/``.  Without
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[eden-agent: tool call arguments were corrupted in this session and "
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> None:
        """
        Preload the LM Studio model with at least Eden OE' minimum context.
        """
        if (self.provider or "").strip().lower() != "lmstudio":
            return
        try:
            from agent.model_metadata import MINIMUM_CONTEXT_LENGTH
            from eden_cli.models import ensure_lmstudio_model_loaded
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``eden chat -q``. In that mode,
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``EDEN_OE_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``EDEN_OE_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return env_float("EDEN_OE_API_TIMEOUT", 1800.0)
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``EDEN_OE_API_CALL_STALE_TIMEOUT`` env var
        ...
        env_timeout = os.getenv("EDEN_OE_API_CALL_STALE_TIMEOUT")
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        eden-agent #21444 for the symptom history.  The upstream backend
        ...
        return (
            ...
            "See eden-agent#21444 for symptom history."
        )
    ...
    def _provider_model_requires_responses_api(
        ...
        *,
        ...
        # Eden serves GPT-5.x models via its OpenAI-compatible chat
        # completions endpoint; its /v1/responses endpoint returns 404.
        if normalized_provider == "eden":
            return False
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use Eden OE against xai-oauth.  Lots of
    ...
    def _hook_payload_max_chars() -> int:
        raw = os.getenv("EDEN_OE_PLUGIN_PAYLOAD_MAX_CHARS", "50000")
        try:
    ...
    def _invoke_api_request_error_hook(
        ...
        *,
        ...
        # ``monkeypatch.setattr("eden_cli.plugins.has_hook", ...)`` still
        ...
        try:
            from eden_cli import plugins as _plugins
    ...
    def _redact_message_content(content):
        ...
        Respects ``EDEN_OE_REDACT_SECRETS`` via ``redact_sensitive_text`` —
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.eden/sessions/session_{sid}.json``
        ...
        # untrusted input (X-Eden-Session-Id header) and must not escape
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects EDEN_OE_REDACT_SECRETS via
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        (e.g. ``~/.eden/config.yaml``) to a messaging channel (#35584).
        """
        if not failed:
            return ""
        lines = [
    ...
    def _turn_completion_explainer_enabled(self) -> bool:
        ...
        True).  ``EDEN_OE_TURN_COMPLETION_EXPLAINER`` env var overrides
        config.  Exposed as a method so tests can patch a single seam,
        mirroring ``_file_mutation_verifier_enabled``.
        """
        try:
            import os as _os
            env = _os.environ.get("EDEN_OE_TURN_COMPLETION_EXPLAINER")
            if env is not None:
                return env.strip().lower() not in {"0", "false", "no", "off"}
            # Read from the persisted config.yaml so gateway and CLI share
            # the same setting.  Import lazily to avoid a startup-time cycle.
    ...
    def _touch_activity(self, desc: str) -> None:
        ...
        process is a dispatcher-spawned worker (EDEN_OE_KANBAN_TASK set),
    ...
    def _capture_credits(self, http_response: Any) -> None:
        """Parse x-eden-credits-* headers, cache CreditsState, fire threshold notices.
        ...
        """
        # Dev test fixture (EDEN_OE_DEV_CREDITS_FIXTURE): inject a chosen notice state
        ...
        if _fixture is not None:
            ...
            logger.info(
                ...
                "(real headers bypassed — `echo clear` / unset EDEN_OE_DEV_CREDITS_FIXTURE to restore)",
        ...
        if not headers:
            return
        _dev = is_truthy_value(os.environ.get("EDEN_OE_DEV_CREDITS"))
        ...
        if state is None:
            if _dev:
                logger.info(
                    "credits ▸ response had no valid x-eden-credits-* headers "
                    "(miss — producer off / non-Eden path / >TTL stale)"
                )
            return
        ...
        if _dev:
            # EDEN_OE_DEV_CREDITS: stream each capture to agent.log — watch live with
            # `eden logs -f` (grep 'credits ▸'). Dev-only; silent for normal users.
    ...
    def _build_keepalive_http_client(base_url: str = "", *, verify: Any = True) -> Any:
        ...
        ``EDEN_OE_CA_BUNDLE`` settings.  It is passed on the client AND on
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``eden auth add xai-oauth``) whose tokens belong to
    ...
    def _try_refresh_nous_client_credentials(
        ...
        *,
        ...
        if self.api_mode != "chat_completions" or self.provider != "eden":
            return False
        ...
        try:
            ...
            logger.debug("Eden credential refresh failed: %s", exc)
            return False
        ...
        self._client_kwargs["api_key"] = self.api_key
        self._client_kwargs["base_url"] = self.base_url
        # Eden requests should not inherit OpenRouter-only attribution headers.
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        try:
            from eden_cli.copilot_auth import resolve_copilot_token
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        known reasoning-capable model families and direct Eden Portal.
        """
        if base_url_host_matches(self._base_url_lower, "projectglacie.com"):
            return True
        if (
            ...
            try:
                from eden_cli.models import github_model_reasoning_efforts
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from eden_cli.models import lmstudio_model_reasoning_options
