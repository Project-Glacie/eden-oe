"""NousDashboardAuthProvider — Eden Portal OAuth (authorization-code + PKCE).

Implements ``eden-account-service/docs/agent-dashboard-oauth-contract.md``
...
Configuration surfaces (env wins over config.yaml when set non-empty):
  ...
  per-deploy values don't need to bake into ``config.yaml``:
      ...
      EDEN_DASHBOARD_OAUTH_CLIENT_ID  — shape ``agent:{agent_instance_id}``
      EDEN_DASHBOARD_PORTAL_URL       — defaults to
                                          ``https://portal.projectglacie.com``
...
Key contract points encoded here:
  ...
  - audience claim is the bare ``client_id`` (no ``eden-cli:`` prefix).
...
"state": …}`` and the route serializes those into the ``eden_session_pkce``
cookie.
...
Skip reasons:
  ...
  ("Set EDEN_DASHBOARD_OAUTH_CLIENT_ID …") instead of the bare "no
...
# Production Portal URL. Override via EDEN_DASHBOARD_PORTAL_URL for
# staging (portal.rewbs.uk) or a custom deployment. Contract docs name
# this as the production issuer.
_DEFAULT_PORTAL_URL = "https://portal.projectglacie.com"
...
class NousDashboardAuthProvider(DashboardAuthProvider):
    """Eden Portal OAuth via authorization-code + PKCE (S256)."""

    name = "eden"
    display_name = "Project Glacie"
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        # The auth-route layer expects ``cookie_payload[\"eden_session_pkce\"]``
        ...
        cookie_payload = {
            "eden_session_pkce": f"state={state};verifier={code_verifier}",
        }
    ...
    def complete_login(
        ...
        *,
        ...
        # we just receive it for symmetry with the protocol. Eden Portal
    ...
    def refresh_session(self, *, refresh_token: str) -> Session:
        ...
        try:
            response = httpx.post(
                ...
                # ``x-eden-refresh-token`` header. Portal's token endpoint
                ...
                # Portal's ``REFRESH_TOKEN_HEADER`` exactly (``x-eden-refresh-
                ...
                headers={
                    ...
                    "x-eden-refresh-token": refresh_token,
                },
                timeout=_TOKEN_ENDPOINT_TIMEOUT_SEC,
            )
        except httpx.RequestError as exc:
    ...
    def _verify_jwt(self, access_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # between EDEN_DASHBOARD_PORTAL_URL / EDEN_DASHBOARD_OAUTH_CLIENT_ID
    ...
    def _check_contract_version(self, claims: Dict[str, Any]) -> None:
        ...
        if contract_version is None:
            logger.warning(
                "Eden Portal token missing oauth_contract_version claim "
...
def _load_config_oauth_section() -> dict:
    ...
    try:
        ...
        logger.debug(
            "dashboard-auth-eden: load_config() raised %s; "
...
def _resolve_client_id() -> str:
    ...
    Order:
      1. ``EDEN_DASHBOARD_OAUTH_CLIENT_ID`` env var (when non-empty
    ...
    """
    env = os.environ.get("EDEN_DASHBOARD_OAUTH_CLIENT_ID", "").strip()
    if env:
        return env
...
def _resolve_portal_url() -> str:
    ...
    Order:
      1. ``EDEN_DASHBOARD_PORTAL_URL`` env var (non-empty after strip).
      2. ``dashboard.oauth.portal_url`` in ``config.yaml``.
      3. :data:`_DEFAULT_PORTAL_URL` (production Portal).
    """
    env = os.environ.get("EDEN_DASHBOARD_PORTAL_URL", "").strip()
    if env:
        return env
    cfg_value = str(
...
def register(ctx) -> None:
    ...
    configured (either via ``EDEN_DASHBOARD_OAUTH_CLIENT_ID`` env var
    ...
    can surface "Set EDEN_DASHBOARD_OAUTH_CLIENT_ID …" instead of the
    ...
    engagement layer (``eden_cli.web_server.should_require_auth`` +
    ...
    if not client_id:
        LAST_SKIP_REASON = (
            "EDEN_DASHBOARD_OAUTH_CLIENT_ID is not set (and "
            "dashboard.oauth.client_id in config.yaml is empty). The "
            "Eden Portal provisions this env var (shape "
            "'agent:{instance_id}') when it deploys a Eden OE "
        ...
        logger.debug("dashboard-auth-eden: %s", LAST_SKIP_REASON)
        return

    if not client_id.startswith("agent:"):
        LAST_SKIP_REASON = (
            f"EDEN_DASHBOARD_OAUTH_CLIENT_ID={client_id!r} doesn't match "
            f"the contract shape 'agent:{{instance_id}}'. The Eden Portal "
            f"provisions this value at deploy time; check your Fly app's "
            f"secrets or override with the value from the Portal admin UI."
        )
        logger.warning("dashboard-auth-eden: %s", LAST_SKIP_REASON)
        return
    ...
    logger.info(
        "dashboard-auth-eden: registered provider (client_id=%s, portal=%s)",
        client_id,
        portal_url,
    )
