...
```bash
eden memory setup    # select "mem0"
```

Or manually:
```bash
eden config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.eden/.env
```

## Config

Behavioral settings live in `$EDEN_HOME/mem0.json` (set them via `eden memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.eden/.env`.
...
| `user_id` | `eden-user` | User identifier on Mem0 |
| `agent_id` | `eden` | Agent identifier |
...
2. Point the plugin at it — via the setup wizard:
   ```bash
   eden memory setup    # select "mem0" → "Self-hosted server"
   # Or non-interactive:
   eden memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
   ```
   or via env vars:
   ```bash
   echo "MEM0_HOST=http://localhost:8888" >> ~/.eden/.env
   echo "MEM0_API_KEY=your-admin-api-key" >> ~/.eden/.env
   ```
   or in `$EDEN_HOME/mem0.json`:
   ```json
   {
   ...
   ```
3. Start a fresh Eden OE session and call `mem0_search` — it connects to your server.
...
```bash
eden memory setup mem0 --mode oss \
...
```bash
eden memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$EDEN_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.eden/mem0_qdrant"}}
  }
}
```
...
```bash
eden memory setup mem0 --mode platform --api-key sk-...
```

### Dry Run (preview without writing)

```bash
eden memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```
...
- Check `user_id` matches between sessions (`$EDEN_HOME/mem0.json`).
