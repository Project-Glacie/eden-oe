...
- Supermemory API key from [app.supermemory.ai/integrations?connect=eden](http://app.supermemory.ai/integrations?connect=eden)

## Setup

```bash
eden memory setup    # select "supermemory"
```

Or manually:

```bash
eden config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.eden/.env
```

## Config

Config file: `$EDEN_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `eden` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `eden-{identity}` → `eden-coder`). |
...
All Supermemory API calls send `x-sm-source: eden`, and document writes stamp
`metadata.sm_source: eden`. This is a **functional routing key, not telemetry**:
it groups Eden-written memories into a dedicated "Eden" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Eden OE profile:

```json
{
  "container_tag": "eden-{identity}"
}
```

For a profile named `coder`, this resolves to `eden-coder`. The default profile resolves to `eden-default`. Without `{identity}`, all profiles share the same container.
