...
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=eden"
...
def _load_supermemory_config(eden_home: str) -> dict:
    config = _default_config()
    config_path = Path(eden_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, eden_home: str) -> None:
    config_path = Path(eden_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str, search_mode: str = "hybrid"):
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "eden"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Eden OE writes into the "Eden" Space in the Supermemory
        ...
        merged = {"sm_source": "eden", **(metadata or {})}
...
def _resolve_container_tag_for_setup(eden_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(eden_home)
...
def _probe_supermemory_connection(api_key: str, eden_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(eden_home)
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(eden_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `eden memory setup`.
        # All other options are documented for $EDEN_HOME/supermemory.json
    ...
    def save_config(self, values, eden_home):
        ...
        _save_supermemory_config(sanitized, eden_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, eden_home)
    ...
    def post_setup(self, eden_home: str, config: dict) -> None:
        ...
        from eden_cli.memory_setup import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(Path(eden_home) / ".env", env_writes)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from eden_constants import get_eden_home
        self._eden_home = kwargs.get("eden_home") or str(get_eden_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._eden_home)
