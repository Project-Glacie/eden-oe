...
```bash
eden memory setup    # select "byterover"
```

Or manually:
```bash
eden config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> ~/.eden/.env
```
...
Working directory: `$EDEN_HOME/byterover/` (profile-scoped).
