...
``{title, url, description, position}`` rows every other Eden OE web
provider produces.
...
prefers Eden-managed xAI Grok OAuth (via ``eden auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.eden/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``eden tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `eden auth` to sign in with "
