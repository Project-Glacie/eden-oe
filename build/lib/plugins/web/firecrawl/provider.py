...
Env vars::
    ...
    FIRECRAWL_GATEWAY_URL=...        # Eden tool-gateway (subscribers)
...
def _is_tool_gateway_ready() -> bool:
    """Return True when gateway URL + Eden Subscriber token are available.
...
def check_firecrawl_api_key() -> bool:
    ...
    existing tests and the ``eden tools`` setup flow.
    """
...
def _firecrawl_backend_help_suffix() -> str:
    ...
    if not _wt.managed_nous_tools_enabled():
        return ""
    return (
        ", or use the Eden Tool Gateway via your subscription "
...
def _raise_web_backend_configuration_error() -> None:
    ...
    if _wt.managed_nous_tools_enabled():
        message += (
            " With your Eden subscription you can also use the Tool Gateway. "
            "run `eden tools` and select Eden Subscription as the web provider."
        )
    else:
