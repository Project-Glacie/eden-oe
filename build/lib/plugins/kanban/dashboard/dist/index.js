/**
 * Eden OE Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.eden/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__EDEN_OE_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__EDEN_OE_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "eden-kanban-dot-triage",
    todo: "eden-kanban-dot-todo",
    ready: "eden-kanban-dot-ready",
    running: "eden-kanban-dot-running",
    blocked: "eden-kanban-dot-blocked",
    done: "eden-kanban-dot-done",
    archived: "eden-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-eden-task";
  ...
  const DOCS_URL = "https://eden-oe.projectglacie.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://eden-oe.projectglacie.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "eden.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="eden-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])eden-kanban-md-code\1/i.test(attrs)) {
      return ' class="eden-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "eden-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "eden-kanban-md",
  ...
  // standard `drop` event and our `eden-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("eden-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("eden-kanban-column--drop");
          if (target) target.classList.add("eden-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("eden-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("eden-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("eden-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__EDEN_OE_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "eden-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "eden-kanban-attention",
        "eden-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "eden-kanban-attention-bar" },
        h("span", { className: "eden-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "eden-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "eden-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "eden-kanban-attention-row",
                  "eden-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "eden-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "eden-kanban-attention-row-id" }, task.id),
                h("span", { className: "eden-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "eden-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "eden-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "eden-kanban-diag-action-btn",
      action.suggested ? "eden-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " eden-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".eden-kanban-drawer-comment-row input, .eden-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "eden-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("eden-kanban-diag", sevClass) },
      h("div", { className: "eden-kanban-diag-header" },
        h("span", { className: "eden-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "eden-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "eden-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "eden-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "eden-kanban-diag-data-row" },
                  h("span", { className: "eden-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "eden-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "eden-kanban-diag-data-row" },
                h("span", { className: "eden-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "eden-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "eden-kanban-diag-reassign-row" },
            h("span", { className: "eden-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "eden-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "eden-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "eden-kanban-diag-msg",
              msg.ok ? "eden-kanban-diag-msg--ok" : "eden-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "eden-kanban-section" },
      h("div", { className: "eden-kanban-section-head-row" },
        h("span", { className: "eden-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "eden-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "eden-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "eden-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "eden-kanban-docs-link",
      title: "Open Eden OE Kanban docs in a new tab",
      "aria-label": "Eden OE Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `eden kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "eden-kanban-msg-ok" : "eden-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "eden-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "eden-kanban-boardswitcher" },
      h("div", { className: "eden-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "eden-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "eden-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "eden-kanban-dialog-title" },
        ...
        h("div", { className: "eden-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Eden OE profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "eden-kanban-bulk" },
      h("span", { className: "eden-kanban-bulk-count" },
      ...
      h("div", { className: "eden-kanban-bulk-priority",
      ...
      h("div", { className: "eden-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Eden OE profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "eden-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("eden-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("eden-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "eden-kanban-trash",
        dragOver ? "eden-kanban-trash--drop" : "",
        props.draggingTaskId ? "eden-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "eden-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "eden-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".eden-kanban-card")) return true;
      if (target.closest && target.closest(".eden-kanban-column-add")) return true;
      if (target.closest && target.closest(".eden-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("eden-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("eden-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".eden-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ...
      className: cn(
        "eden-kanban-columns",
        isScrollable ? "eden-kanban-columns--scrollable" : "",
        isPanning ? "eden-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("eden-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("eden-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "eden-kanban-column",
        dragOver ? "eden-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "eden-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "eden-kanban-col-check",
        ...
        h("span", { className: cn("eden-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "eden-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "eden-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "eden-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "eden-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "eden-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "eden-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "eden-kanban-lane" },
                  h("div", { className: "eden-kanban-lane-head" },
                    h("span", { className: "eden-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "eden-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "eden-kanban-card--stale-red";
    if (age >= tier.amber) return "eden-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".eden-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "eden-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "eden-kanban-card",
        props.selected ? "eden-kanban-card--selected" : "",
        props.failed ? "eden-kanban-card--failed" : "",
        props.draggingSource ? "eden-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "eden-kanban-card-content" },
          h("div", { className: "eden-kanban-card-row" },
            h("label", {
              className: "eden-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "eden-kanban-card-check",
            ...
            h("span", { className: "eden-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or eden kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "eden-kanban-warning-badge",
                    "eden-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "eden-kanban-needs-assignee",
          ...
          h("div", { className: "eden-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "eden-kanban-card-row eden-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "eden-kanban-assignee",
                            title: `Assigned to Eden OE profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "eden-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "eden-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "eden-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "eden-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Eden OE profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Eden OE profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "eden-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "eden-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "eden-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "eden-kanban-drawer-close",
        ...
        data ? h("div", { className: "eden-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "eden-kanban-section" },
      h("div", { className: "eden-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "eden-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "eden-kanban-drawer-body" },
      h("div", { className: "eden-kanban-drawer-title" },
        h("span", { className: cn("eden-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "eden-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "eden-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "eden-kanban-section" },
            h("div", { className: "eden-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "eden-kanban-section" },
            h("div", { className: "eden-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "eden-kanban-done-no-result eden-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "eden-kanban-section" },
            h("div", { className: "eden-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "eden-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "eden-kanban-section" },
        h("div", { className: "eden-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "eden-kanban-comment" },
            h("div", { className: "eden-kanban-comment-head" },
              h("span", { className: "eden-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "eden-kanban-diag-action-btn",
      ...
      h("div", { className: "eden-kanban-section" },
        h("div", { className: "eden-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "eden-kanban-comment" },
                h("div", { className: "eden-kanban-comment-head" },
                  h("span", { className: "eden-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "eden-kanban-comment-ago" },
      ...
      h("div", { className: "eden-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "eden-kanban-event",
              isDiag ? "eden-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "eden-kanban-event-header" },
                  h("span", { className: "eden-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "eden-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "eden-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "eden-kanban-event-header-plain" },
                  h("span", { className: "eden-kanban-event-kind" }, e.kind),
                  h("span", { className: "eden-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "eden-kanban-event-phantom-row" },
                  h("span", { className: "eden-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "eden-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "eden-kanban-event-payload" },
    ...
    return h("div", { className: "eden-kanban-section" },
      h("div", { className: "eden-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "eden-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `eden-kanban-run--${r.outcome || r.status || "ended"}`
          : "eden-kanban-run--active";
        return h("div", { key: r.id, className: cn("eden-kanban-run", outcomeClass) },
          h("div", { className: "eden-kanban-run-head" },
            h("span", { className: "eden-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "eden-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "eden-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "eden-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "eden-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "eden-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "eden-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "eden-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "eden-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "eden-kanban-pre eden-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "eden-kanban-meta-row" },
      h("span", { className: "eden-kanban-meta-label" }, props.label),
      h("span", { className: "eden-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "eden-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "eden-kanban-meta-row" },
        h("span", { className: "eden-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "eden-kanban-meta-value eden-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "eden-kanban-meta-row" },
        h("span", { className: "eden-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "eden-kanban-meta-value eden-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "eden-kanban-section" },
      h("div", { className: "eden-kanban-section-head-row" },
        h("span", { className: "eden-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "eden-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "eden-kanban-section" },
      h("div", { className: "eden-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "eden-kanban-deps-row" },
        h("span", { className: "eden-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "eden-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "eden-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "eden-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "eden-kanban-dep-chip-x",
      ...
      h("div", { className: "eden-kanban-deps-row" },
        h("span", { className: "eden-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "eden-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "eden-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "eden-kanban-section" },
      ...
      h("div", { className: "eden-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "eden-kanban-home-sub eden-kanban-home-sub--on"
              : "eden-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__EDEN_OE_PLUGINS__ && typeof window.__EDEN_OE_PLUGINS__.register === "function") {
    window.__EDEN_OE_PLUGINS__.register("kanban", KanbanPage);
  }
})();
