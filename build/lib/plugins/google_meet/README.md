...
Let the eden agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where eden runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `eden meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `eden meet` CLI |
...
| `cli.py` | `eden meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$EDEN_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `eden meet node {run,list,approve,remove,status,ping}` |
...
eden meet install                                      # pip + Chromium
eden meet setup                                        # preflight
eden meet auth                                         # optional
eden meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
eden meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.eden/.env
eden meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
eden meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
eden meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.eden/.env
eden meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, eden will **not** switch your system audio input automatically — the
...
eden plugins enable google_meet
eden meet node run --display-name my-mac --host 0.0.0.0 --port 18789
