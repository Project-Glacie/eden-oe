...
session-scoped state file under ``$EDEN_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $EDEN_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_eden_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this eden install, leave it first —
    ...
    env = os.environ.copy()
    env["EDEN_OE_MEET_URL"] = url
    env["EDEN_OE_MEET_OUT_DIR"] = str(out)
    env["EDEN_OE_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["EDEN_OE_MEET_HEADED"] = "1"
    if auth_state:
        env["EDEN_OE_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["EDEN_OE_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if EDEN_OE_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["EDEN_OE_MEET_MODE"] = mode
    if realtime_model:
        env["EDEN_OE_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["EDEN_OE_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["EDEN_OE_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["EDEN_OE_MEET_REALTIME_KEY"] = realtime_api_key
