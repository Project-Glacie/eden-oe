"""ntfy platform adapter (Eden OE plugin).
...
a Eden OE dependency.

This adapter ships as a Eden OE platform plugin under
``plugins/platforms/ntfy/``. The Eden OE plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "eden-in"              # subscribe topic (incoming)
          publish_topic: "eden-out"     # optional — defaults to topic
...
_ECHO_TAG = "eden-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``eden cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Eden OE dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `eden gateway status` without
