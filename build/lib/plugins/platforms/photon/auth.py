...
Credential storage mirrors every other Eden OE channel:

    * runtime SDK creds  -> ``~/.eden/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.eden/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a eden dependency
...
# registers Eden OE as its own client_id.
...
# Default name of the project Eden OE provisions for the operator.
DEFAULT_PROJECT_NAME = "Eden OE"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of eden-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.eden/auth.json`` honouring the active Eden OE profile."""
    try:
        from eden_constants import get_eden_home
        return Path(get_eden_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.eden")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.eden/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.eden/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.eden/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: eden_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `eden photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `eden photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `eden photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `eden photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `eden photon setup`)"
