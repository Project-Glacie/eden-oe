"""
LINE Messaging API platform adapter for Eden OE.
...
to Eden OE. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``EDEN_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            eden_home = Path(get_eden_home()).resolve()
        except Exception:
            eden_home = Path.home().joinpath(".eden").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``eden status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``eden status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``eden setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.eden/.env`` via ``eden_cli.config``.
    """
    print()
    ...
    try:
        from eden_cli.config import get_env_var, set_env_var
    except ImportError:
        print("eden_cli.config not available; set LINE_* vars manually in ~/.eden/.env")
        return
