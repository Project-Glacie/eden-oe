"""CLI handlers for the ``eden proxy`` subcommand."""
...
from eden_cli.proxy.adapters import ADAPTERS, get_adapter
from eden_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "eden proxy requires aiohttp. Install one of:\n"
        "  pip install 'eden-agent[messaging]'\n"
        "  pip install aiohttp",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    provider = getattr(args, "provider", None) or "eden"
    try:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"eden auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Eden OE proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Eden OE proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: eden proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``eden proxy <subcommand>``."""
    ...
    print(
        "eden proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  eden proxy start [--provider eden|xai] [--host 127.0.0.1] [--port 8645]\n"
        "      Run the proxy in the foreground.\n"
        "  eden proxy status\n"
        "      Show which upstream adapters are ready.\n"
        "  eden proxy providers\n"
