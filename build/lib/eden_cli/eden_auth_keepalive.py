"""Background keepalive for long-lived Eden Portal sessions."""
...
def _timeout_seconds(value: Optional[float]) -> float:
    if value is not None:
        return float(value)
    try:
        return float(os.getenv("EDEN_OE_NOUS_TIMEOUT_SECONDS", "15"))
...
def _refresh_selected_pool_entry(
    *,
    ...
    """Refresh the current Eden credential pool entry when it is stale.

    Returns True when a pool entry exists and is usable/refreshed, False when a
    pool exists but no entry can be used, and None when no Eden pool exists.
    """
    try:
        ...
        logger.debug("Eden auth keepalive: credential pool unavailable: %s", exc)
        return None
    ...
    try:
        ...
        logger.debug("Eden auth keepalive: credential pool selection failed: %s", exc)
        return False
    ...
    if access_expiring or not key_usable:
        ...
        if refreshed is None:
            return False
        logger.debug("Eden auth keepalive: refreshed credential pool entry")
        return True
...
def refresh_nous_auth_keepalive_once(
    *,
    ...
    """Refresh Eden auth once if credentials are configured."""
    ...
    try:
        ...
        logger.debug("Eden auth keepalive: refreshed singleton auth state")
        return True
    except AuthError as exc:
        if exc.relogin_required:
            logger.info("Eden auth keepalive requires re-login: %s", exc)
        else:
            logger.debug("Eden auth keepalive failed: %s", exc)
        return False
    except Exception as exc:
        logger.debug("Eden auth keepalive failed: %s", exc)
        return False
...
def start_nous_auth_keepalive(
    *,
    ...
    """Start the process-wide Eden auth keepalive thread."""
    ...
    with _keepalive_lock:
        ...
        _keepalive_thread = threading.Thread(
            ...
            daemon=True,
            name="eden-auth-keepalive",
        )
        _keepalive_thread.start()
        logger.debug("Eden auth keepalive started")
        return _keepalive_thread
