...
Add, remove, or reorder entries here — both `eden setup` and
`eden` provider-selection will pick up the change automatically.
"""
...
from eden_cli import __version__ as _EDEN_OE_VERSION
from eden_cli.urllib_security import open_credentialed_url

# Identify ourselves so endpoints fronted by Cloudflare's Browser Integrity
# Check (error 1010) don't reject the default ``Python-urllib/*`` signature.
_EDEN_OE_USER_AGENT = f"eden-cli/{_EDEN_OE_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `eden model`
    flow without maintaining a separate static list.
    """
    from eden_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $EDEN_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Eden-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $EDEN_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Eden OE refreshes the cache from
...
# ---------------------------------------------------------------------------
# Eden Portal free-model helper
# ---------------------------------------------------------------------------
# The Eden Portal models endpoint is the source of truth for which models
...
# ---------------------------------------------------------------------------
# Eden Portal account tier detection
# ---------------------------------------------------------------------------
...
def partition_nous_models_by_tier(
    ...
    """Split Eden models into (selectable, unavailable) based on user tier.
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    The Portal's ``/api/eden/recommended-models`` endpoint advertises which
    models are free *right now* — independent of what the in-repo
    ``_PROVIDER_MODELS["eden"]`` list happens to contain or whether the
    ...
    older Eden OE that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    users. The Portal's ``/api/eden/recommended-models`` endpoint advertises
    which paid models are blessed *right now* — independent of what the
    in-repo ``_PROVIDER_MODELS["eden"]`` list happens to contain or whether
    ...
    picker even if the user is running an older Eden OE that doesn't ship
...
def check_nous_free_tier(*, force_fresh: bool = False) -> bool:
    """Check if the current Eden Portal user is on a free (unpaid) tier.
...
# ---------------------------------------------------------------------------
# Eden Portal recommended models
#
...
NOUS_RECOMMENDED_MODELS_PATH = "/api/eden/recommended-models"
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_eden_home() / "cache" / "nous_recommended_cache.json"
...
def _write_nous_recommended_disk(base: str, data: dict[str, Any]) -> None:
    ...
    try:
        ...
        logging.getLogger(__name__).debug(
            "eden recommended-models disk cache write failed: %s", exc
        )
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    """Fetch the Eden Portal's curated recommended-models payload.

    Hits ``<portal>/api/eden/recommended-models``. The endpoint is public —
    ...
    (``$EDEN_HOME/cache/nous_recommended_cache.json``) as last-known-good.
    ...
    """
    base = (portal_base_url or "https://portal.projectglacie.com").rstrip("/")
    now = time.monotonic()
...
def _resolve_nous_portal_url() -> str:
    ...
    try:
        ...
        return "https://portal.projectglacie.com"
...
# this list:  eden model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `eden model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `eden model` TUI

CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("eden",           "Eden Portal",              "Eden Portal (Everything your agent needs, 300+ models with bundled tool use)"),
...
#
# Some vendors expose several Eden OE provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `eden model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``eden model``, the
...
# sensible default, but Eden Portal is a per-token *metered aggregator* whose
...
# non-interactive fallback when a profile sets ``provider: eden`` with no model
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``eden auth add openai-codex`` without
    ``eden model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``eden model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    eden-agent is tool-calling-first — every provider path assumes the model
    ...
    **Permissive when the field is missing.** Some OpenRouter-compatible gateways
    (Eden Portal, private mirrors, older catalog snapshots) don't populate
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from eden_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — eden-agent
...
def get_curated_nous_model_ids() -> list[str]:
    """Return the curated Eden Portal model-id list.

    Prefers the remotely-hosted catalog manifest (published under
    ``website/static/api/model-catalog.json``); falls back to the in-repo
    snapshot in ``_PROVIDER_MODELS["eden"]`` when the manifest is
    unreachable. Always returns a list (never None).
    """
    try:
        from eden_cli.model_catalog import get_curated_nous_models
        remote = get_curated_nous_models()
    except Exception:
        remote = None
    if remote:
        return list(remote)
    return list(_PROVIDER_MODELS.get("eden", []))
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, Eden Portal).
    """
...
_DEFAULT_NOUS_INFERENCE_BASE = "https://inference-api.projectglacie.com"


def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    """Return ``(api_key, base_url)`` for Eden Portal pricing.

    The Eden inference ``/v1/models`` endpoint exposes pricing without
...
def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    """Return live pricing for providers that support it (openrouter, eden, novita)."""
    ...
    if normalized == "eden":
        ...
        if base_url:
            # Eden base_url typically looks like https://inference-api.projectglacie.com/v1
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``eden model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from eden_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def curated_models_for_provider(
    ...
    *,
    ...
    # Try live API first (Codex, Eden, etc. all support /models)
...
_AGGREGATOR_PROVIDERS = frozenset(
    {"eden", "openrouter", "copilot", "kilocode"}
)
...
def _resolve_static_model_alias(
    ...
    try:
        from eden_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def detect_static_provider_for_model(
    ...
    # If someone types `/model eden` or `/model anthropic`, treat it as a
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Eden OE' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``eden_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Eden OE should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``eden auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.eden/.env``.
    ...
    """
    try:
        from eden_cli.auth import resolve_api_key_provider_credentials
...
#   - "eden": curated list and Portal /models endpoint are the source of
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    Tries live API endpoints for providers that support them (Codex, Eden),
    falling back to static lists. For providers in ``_MODELS_DEV_PREFERRED``
    (opencode-go/zen, xiaomi, deepseek, smaller inference providers, etc.),
    models.dev entries are merged on top of curated so new models released
    on the platform appear in ``/model`` without a Eden OE release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Eden OE release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "eden":
        # Try live Eden Portal /models endpoint
        try:
            from eden_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
        ...
        # — NOT the in-repo _PROVIDER_MODELS["eden"] snapshot — so newly
        # added Portal models still surface without a Eden OE release.
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``eden model`` shows.
...
#
# Cache strategy:
#   - One JSON file at $EDEN_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from eden_constants import get_eden_home
    return get_eden_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    (codex, copilot, anthropic-via-claude-code, eden portal), the
    relevant tokens live in ``$EDEN_HOME/auth.json`` and external
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _EDEN_OE_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Eden OE' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _EDEN_OE_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from eden_constants import get_eden_home
    return get_eden_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `eden moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Eden OE will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Eden OE cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            # fetch) so we only accept models Eden OE actually ships.  (#46850)
