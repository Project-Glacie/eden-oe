"""``eden gateway`` and ``eden proxy`` subcommand parsers.

Extracted verbatim from ``eden_cli/main.py:main()`` (god-file Phase 2).
...
from eden_cli.subcommands._shared import add_accept_hooks_flag
...
def build_gateway_parser(
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if EDEN_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy eden.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Eden OE gateway unit files "
            "(e.g. eden.service) left over from older installs. Profile "
            "units (eden-gateway-<profile>.service) and unrelated "
    ...
    gateway_enroll = gateway_subparsers.add_parser(
        ...
        description=(
            ...
            "Authenticates as your Eden Portal account (the connector derives the "
            ...
            "GATEWAY_RELAY_SECRET / GATEWAY_RELAY_DELIVERY_KEY into ~/.eden/.env. "
            "Requires being logged in (eden setup). Not available in managed installs."
        ),
    )
    ...
    gateway_enroll.add_argument(
        ...
        help=(
            ...
            "GATEWAY_RELAY_WAKE_URL in ~/.eden/.env and forwarded at provision. "
    ...
    proxy_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "to an OAuth-authenticated provider (e.g. Eden Portal). External "
    ...
    proxy_start.add_argument(
        "--provider",
        default="eden",
        help="Upstream provider: eden or xai (default: eden). See `eden proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
