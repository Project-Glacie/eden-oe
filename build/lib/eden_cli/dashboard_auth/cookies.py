...
Three cookies in play:
  - eden_session_at:   the OAuth access token
                         (HttpOnly, lifetime = token TTL, ~15 min)
  - eden_session_rt:   the OAuth refresh token
                         (HttpOnly, lifetime = 24h, ROTATING + reuse-detected)
                         Eden Portal issues a rotating refresh token for the
                         dashboard auth-code grant (Portal NAS #293 / eden
  ...
  - eden_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
setter wrote ``__Secure-eden_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "eden_session_at"
SESSION_RT_COOKIE = "eden_session_rt"
PKCE_COOKIE = "eden_session_pkce"
...
SSO_ATTEMPT_COOKIE = "eden_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /eden`` we want ``Path=/eden`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    ``refresh_token`` is written as the RT cookie when non-empty. Eden Portal
    issues a 24h rotating refresh token (eden #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/eden``)
