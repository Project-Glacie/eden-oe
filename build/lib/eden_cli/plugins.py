...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with eden-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.eden/plugins/<name>/``
3. **Project plugins** – ``./.eden/plugins/<name>/`` (opt-in via
   ``EDEN_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``eden_os.plugins``
   entry-point group.
...
from eden_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE


def get_bundled_plugins_dir() -> Path:
    """Locate the bundled ``plugins/`` directory.

    Honours ``EDEN_OE_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("EDEN_OE_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``EDEN_OE_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.eden/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("EDEN_OE_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When EDEN_OE_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Eden OE loggers.
    """
    ...
    logger.debug(
        "EDEN_OE_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Eden OE' shipped guidance lives in the evidence-based
    ...
    # Kanban task lifecycle hooks. Fired by eden_cli.kanban_db when a task
    ...
    # separate `eden -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `eden kanban dispatch`),
...
ENTRY_POINTS_GROUP = "eden_os.plugins"

_NS_PARENT = "eden_plugins"
...
class PluginManifest:
    ...
    #              in ~/.eden/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``eden plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        """Return the active Eden OE profile name (e.g. ``"default"``).

        Derived from ``EDEN_HOME`` via
        :func:`eden_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.eden/profiles/<name>``, or ``"custom"`` when
        ``EDEN_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Eden OE core) are trusted by default —
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``eden honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``eden <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`eden_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from eden_cli.dashboard_auth import (
    ...
    def register_secret_source(self, source) -> None:
        ...
        sources run during ``load_eden_dotenv()`` startup — after
        ``~/.eden/.env`` loads, before Eden OE reads credentials — when
    ...
    def register_slack_action_handler(
        ...
        Eden OE' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``eden model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: eden_cli.main imports plugins indirectly
        from eden_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.eden/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        if self._discovered and not force:
            return
        if env_var_enabled("EDEN_OE_SAFE_MODE"):
            logger.info("EDEN_OE_SAFE_MODE=1 — plugin discovery skipped")
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to eden_cli/. Two layouts are
        ...
        # 2. User plugins (~/.eden/plugins/)
        user_dir = get_eden_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.eden/plugins/)
        if _env_enabled("EDEN_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".eden" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set EDEN_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with eden and must
            ...
            # all ~20 of them added several seconds to every `eden`
            # invocation — including plain `eden chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform Eden OE ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `eden plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``eden plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`eden plugins
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `eden plugins list`.
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``eden_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``eden_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="eden-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``eden tools`` TUI so plugin-provided toolsets appear
