"""Migrate Eden OE' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Eden OE' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Eden OE mcp_servers.<n>.command/args/env  → codex stdio transport
  Eden OE mcp_servers.<n>.url/headers       → codex streamable_http transport
  Eden OE mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Eden OE mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Eden-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Eden OE' own
...
MIGRATION_MARKER = (
    "# managed by eden-agent — `eden codex-runtime migrate` regenerates this section"
)
MIGRATION_END_MARKER = (
    "# end eden-agent managed section"
)
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Eden OE config.")
...
# Eden OE keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Eden OE' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, eden_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Eden OE MCP server config to the codex inline-table dict
    ...
    if not isinstance(eden_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(eden_cfg.get("command"))
    has_url = bool(eden_cfg.get("url"))
    ...
    for key in eden_cfg:
        ...
        elif key not in _KNOWN_EDEN_OE_KEYS:
            skipped.append(f"{key} (unknown Eden OE key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (EDEN_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Eden OE)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Eden OE' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Eden OE' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Eden OE
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="eden-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a EDEN_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed eden-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.eden`` symlink that happens to live under ``/private/var/folders``
...
def _build_eden_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Eden OE' own
    ...
    so a eden installed under /opt/, /usr/local/, or a venv all work.
    EDEN_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # EDEN_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_eden_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever EDEN_HOME its launcher sets
    ...
    # — that would override the launcher's EDEN_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("EDEN_HOME", tmp_path) would
    ...
    eden_home = os.environ.get("EDEN_HOME") or ""
    if eden_home and _looks_like_test_tempdir(eden_home):
        eden_home = ""
    if eden_home:
        env["EDEN_HOME"] = eden_home
    # PYTHONPATH passes through so a worktree-launched eden finds the
    ...
    env["EDEN_OE_QUIET"] = "1"
    env["EDEN_OE_REDACT_SECRETS"] = env.get("EDEN_OE_REDACT_SECRETS", "true")

    out: dict[str, Any] = {
        "command": sys.executable,
        "args": ["-m", "agent.transports.eden_tools_mcp_server"],
    }
    if env:
        out["env"] = env
...
def migrate(
    ...
    *,
    ...
    expose_eden_tools: bool = True,
) -> MigrationReport:
    """Translate Eden OE mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        eden_config: full ~/.eden/config.yaml dict
        ...
        expose_eden_tools: when True (default), register Eden OE' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Eden OE for tools
    ...
    eden_servers = (eden_config or {}).get("mcp_servers") or {}
    if not isinstance(eden_servers, dict):
        report.errors.append(
            "mcp_servers in Eden OE config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in eden_servers.items():
    ...
    # Inject Eden OE' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Eden OE for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/eden_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_eden_tools:
        translated["eden-tools"] = _build_eden_tools_mcp_entry()
        if "eden-tools" not in report.migrated:
            report.migrated.append("eden-tools")
