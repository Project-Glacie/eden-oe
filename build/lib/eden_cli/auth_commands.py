...
_OAUTH_CAPABLE_PROVIDERS = {"anthropic", "eden", "openai-codex", "xai-oauth", "qwen-oauth", "minimax-oauth"}
...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_eden_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:eden_pkce",
    ...
    if provider == "eden":
        # Codex-style auto-import: if a shared Eden credential lives at
        # <eden-root>/shared/nous_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `eden --profile <name>
        # auth add eden --type oauth` a one-tap operation for users who
        ...
        if shared:
            ...
            print()
            if path:
                print(f"Found existing Eden OAuth credentials at {path}")
            else:
        ...
        # Honor `--label <name>` so eden matches other providers' UX.  The
        # helper embeds this into providers.eden so that label_from_token
        # doesn't overwrite it on every subsequent load_pool("eden").
        custom_label = (getattr(args, "label", None) or "").strip() or None
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``eden auth add openai-codex`` overwrote
    ...
    raise SystemExit(f"`eden auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Eden OE reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `eden auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `eden auth` is called bare."""
