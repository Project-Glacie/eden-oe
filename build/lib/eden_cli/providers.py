"""
Single source of truth for provider identity in Eden OE.
...
2. **Eden overlays** — transport type, auth patterns, aggregator flags,
...
# -- Eden OE overlay ----------------------------------------------------------
# Eden-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class EdenOverlay:
    """Eden-specific provider metadata layered on top of models.dev."""
...
EDEN_OE_OVERLAYS: Dict[str, EdenOverlay] = {
    "moa": EdenOverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": EdenOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "eden": EdenOverlay(
        ...
        base_url_override="https://inference-api.projectglacie.com/v1",
    ),
    "openai-codex": EdenOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": EdenOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": EdenOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": EdenOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="EDEN_OE_QWEN_BASE_URL",
    ),
    "lmstudio": EdenOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": EdenOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": EdenOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": EdenOverlay(
    ...
    "minimax-oauth": EdenOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": EdenOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": EdenOverlay(
        transport="openai_chat",
    ...
    "alibaba-coding-plan": EdenOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "opencode": EdenOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": EdenOverlay(
        transport="openai_chat",
        is_aggregator=True,
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "eden", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Eden OE overlays (for providers not in models.dev: eden, openai-codex, etc.)
      2. models.dev catalog + Eden OE overlay
    ...
    overlay = EDEN_OE_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + eden extra
    ...
    if overlay is not None:
        # Eden-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in EDEN_OE_OVERLAYS
    if provider == "bedrock":
