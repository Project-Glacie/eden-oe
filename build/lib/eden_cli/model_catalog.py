...
The Eden OE docs site hosts a JSON manifest of curated models for providers
we want to update without shipping a release (currently OpenRouter and
Eden Portal). This module fetches, validates, and caches that manifest,
...
1. ``get_catalog()`` — returns a parsed manifest dict.
   ...
   - Reads disk cache at ``~/.eden/cache/model_catalog.json``.
...
DEFAULT_CATALOG_URL = (
    "https://eden-agent.projectglacie.com/docs/api/model-catalog.json"
)
...
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    "https://raw.githubusercontent.com/ProjectGlacie/eden-agent/main/website/static/api/model-catalog.json",
)
DEFAULT_TTL_HOURS = 1
...
def _cache_path() -> Path:
    ...
    return get_eden_home() / "cache" / "model_catalog.json"
...
def get_curated_nous_models() -> list[str] | None:
    """Return Eden Portal's curated list of model ids from the manifest.

    Returns ``None`` when the manifest is unavailable.
    """
    block = _get_provider_block("eden")
    if not block:
        return None
    out: list[str] = []
    for m in block.get("models", []):
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``eden update`` pulls the latest repo, so the freshly-pulled
    ...
    to ``~/.eden/cache/model_catalog.json`` via the same atomic writer the
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``eden model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
