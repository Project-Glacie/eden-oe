...
Pure logic: walks a Eden OE config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `eden doctor` and a future `eden migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Eden OE config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Eden OE config and return retirement issues.
...
def apply_migration(
    ...
    from eden_cli.config import require_readable_config_before_write
