...
on Japanese locales, etc.).  Eden OE's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``eden_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Eden OE is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``EDEN_OE_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("EDEN_OE_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Eden-managed Git install directories so
    ...
    # already-running shells don't see the change — which means eden
    ...
    # install without Eden-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Eden OE's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Eden OE's
    ``eden config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Eden OE (User env var in Windows Settings, or
    export in a PowerShell profile) and Eden OE picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Eden-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\eden\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``eden`` from right after install) retain their old PATH.

    Any subprocess Eden OE spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Eden-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "eden", "git", "cmd"),
        os.path.join(local_appdata, "eden", "git", "bin"),
        os.path.join(local_appdata, "eden", "git", "usr", "bin"),
        # Eden OE venv Scripts directory — host of the eden.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes eden, but harmless to include.
        os.path.join(local_appdata, "eden", "eden-agent", "venv", "Scripts"),
