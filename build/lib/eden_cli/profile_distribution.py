"""Profile distributions — shareable, packaged Eden OE profiles via git.

A distribution is a Eden OE profile published as a git repository (or
...
* ``eden profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``eden skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``eden profile``, not a parallel tree):

    eden profile install <source> [--name N] [--alias] [--force] [--yes]
    eden profile update  <name>  [--force-config] [--yes]
    eden profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "eden-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            eden_requires=str(data.get("eden_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.eden_requires:
            out["eden_requires"] = self.eden_requires
        if self.author:
...
def check_eden_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Eden OE {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this Eden OE distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``eden profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Eden OE profile distribution."
            )
...
def plan_install(
    ...
    from eden_cli import __version__ as eden_version
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Eden OE distribution."
        )

    # Version check up-front so we fail fast
    check_eden_requires(manifest.eden_requires, eden_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.eden).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="eden_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `eden profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`eden profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="eden_dist_update_") as tmp:
        plan = plan_install(
