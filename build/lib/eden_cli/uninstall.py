...
- Keep data: Remove code but keep ~/.eden/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Eden OE PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing eden-agent or eden PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Eden OE" comment and following line
                if '# Eden OE' in line or '# eden-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('eden' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing eden
                if 'eden' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the eden wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "eden",
        Path("/usr/local/bin/eden"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains eden_cli reference)
                content = wrapper.read_text()
                if 'eden_cli' in content or 'eden-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(eden_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``eden`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Eden OE home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (eden_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``eden gateway run`` processes
    ...
    try:
        from eden_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from eden_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``EDEN_HOME`` and ``EDEN_OE_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\eden\git\cmd``, ``%LOCALAPPDATA%\eden\git\bin``,
#      ``%LOCALAPPDATA%\eden\git\usr\bin``, ``%LOCALAPPDATA%\eden\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\eden\git\`` and Node to
#      ``%LOCALAPPDATA%\eden\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``eden dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\eden\gateway-service\`` and sometimes
...
def _eden_path_markers(eden_home: Path) -> list[str]:
    """Path-entry substrings that identify Eden-owned User-PATH entries."""
    root = str(eden_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare eden-agent install dir.
    markers = [root + "\\eden-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if EDEN_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "eden".  We don't want to catch
    # unrelated entries like "ceden-foo" or "ephermeral", so we look for
    # backslash-eden as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(eden_home: Path) -> list[str]:
    """Strip Eden-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _eden_path_markers(eden_home)
            kept: list[str] = []
...
def remove_eden_env_vars_windows() -> list[str]:
    """Delete EDEN_HOME and EDEN_OE_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("EDEN_HOME", "EDEN_OE_GIT_BASH_PATH"):
                try:
...
def _is_default_eden_home(eden_home: Path) -> bool:
    """Return True when ``eden_home`` points at the default (non-profile) root."""
    try:
        from eden_constants import get_default_eden_root
        return eden_home.resolve() == get_default_eden_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its EDEN_HOME directory.

    We shell out to ``eden -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current EDEN_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m eden_cli.main` so we don't depend on a `eden`
    #    wrapper that may be half-removed mid-uninstall.
    eden_invocation = [_sys.executable, "-m", "eden_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                eden_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's EDEN_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``eden uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$EDEN_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from eden_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(eden_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ❤️ Eden OE Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Eden OE Chat GUI installation was found.")
        print(f"  Checked: {eden_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Eden OE agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(eden_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Eden OE agent at {eden_home / 'eden-agent'}")
        print(f"  • Your config, sessions, and secrets under {eden_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Eden OE agent is still installed. Run 'eden' to use the CLI,")
    print("or 'eden uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.eden/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.eden/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie EDEN_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_eden_home(eden_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.eden data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ❤️ Eden OE Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {eden_home / 'config.yaml'}")
    print(f"  Secrets: {eden_home / '.env'}")
    print(f"  Data:    {eden_home / 'cron/'}, {eden_home / 'sessions/'}, {eden_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their EDEN_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Eden OE data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Eden OE code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, eden_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Eden OE PATH entries from shell configs / Windows User PATH")
    print("  • Eden OE wrapper scripts and Eden-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Eden OE config/data: {eden_home}")
        if _is_default_eden_home(eden_home):
        ...
        print(f"  • Keep Eden OE config/data: {eden_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``eden`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$EDEN_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in eden_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\eden\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(eden_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Eden-owned PATH entries in User environment")

        log_info("Removing EDEN_HOME / EDEN_OE_GIT_BASH_PATH User env vars...")
        removed_env = remove_eden_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Eden-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing eden command...")
    ...
    #     (only when they still point into this Eden OE home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Eden-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(eden_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Eden-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(eden_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE EDEN_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from eden_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(eden_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.eden/, just remove the eden-agent subdir
            if eden_home in project_root.parents or project_root.parent == eden_home:
    ...
    #     under EDEN_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(eden_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(eden_home)
    ...
    # 5. Optionally remove ~/.eden/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile EDEN_HOME dirs live under
        ...
        try:
            if eden_home.exists():
                shutil.rmtree(eden_home)
                log_success(f"Removed {eden_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {eden_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {eden_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://eden-agent.projectglacie.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://eden-agent.projectglacie.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Eden OE! ❤️")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m eden_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import eden_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``eden_constants`` + ``eden_cli.colors``
    (and lazily ``eden_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m eden_cli.uninstall")
    parser.add_argument(
        "--mode",
