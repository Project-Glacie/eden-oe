...
- ``list_authenticated_providers`` already populates each row's
  ...
  curation and pulls in non-agentic models (Eden /models returns ~400
...
def build_models_payload(
    ...
    *,
    ...
    - ``pricing``: enrich each row with formatted per-model pricing and,
      for Eden, ``free_tier``/``unavailable_models`` so the GUI picker can
      show $/Mtok columns and gate paid models on free accounts —
      mirroring the ``eden model`` CLI picker. Adds network calls
      (pricing fetch + Eden tier check); only set for interactive pickers.
    ...
    - ``force_fresh_nous_tier``: bypass the short Eden free-tier cache when
      selecting Portal-recommended Eden models and applying tier gating. Keep
    ...
    try:
        from eden_cli.providers import is_routing_aggregator as _is_routing_aggregator
    except Exception:
...
def _append_unconfigured_rows(
    ...
    *,
    ...
    from eden_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    for entry in CANONICAL_PROVIDERS:
        ...
        if entry.slug.lower() == cur:
            ...
            warning = (
                ...
                else "Configured provider is not authenticated; run `eden model` to reactivate. "
                "Showing the saved model only."
            )
            extras.append(
                {
...
def _filter_explicit_provider_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    pickers want the narrower subset the user explicitly configured for Eden OE.
    """
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (
            ...
            if auth_type == "api_key" and key_env
            else f"run `eden model` to configure ({auth_type})"
        )
...
def _reorder_canonical(rows: list[dict]) -> list[dict]:
    ...
    """
    from eden_cli.models import CANONICAL_PROVIDERS
...
def _apply_pricing(
    ...
    *,
    ...
    # Resolve Eden free-tier once (cached in models.py for the TTL window).
...
def _moa_provider_row(current_provider: str = "") -> dict | None:
    ...
    picker path (:func:`eden_cli.model_switch.list_picker_providers`) so the
