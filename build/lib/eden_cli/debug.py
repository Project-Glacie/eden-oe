"""``eden debug`` debug tools for Eden OE.

Currently supports:
    eden debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.eden/logs/*.log`` are not leaked into
                          the public paste service. Pass ``--no-redact``
                          to disable.
                          Pass ``--eden`` to upload instead to Eden-internal
                          storage (AWS S3) via a signed URL minted by the
                          Eden account service: the bundle is private
                          (viewable only by Eden staff / allowlisted mods via
...
_REDACTION_BANNER = (
    "[eden debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.eden/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``eden debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``eden debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `eden debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "eden-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``eden debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.eden/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``eden debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "eden-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----EdenDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from eden_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_eden_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``eden dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``eden dump`` is run
        internally.
...
# ---------------------------------------------------------------------------
# Shared bundle collection (used by both the paste.rs and Eden-S3 paths)
# ---------------------------------------------------------------------------

# Bundle format identifier embedded in the Eden-S3 JSON envelope. The
# discord-support viewer keys off this string to parse the bundle.
_NOUS_BUNDLE_FORMAT = "eden-debug-share/1"
...
def collect_share_bundle(
    ...
    Eden-S3 path (``--eden``).  Centralising it guarantees the Eden bundle is
    built from the *same* force-redacted snapshots as the public paste path —
    redaction is the safety boundary, so the Eden path must never see raw
    logs.
...
def build_nous_bundle(bundle: dict[str, str], redact: bool = True) -> bytes:
    """Gzip-compress a :func:`collect_share_bundle` mapping into the Eden envelope.

    The JSON shape is what the discord-support viewer (Repo 3) parses::

        {"format": "eden-debug-share/1",
         "redacted": <bool>,
         "created": <iso8601>,
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``eden debug share`` (CLI) and the
    ...
    # shared collector so the paste.rs and Eden-S3 paths build identical,
    ...
    if redact:
        logger.info(
            "eden debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `eden debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print("To delete now:  eden debug delete <url>")

    print("\nShare these links with the Eden OE team for support.")


_NOUS_PRIVACY_NOTICE = """\
⚠️  --eden: This uploads your debug bundle to Eden-INTERNAL storage (AWS S3),
    NOT a public paste service. The following is included:
  • System info (OS, Python/Eden version, provider, which API keys are
  ...
  • The bundle is viewable only by Eden staff (and allowlisted Discord mods)
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``eden debug share --eden``: upload the bundle to Eden-S3.

    Collects the same force-redacted bundle as the paste path, gzips it into
    the Eden envelope, requests a signed URL from NAS, uploads, and prints the
    private viewer link. On any failure falls back to a clear error that
    suggests ``--local``.
    """
    from eden_cli.diagnostics_upload import share_to_nous
    ...
    if redact:
        logger.info(
            "eden debug share --eden: applied force-mode redaction before upload"
        )
    blob = build_nous_bundle(bundle, redact=redact)

    print("Uploading to Eden diagnostics storage...")
    try:
        res = share_to_nous(blob)
    except Exception as exc:
        print(
            f"\nNous upload failed: {exc}\n"
            "\nThe Eden diagnostics service may be unavailable or not yet "
            "provisioned.\n"
            "Run `eden debug share --local` to print the report instead, "
            "or `eden debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)

    view_url = res.get("viewUrl") or res.get("view_url")
    print("\nDebug bundle uploaded to Eden (private):")
    if view_url:
    ...
    print(
        "\nShare this private link with the Eden team — only Eden staff "
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: eden debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'eden debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``eden debug`` call.
    ...
    # best-effort — any failure is swallowed so ``eden debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: eden debug <command>")
        print()
        print("Commands:")
        ...
        print("  --eden       Upload to Eden-internal storage (private, staff-only,")
