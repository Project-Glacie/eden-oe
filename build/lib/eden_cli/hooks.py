"""eden hooks — inspect and manage shell-script hooks.

Usage::

    eden hooks list
    eden hooks test <event> [--for-tool X] [--payload-file F]
    eden hooks revoke <command>
    eden hooks doctor

Consent records live under ``~/.eden/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.eden/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``eden hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: eden hooks {list|test|revoke|doctor}")
        print("Run 'eden hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.eden/config.yaml.")
        print("See `eden hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `eden hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `eden hooks test` and `eden hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Eden OE wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `eden hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `eden hooks doctor` would execute
