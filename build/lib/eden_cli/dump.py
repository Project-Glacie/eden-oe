"""
Dump command for eden CLI.

Outputs a compact, plain-text summary of the user's Eden OE setup
...
from eden_cli.config import get_eden_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.eden/.env.
    ...
    exports. ``eden debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.eden_build_sha`` by the Dockerfile's
    ``EDEN_OE_GIT_SHA`` build-arg (see ``eden_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``eden dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from eden_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(eden_home: Path) -> int:
...
def _cron_summary(eden_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = eden_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"eden_home:      {display_eden_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.eden/.env: a managed
        ...
        # A credential added via `eden auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `eden auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["eden-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(eden_home)}")
    lines.append(f"  skills:             {_count_skills(eden_home)}")
