#!/usr/bin/env python3
"""Live test harness for Eden OE's Tool Search feature.
...
# Force-isolate the test environment BEFORE any eden imports.
ORIGINAL_HOME = os.environ.get("EDEN_HOME")
ORIGINAL_AUTH = Path.home() / ".eden" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.eden/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.eden/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="eden_ts_live_"))
    eden_home = home_dir / ".eden"
    eden_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, eden_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".eden" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, eden_home / ".env")
        ...
        load_eden_dotenv(eden_home=str(Path.home() / ".eden"))
    ...
    (eden_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return eden_home
...
def reset_module_state():
    """Drop cached modules so the new EDEN_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "eden_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def main():
    ...
    # Restore original EDEN_HOME
    if ORIGINAL_HOME is not None:
        os.environ["EDEN_HOME"] = ORIGINAL_HOME
    else:
        os.environ.pop("EDEN_HOME", None)
