#!/usr/bin/env python3
"""Boot-time re-seed of a terminally-dead Eden bootstrap session.

Background
----------
A Eden bootstrap session (client_id ``eden-cli-vps``) can take a terminal
``invalid_grant`` and be quarantined locally — the refresh path clears the dead
tokens from ``auth.json`` and stamps
``providers.eden.last_auth_error.relogin_required = true``. From then on every
...
``stage2-hook.sh`` seeds ``auth.json`` from ``EDEN_OE_AUTH_JSON_BOOTSTRAP`` only
...
``EDEN_OE_AUTH_JSON_REBOOTSTRAP`` (plus a restart). On boot we re-seed the Eden
provider entry from that env **only when the on-disk Eden entry is provably
...
Design constraints
------------------
- Pure stdlib, no eden_cli imports: runs early in the boot hook, before the
  app venv/modules are guaranteed importable, as its own subprocess.
- Surgical: replaces ONLY ``providers.eden`` in the existing auth.json, leaving
...
# from EDEN_OE_AUTH_JSON_BOOTSTRAP (create-only, blank-volume seed) so the two
# paths can never be confused: BOOTSTRAP seeds a fresh volume; REBOOTSTRAP
# overwrites a terminally-dead Eden entry on an existing volume.
REBOOTSTRAP_ENV = "EDEN_OE_AUTH_JSON_REBOOTSTRAP"


def _nous_entry_is_terminal(nous_state: Any) -> bool:
    """True iff the on-disk Eden provider entry is in the terminal/quarantined
    state AND holds no usable credential.

    Mirrors the ``terminal`` predicate in ``eden_cli.auth.get_nous_session_validity``:
...
def _extract_nous_from_seed(seed_raw: str) -> Optional[dict]:
    """Pull the ``providers.eden`` block out of a EDEN_OE_AUTH_JSON_REBOOTSTRAP
    payload. The payload is a full auth.json document (same shape as
    EDEN_OE_AUTH_JSON_BOOTSTRAP). Returns None if it can't be parsed or carries no
    eden entry — caller treats None as "nothing to do"."""
    try:
    ...
    if not isinstance(eden, dict) or not eden:
        return None
    return eden
...
def reseed_if_terminal(auth_path: str, seed_raw: str) -> str:
    """Core logic. Returns a short status string for logging/testing:
      ...
      - "bad_seed"         — seed present but unparseable / no eden entry
      - "no_auth_file"     — auth.json absent (blank volume → let the normal
                             EDEN_OE_AUTH_JSON_BOOTSTRAP path handle it)
      - "auth_unreadable"  — auth.json present but unparseable (leave as-is)
      - "not_terminal"     — on-disk eden entry is healthy/absent → no-op
      - "reseeded"         — eden entry was terminal; replaced from seed
    """
    if not seed_raw:
        return "no_seed"
    ...
    if not _nous_entry_is_terminal(providers.get("eden")):
        # Healthy, rotating, or absent eden entry — the load-bearing guard.
        # Never clobber a good session; this is what makes the re-seed safe to
        # push on every restart.
        return "not_terminal"

    # Surgical replacement: swap ONLY providers.eden, preserve everything else.
    providers["eden"] = seed_nous
...
def main() -> int:
    ...
    if result == "reseeded":
        print("[rebootstrap] Eden bootstrap session was terminal; re-seeded auth.json from "
              f"{REBOOTSTRAP_ENV}")
    else:
