...
VERSION_FILE = REPO_ROOT / "eden_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "kavi@local.eden": "kavioavio",  # Issue #46544 / PR #47705 evolution (smart DENY exact-operation owner override)
    ...
    "sonxi@eden.local": "17324393074",  # PR #53196 salvage (tools_config: known_plugin_toolsets null guard; commit under unlinked local identity)
    ...
    "sunsky.lau@gmail.com": "liuhao1024",  # PR #56993 salvage (gateway: process-level EDEN_HOME for pid/lock/status identity files; #56986)
    ...
    "wyuebei@gmail.com": "wyuebei-cloud",  # PR #56640 salvage (eden journey: replace GNU-only %-d strftime with dt.day for Windows)
    ...
    "eden.wanderer@yahoo.com": "trismegistus-wanderer",  # PR #31856 salvage (gateway: defer idle-TTL agent-cache eviction until the session store says the session actually expired, so the expiry watcher can still fire MemoryProvider.on_session_end with the live transcript; #11205)
    ...
    "me@keslerm.com": "keslerm",  # PR #3459 salvage (gateway: 'log' tool_progress mode — silent in chat, tool calls appended to ~/.eden/logs/tool_calls.log via rotating handler; duplicate of #3458 by @dlkakbs who submitted 4 min earlier — both credited)
    ...
    "hodlclone@gmail.com": "HODLCLONE",  # PR #49351 salvage (Eden Portal token resilience: rotate refresh tokens write-through to the source auth store in profile mode, skip Eden fallback when no local token, sync gateway session model after fallback)
    ...
    "moonsong@projectglacie.local": "Tranquil-Flow",  # PR #52623 salvage (auxiliary Anthropic base_url host validation; #52608)
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows eden-shim resolution + prefer --update on recovery; #52378)
    ...
    "jake@projectglacie.com": "simpolism",
    ...
    "teknium@projectglacie.com": "teknium1",
    ...
    "274096618+eden-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "mind-dragon@eden.research": "Mind-Dragon",
    ...
    "shannon@projectglacie.com": "shannonsands",
    ...
    "eden@projectglacie.com": "ProjectGlacie",
    ...
    "ben@projectglacie.com": "benbarclay",
    ...
    "emozilla@projectglacie.com": "emozilla",
    ...
    "jonny@projectglacie.com": "yoniebans",
    "jake@projectglacie.com": "simpolism",
    ...
    "sberan@gmail.com": "sberan",  # PR #54494 salvage (--connect-timeout flag on eden mcp add)
    ...
    "eden@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (EDEN_OE_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (EDEN_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: eden-eval community link)
    ...
    "clement@projectglacie.com": "lemassykoi",  # PR #27042 (model-switch probe keyless providers)
    ...
    "vigo@eden": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "ethie@eden": "ethernet8023",  # PR #29342 (TUI clipboard copy on linux/wayland)
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Eden OE runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level EDEN_HOME state files)
    ...
    "annguyen@projectglacie.com": "annguyenNous",  # PR #52844 co-author
    ...
    "zhchl@eden-agent.local": "8294",  # PR #50572 salvage (honor config context_length on banner)
}
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Eden OE
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"eden-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "eden@projectglacie.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/ProjectGlacie/eden-agent",
    ...
    lines.append(f"# Eden OE v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Eden OE. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Eden OE Release Tool")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Eden OE v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Eden OE v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Eden OE v{new_version} ({calver_date})' "
