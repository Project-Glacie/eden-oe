"""CLI subcommand: ``eden send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"eden send: {file_path} is not a text file. --file reads the "
                ...
                f'  eden send --to telegram "MEDIA:{file_path}"\n'
                f'  eden send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  eden send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"eden send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"eden send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"eden send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"eden send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"eden send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `eden gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.eden/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_eden_env() -> None:
    """Populate ``os.environ`` from ``~/.eden/.env`` AND bridge top-level
    ...
    startup that ``eden send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.eden/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.eden/config.yaml`` into
    ...
    intentionally reimplement the minimum needed here so ``eden send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.eden/.env and ~/.eden/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "eden send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  eden send --to telegram \"hello\"\n"
            "  eden send --to discord:#ops --file report.md\n"
            "  eden send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "eden send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `eden send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Eden OE "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.eden/.env + ~/.eden/config.yaml) — no LLM, "
            "no agent loop, no running gateway required for bot-token "
            "platforms like Telegram/Discord/Slack/Signal."
        ),
        epilog=(
            "Examples:\n"
            "  eden send --to telegram \"deploy finished\"\n"
            "  echo \"RAM 92%\" | eden send --to telegram:-1001234567890\n"
            "  eden send --to discord:#ops --file /tmp/report.md\n"
            "  eden send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  eden send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
            "  eden send --list                  # all platforms\n"
            "  eden send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `eden send --list telegram`.",
    )
