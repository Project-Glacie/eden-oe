...
- ``eden_cli.providers``        -- canonical provider identity + overlays
- ``eden_cli.model_normalize``  -- per-provider name formatting
...
_EDEN_OE_MODEL_WARNING = (
    "Project Glacie Eden OE 3 & 4 models are NOT agentic and are not designed "
    "for use with Eden OE. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Project Glacie Eden OE 3 / Eden OE 4 chat families.
# The previous substring check (`"eden" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``eden-brain:qwen3-14b-ctx16k`` that just
# happen to carry "eden" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   ProjectGlacie/Eden-3-Llama-3.1-70B, eden-4-405b, openrouter/eden3:70b
# Negative examples it must NOT match:
#   eden-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_EDEN_OE_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])eden[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_eden_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Eden Eden OE 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_EDEN_OE_NON_AGENTIC_RE.search(model_name))


def _check_eden_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Eden Eden OE 3/4 chat model."""
    if is_nous_eden_non_agentic(model_name):
        return _EDEN_OE_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``eden config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from eden_cli.model_switch import
...
def _resolve_alias_fallback(
    ...
    Falls back to ``("openrouter", "eden")`` only when no authenticated
    providers are supplied (backwards compat for non-interactive callers).
    """
    providers = authenticated_providers or ("openrouter", "eden")
...
def resolve_display_context_length(
    ...
    about Codex OAuth, Copilot, Eden, and falls back to models.dev for the
    rest.
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'eden model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'eden doctor' — config issues detected:"
        ...
        from eden_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from eden_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from eden_cli.config import load_config
            from eden_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from eden_cli.providers import resolve_user_provider as _ruser
    ...
    # eden_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    if target_provider in {"opencode-zen", "opencode-go"} and isinstance(base_url, str):
        from eden_cli.models import normalize_opencode_base_url
    ...
    eden_warn = _check_eden_model_warning(new_model)
    if eden_warn:
        warnings.append(eden_warn)
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from eden_cli.config import normalize_extra_headers
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from eden_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from eden_cli/models.py (OPENROUTER_MODELS,
    ...
    ``force_fresh_nous_tier`` bypasses the short Eden tier cache for explicit
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from eden_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by eden provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "eden" pulls from the remote model-catalog manifest published at
    # https://eden-agent.projectglacie.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Eden OE release. Falls back to the in-repo
    # _PROVIDER_MODELS["eden"] snapshot when the manifest is unreachable.
    curated["eden"] = get_curated_nous_model_ids()
    # Ollama Cloud uses dynamic discovery (no static curated list)
    if "ollama-cloud" not in curated:
        from eden_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from eden_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check Eden-mapped providers ---
    from eden_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from eden_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for eden_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(eden_id)
        if (
            _alias_target
            and _alias_target != eden_id
        ...
        pconfig = PROVIDER_REGISTRY.get(eden_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (EDEN_OE_OVERLAYS) with proper auth store checking.
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers Eden OE may not route yet.
        ...
        from eden_cli.auth import is_runtime_provider_routable
        if not is_runtime_provider_routable(eden_id):
            continue
        ...
        if not has_creds:
            try:
                from eden_cli.auth import _load_auth_store
                store = _load_auth_store()
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(eden_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        eden_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `eden model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(eden_id)
        if not model_ids:
            model_ids = curated.get(eden_id, [])
            if eden_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(eden_id, model_ids)
    ...
    # --- 2. Check Eden-only providers (eden, openai-codex, copilot, opencode-go) ---
    from eden_cli.providers import EDEN_OE_OVERLAYS
    from eden_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Eden OE provider ID.
    # EDEN_OE_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Eden OE IDs ("copilot").
    _mdev_to_eden = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in EDEN_OE_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Eden OE slug — e.g. "github-copilot" → "copilot"
        eden_slug = _mdev_to_eden.get(pid, pid)
        if eden_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(eden_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, eden_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or eden_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                if _credential_pool_is_usable(eden_slug):
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", eden_slug, exc)
        ...
        # configured.
        if not has_creds and eden_slug == "anthropic":
            try:
                ...
                if (eden_creds and eden_creds.get("accessToken")) or \
        ...
        if eden_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(eden_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(eden_slug)
                model_ids = _ids if _ids else (curated.get(eden_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(eden_slug, []) or curated.get(pid, [])
        elif eden_slug == "eden":
            # Eden serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `eden model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from eden_cli.auth import get_provider_auth_state as _nous_state

                _pricing = _nous_pricing("eden") or {}
                _portal = ""
                try:
                    _st = _nous_state("eden") or {}
            ...
            if not model_ids:
                model_ids = curated.get(eden_slug, []) or curated.get(pid, [])
                if eden_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(eden_slug, model_ids)
        total = len(model_ids)
        if eden_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": eden_slug,
            "name": get_label(eden_slug),
            "is_current": eden_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "eden",
        ...
        seen_slugs.add(eden_slug.lower())
        _record_builtin_endpoint(eden_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or EDEN_OE_OVERLAYS (keeps /model in sync
    # with `eden model`).
    try:
        from eden_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Eden OE's canonical write key (matches
            ...
            # Eden OE writes ``models:`` as a dict keyed by model id, but older
            ...
            if should_probe:
                try:
                    from eden_cli.models import fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Eden OE's own writer uses; a hyphen variant is accepted
            ...
            # active model. Eden OE's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, eden_cli/config.py) already consume that dict.
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from eden_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`eden_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from eden_cli.models import fetch_openrouter_models
