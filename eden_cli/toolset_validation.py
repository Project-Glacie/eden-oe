...
the tool registry or launching Eden OE (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``eden-cli`` to the non-existent ``eden``. ``resolve_toolset('eden')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``eden-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``eden`` instead of ``eden-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `eden tools` to reconfigure."
        )
    return warnings
