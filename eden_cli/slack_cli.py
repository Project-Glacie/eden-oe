"""``eden slack ...`` CLI subcommands.

Today only ``eden slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ eden slack manifest > slack-manifest.json
    # or:
    $ eden slack manifest --write
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Eden OE. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Eden OE deployment — users can tweak them in the Slack UI after
    pasting.
    ...
    ``/new``, etc. work inline.
    """
    from eden_cli.commands import slack_app_manifest
    ...
    if include_assistant:
        features["assistant_view"] = {
            "assistant_description": "Chat with Eden OE in threads and DMs.",
        }
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your Eden OE agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``eden_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$EDEN_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Eden")
    ...
    """
    name = getattr(args, "name", None) or "Eden"
    description = getattr(args, "description", None) or "Your Eden OE agent on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("EDEN_HOME") or str(Path.home() / ".eden")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Eden OE app\n"
