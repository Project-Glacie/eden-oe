"""Shell completion script generation for eden CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_eden_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Eden OE bash completion
# Add to ~/.bashrc:
#   eval "$(eden completion bash)"

_eden_profiles() {{
    local profiles_dir="$HOME/.eden/profiles"
...
_eden_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_eden_profiles)" -- "$cur"))
        return
    fi
...
complete -F _eden_completion eden
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef eden
# Eden OE zsh completion
# Add to ~/.zshrc:
#   eval "$(eden completion zsh)"

_eden_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.eden/profiles" ]]; then
        profiles+=($HOME/.eden/profiles/*(N/:t))
    fi
...
_eden() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_eden_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'eden command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Eden OE fish completion",
        "# Add to your config:",
        "#   eden completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __eden_profiles",
        "    echo default",
        "    if test -d $HOME/.eden/profiles",
        "        for d in $HOME/.eden/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c eden -f -s p -l profile"
        " -d 'Profile name' -xa '(__eden_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__eden_profiles)' -d 'Profile name'"
                )
