"""Windows ConPTY bridge for the `eden dashboard` chat tab.

Drop-in counterpart to ``eden_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``eden_cli.web_server`` consumes: ``spawn``, ``read``,
