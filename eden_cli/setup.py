"""
Interactive setup wizard for Eden OE.
...
from eden_constants import get_optional_skills_dir

logger = logging.getLogger(__name__)

PROJECT_ROOT = Path(__file__).parent.parent.resolve()

_DOCS_BASE = "https://eden-agent.projectglacie.com/docs"
...
# display_eden_home imported lazily at call sites (stale-module safety during eden update)
...
from eden_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from eden_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("❤️ Eden OE Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Eden OE using environment variables or config commands:")
    print_info("  eden config set model.provider custom")
    print_info("  eden config set model.base_url http://localhost:8080/v1")
    print_info("  eden config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'eden setup' in an interactive terminal to use the full wizard.")
    print()
...
def is_noninteractive() -> bool:
    ...
    ``EDEN_OE_NONINTERACTIVE=1`` (see ``eden_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("EDEN_OE_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``EDEN_OE_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from eden_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'eden setup')")


def _print_setup_summary(config: dict, eden_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'eden setup' to configure"))


    # Web tools (Exa, Parallel, Firecrawl, or Tavily)
    if subscription_features.web.managed_by_nous:
        tool_status.append(("Web Search & Extract (Eden subscription)", True, None))
    ...
    if subscription_features.browser.managed_by_nous:
        tool_status.append(("Browser Automation (Eden Browser Use)", True, None))
    ...
    # Image generation — FAL (direct or via Eden), or any plugin-registered
    # provider (OpenAI, etc.)
    if subscription_features.image_gen.managed_by_nous:
        tool_status.append(("Image Generation (Eden subscription)", True, None))
        ...
        try:
            ...
            from eden_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `eden tools` → Video Generation.
    # Only show the row when a plugin reports available so we don't badger
    # users who don't care about video gen with a "missing" status line.
    if subscription_features.video_gen.managed_by_nous:
        tool_status.append(("Video Generation (FAL via Eden subscription)", True, None))
    else:
        try:
            from agent.video_gen_registry import list_providers as _list_video_providers
            from eden_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        tool_status.append(("Text-to-Speech (OpenAI via Eden subscription)", True, None))
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'eden setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'eden setup tts'"))
    else:
        tool_status.append(("Text-to-Speech (Edge TTS)", True, None))

    if subscription_features.modal.managed_by_nous:
        tool_status.append(("Modal Execution (Eden subscription)", True, None))
    elif cfg_get(config, "terminal", "backend") == "modal":
        if subscription_features.modal.direct_override:
            tool_status.append(("Modal Execution (direct Modal)", True, None))
        else:
            tool_status.append(("Modal Execution", False, "run 'eden setup terminal'"))
    elif managed_nous_tools_enabled() and subscription_features.nous_auth_present:
        tool_status.append(("Modal Execution (optional via Eden subscription)", True, None))

    # Home Assistant
    if get_env_value("HASS_TOKEN"):
        tool_status.append(("Smart Home (Home Assistant)", True, None))

    # Spotify (OAuth via eden auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'eden setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {eden_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('eden setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('eden setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('eden setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('eden setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('eden setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('eden config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('eden config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('eden config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('eden', Colors.GREEN)}              Start chatting")
    print(f"   {color('eden gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('eden doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `eden tools` and `eden setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``eden model``)
    ...
    provider added to ``eden model`` is automatically available here.
    ...
    # Delegate to the shared eden model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from eden_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: eden model")
    ...
    # on demand via `eden auth add`, `eden setup` vision, and
    # `eden setup tts`. This keeps both quick and full setup thin.
...
def _install_neutts_deps() -> bool:
    ...
    from eden_cli.tools_config import _pip_install
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``eden model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from eden_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if managed_nous_tools_enabled() and subscription_features.nous_auth_present:
        choices.append("Eden Subscription (managed OpenAI TTS, billed to your subscription)")
        providers.append("eden-openai")
    choices.extend(
        [
    ...
    selected_via_nous = selected == "eden-openai"
    if selected == "eden-openai":
        selected = "openai"
        print_info("OpenAI TTS will use the managed Eden gateway and bill to your subscription.")
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.eden/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Eden OE auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via eden setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'eden setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Eden OE runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `eden setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `eden setup terminal`.
        ...
        if use_managed_modal:
            ...
            print_info("Modal execution will use the managed Eden gateway and bill to your subscription.")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into EDEN_OE_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("EDEN_OE_MAX_ITERATIONS")
    ...
    print_info("  Run `eden setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # EDEN_OE_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.eden/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from eden_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_eden_home(Path(get_eden_home()))
    except Exception:
        pass
...
def _profile_name_from_eden_home(eden_home) -> str | None:
    """Return the active profile name when EDEN_HOME is a profile dir."""
    if eden_home.parent.name == "profiles":
        return eden_home.name
    return None
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Eden OE delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Eden OE to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from eden_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://eden-agent.projectglacie.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://eden-agent.projectglacie.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  eden config edit")
    print_info("   Open config in your editor:  eden config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from eden_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Eden OE from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'eden setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     eden config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_eden_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd and os.geteuid() == 0:  # windows-footgun: ok — guarded by supports_systemd (Linux only)
                    print_info("  Or as a boot-time service: eden gateway install --system")
                print_info("  Or run in foreground:  eden gateway")
        else:
            from eden_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   eden gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   eden gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `eden setup tools` and `eden tools` use the same flow:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from eden_cli.config
    ...
    if section_key == "model":
        ...
        from eden_cli.gateway import _all_platforms, _platform_status
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_eden migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_eden", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Eden OE.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Eden OE to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Eden OE at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Eden OE at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Eden OE at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Eden OE at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Eden OE equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Eden OE config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Eden OE.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Eden OE's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(eden_home: Path) -> bool:
    ...
    print_info("Eden OE can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: eden claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: eden claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Eden OE configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Eden OE config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Eden OE (use eden claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    """One-shot Eden Portal setup — OAuth + model pick + provider + Tool Gateway.

    Wired into ``eden setup --portal`` and ``eden portal``. This is the
    Eden-Portal slice of the first-time quick setup, collapsed into a single
    shareable command so a brand-new user goes from zero to a fully working
    Eden OE session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``eden setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``eden model`` runs
    when you pick Eden. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``eden portal`` always offers a model picker,
    and there is a single source of truth for the Eden onboarding steps.
    """
    ...
    print(color("│     ❤️ Eden OE Setup — Eden Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    print_info("    — all routed through your Eden Portal sub.")
    print()
    print_info("  Sign up: https://portal.projectglacie.com/manage-subscription")
    print()
    ...
    # Eden model picker), then offers the Tool Gateway opt-in and sets
    # provider=eden via the login/model save. This is the same routine quick
    # setup calls, so `eden portal` == quick setup's Eden step.
    try:
        from eden_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `eden portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `eden portal`: %s", exc)
        print()
        print_error(f"  Eden Portal setup encountered an error: {exc}")
        print_info("  You can retry later with `eden portal`.")
        return
    ...
    print_info("  Run `eden portal info` to inspect routing.")
    print_info("  Run `eden` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      eden setup           — full or quick (auto-detected)
      eden setup model     — just model/provider
      eden setup tts       — just text-to-speech
      eden setup terminal  — just terminal backend
      eden setup gateway   — just messaging platforms
      eden setup tools     — just tool configuration
      eden setup agent     — just agent settings
    """
    from eden_cli.config import is_managed, managed_error
    if is_managed():
    ...
    # --portal: one-shot Eden Portal setup. Skips the rest of the wizard.
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ❤️ Eden OE Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ❤️ Eden OE Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Eden OE installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, eden_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Eden OE configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'eden setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(eden_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Eden OE?",
            [
                "Full setup — cloud provider + tools + Genesis ceremony (recommended)",
                "Quick Setup (Eden Portal) — for Eden Portal subscribers only",
        ...
        if setup_mode == 1:
            _run_first_time_quick_setup(config, eden_home, is_existing)
            return
        if setup_mode == 2:
            _run_blank_slate_setup(config, eden_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {eden_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'eden config edit'")
    ...
    # Tune later with `eden setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, eden_home)


def _run_first_time_quick_setup(config: dict, eden_home, is_existing: bool):
    """Streamlined first-time setup via Eden Portal: OAuth, model, terminal & messaging.

    Routes straight to the Eden Portal provider — runs the device-code OAuth
    login, picks a Eden model, then configures the terminal backend and (optionally)
    a messaging platform. Applies sensible defaults for everything else (agent
    settings, tools); the user can customize later via ``eden setup <section>``
    or switch providers with ``eden model``.
    """
    from eden_cli.config import load_config

    # Step 1: Eden Portal — OAuth login + model selection.
    # _model_flow_nous() handles both the logged-out path (device-code OAuth,
    # which selects a model internally) and the already-logged-in path (curated
    # Eden model picker). Provider is set to "eden" by the login/model save.
    print()
    print_header("Eden Portal")
    print_info("One subscription, 300+ models, plus the Tool Gateway:")
    print_info("  web search, image generation, TTS, browser automation.")
    print_info("Sign up: https://portal.projectglacie.com/manage-subscription")
    print()
    try:
        ...
        print()
        print_info("Eden Portal setup cancelled.")
    except Exception as exc:
        logger.debug("_model_flow_nous error during quick setup: %s", exc)
        print_warning(f"Eden Portal setup encountered an error: {exc}")
        print_info("You can try again later with: eden model")
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'eden setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    eden setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  eden setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``eden tools`` (which
    ...
    try:
        ...
        from eden_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``eden setup agent`` /
    ``eden config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, eden_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `eden update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        eden tools")
        print_info("  Seed skills:         eden skills opt-in --sync")
        print_info("  Add MCP servers:     eden mcp add")
        print_info("  Enable plugins:      eden plugins")
        print_info("  Tune agent settings: eden setup agent")
        print()
        _print_setup_summary(config, eden_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, eden_home)


def _blank_slate_walkthrough(config: dict, eden_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`eden update` runs from re-injecting them. Opt back in any")
            print_info("time with `eden skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `eden tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `eden plugins list` / `eden plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `eden plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `eden mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `eden mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   eden tools")
    print_info("  Seed skills:         eden skills opt-in --sync")
    print_info("  Add MCP servers:     eden mcp add")
    print_info("  Tune agent settings: eden setup agent")
    print()
...
def _run_quick_setup(config: dict, eden_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'eden setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Eden OE to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'eden setup gateway'.")
