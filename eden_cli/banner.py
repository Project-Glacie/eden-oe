...
Pure display functions with no EdenCLI state dependency.
"""
import json
import logging
import os
...
from eden_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE

EDEN_OE_LOGO = """[bold #FFD700]███████╗██████╗ ███████╗███╗   ██╗      ██████╗ ███████╗[/]
[bold #FFD700]██╔════╝██╔══██╗██╔════╝████╗  ██║      ██╔══██╗██╔════╝[/]
[#FFBF00]█████╗  ██║  ██║█████╗  ██╔██╗ ██║      ██║  ██║█████╗  [/]
[#FFBF00]██╔══╝  ██║  ██║██╔══╝  ██║╚██╗██║      ██║  ██║██╔══╝  [/]
[#CD7F32]███████╗██████╔╝███████╗██║ ╚████║      ╚██████╔╝███████╗[/]
...
# (e.g. nix-built eden — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/ProjectGlacie/eden-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/projectglacie/eden-agent"
...
def _fetch_pypi_latest(package: str = "eden-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Eden update is available.

    Two paths: if ``EDEN_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = eden_home / ".update_check"
    embedded_rev = os.environ.get("EDEN_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # EDEN_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `eden update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/eden/update/check`
    ...
    try:
        from eden_cli.config import detect_install_method, get_project_root
    ...
    if embedded_rev:
        ...
        # $EDEN_HOME/eden-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = eden_home / "eden-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Eden git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$EDEN_HOME/eden-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``eden_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from eden_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/ProjectGlacie/eden-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Eden OE checkout. Cached per-process. Release URL always points at the
    canonical ProjectGlacie/eden-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Eden OE v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    if (provider or "").strip().lower() == "moa":
        ...
        left_lines.append(f"[{accent}]MoA: {preset_name}[/]{agg_str}{ctx_str} [dim {dim}]·[/] [dim {dim}]Project Glacie[/]")
    else:
        ...
        ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
        left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]Project Glacie[/]")

    if os.getenv("EDEN_YOLO_MODE"):
    ...
    try:
        from eden_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from eden_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built eden; we know an update
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else EDEN_OE_LOGO
        console.print(_logo)
        console.print()
