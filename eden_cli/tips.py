...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or eden -c.",
    ...
    "Ctrl+Z suspends Eden OE to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "eden -c resumes your most recent CLI session. eden -c \"project name\" resumes by title.",
    "eden -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "eden -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "eden chat -t web,terminal enables only specific toolsets for a focused session.",
    "eden chat -s github-pr-workflow preloads a skill at launch.",
    "eden chat -q \"query\" runs a single non-interactive query and exits.",
    "eden chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "eden chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "eden --yolo bypasses all dangerous command approval prompts for the entire session.",
    "eden chat --source telegram tags the session for filtering in eden sessions list.",
    "eden -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "eden doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "eden dump outputs a compact setup summary — great for bug reports.",
    "eden config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "eden config edit opens config.yaml in your default editor.",
    "eden config check scans for missing or stale configuration options.",
    "eden sessions browse opens an interactive session picker with search.",
    "eden sessions stats shows session counts by platform and database size.",
    "eden sessions prune --older-than 30 cleans up old sessions.",
    "eden skills search react --source skills-sh searches the skills.sh public directory.",
    "eden skills check scans installed hub skills for upstream updates.",
    "eden skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "eden skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "eden mcp add github --command npx adds MCP servers from the command line.",
    "eden mcp serve runs Eden OE itself as an MCP server for other agents.",
    "eden auth add lets you add multiple API keys for credential pool rotation.",
    "eden completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "eden logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "eden backup creates a zip backup of your entire Eden OE home directory.",
    "eden profile create coder creates an isolated profile that becomes its own command.",
    "eden profile create work --clone copies your current config and keys to a new profile.",
    "eden update syncs new bundled skills to ALL profiles automatically.",
    "eden gateway install sets up Eden OE as a system service (systemd/launchd).",
    "eden memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "eden webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: eden tools disables unused tools, eden skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "eden models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as eden -w).",
    ...
    "EDEN_OE_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Eden OE tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'eden profile create coder' creates the 'coder' command.",
    "eden profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Eden OE prints a resume command with session ID and stats.",
    "eden sessions export backup.jsonl exports all sessions for backup or analysis.",
    "eden -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "eden skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .eden/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: eden cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Eden OE runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "eden gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Eden OE via a local macOS server.",
    ...
    "Context files (.eden.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Eden OE loads project context from .eden.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "eden mcp opens an interactive picker of Eden-approved MCPs you can install in one keystroke.",
    "eden mcp catalog lists Eden-approved MCP servers shipped with the repo.",
    "eden mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — eden tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.eden/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "eden chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "eden plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "eden acp runs Eden OE as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "EDEN_OE_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "eden auth add eden or eden auth add openai-codex sets up OAuth-based providers.",
    ...
    "eden status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.eden/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Eden OE your own.",
    ...
    "Before a gateway session reset, Eden OE auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.eden/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Profiles scope Eden OE state via EDEN_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "EDEN_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in EDEN_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.eden/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'eden profile create ls' would be rejected.",
    "eden profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "eden claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "EDEN_OE_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via eden tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.eden/.env.",
    "Skills from trusted repos (ProjectGlacie) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.eden/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Eden OE auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Eden OE config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Eden OE is working.',
    ...
    '/reload reloads ~/.eden/.env into the running session — pick up new API keys without restarting.',
    ...
    "EDEN_OE_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.eden/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.eden/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'eden curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "eden curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'eden curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'eden auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    'credential_pool_strategies.<provider>: round_robin cycles keys evenly instead of the fill_first default.',
    'use_gateway: true per-tool routes web, image, tts, or browser through your Eden subscription — no extra keys.',
    ...
    # --- TUI & Dashboard ---
    'EDEN_OE_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "EDEN_OE_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'eden dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'eden dashboard embeds the full Eden OE TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.eden/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.eden/dashboard-plugins/ — no npm build required.',
    ...
    'EDEN_OE_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'EDEN_OE_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'EDEN_OE_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'EDEN_OE_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.eden/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.eden/skills/ so drop-in skills appear without restarting the session.',
    ...
    'eden -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'eden chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'eden chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'eden chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "eden chat --source tool tags programmatic chats so they don't clutter eden sessions list.",
    'eden dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'eden sessions rename <ID> "new title" renames any past session; eden sessions delete <ID> removes one.',
    'eden import restores a full Eden OE backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'eden fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'eden pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'eden setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'eden status --deep runs the full health sweep across every component; plain eden status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'EDEN_OS_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'EDEN_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.eden/plugins/ — trust-gated by design.',
    "EDEN_OE_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'EDEN_OE_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'EDEN_OE_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'EDEN_OE_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'EDEN_OE_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'EDEN_OE_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'EDEN_OE_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'EDEN_OE_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'EDEN_OS_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'EDEN_OE_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'EDEN_OE_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Eden OE block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from eden sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.eden/dashboard-plugins/.',
]
