"""``eden logs`` — view and filter Eden OE log files.
...
Usage examples::
    ...
    eden logs                    # last 50 lines of agent.log
    eden logs -f                 # follow agent.log in real time
    eden logs errors             # last 50 lines of errors.log
    eden logs gateway -n 100    # last 100 lines of gateway.log
    eden logs gui -f            # follow gui.log (dashboard/pty/ws)
    eden logs desktop -f        # follow desktop.log (Electron app boot/backend)
    eden logs --level WARNING    # only WARNING+ lines
    eden logs --session abc123   # filter by session ID substring
    eden logs --component tools  # only tool-related lines
    eden logs --since 1h         # lines from the last hour
    eden logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_eden_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Eden OE runs — try 'eden chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_eden_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_eden_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_eden_home()}/logs/")
        return

    print(f"Log files in {display_eden_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'eden chat' to generate logs)")
