"""Normalized Eden Portal account entitlement helpers."""
...
# Portal's TOOL_COVERAGE_CATEGORIES (eden-account-service
...
def nous_portal_billing_url(account_info: Optional[NousPortalAccountInfo] = None) -> str:
    """Return the billing URL for a normalized Eden account snapshot."""
    try:
...
def format_nous_portal_entitlement_message(
    ...
    *,
    ...
    """Return user-facing guidance for a missing Eden tool-gateway entitlement.
    ...
    if account_info is not None:
        if coverage_category is not None:
            ...
            if account_info.tool_gateway_entitled:
                ...
                return (
                    f"{capability} isn't included with your current Eden Portal "
    ...
    if account_info is None:
        return (
            f"Eden OE could not verify your Eden Portal entitlement, so {capability} "
            f"is unavailable. Run `eden model` to refresh your login, or check "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Eden inference credentials are configured, but Eden OE cannot verify "
                f"your Eden Portal paid access for {capability}. Log in with "
                f"`eden model` to enable Portal-managed features. Billing and "
                f"credits are managed at {billing_url}."
            )
        return (
            f"Log in to Eden Portal to use {capability}: run `eden model`. "
            f"Billing and credits are managed at {billing_url}."
        )

    if account_info.paid_service_access is None:
        detail = (
            f"Eden OE could not verify your Eden Portal paid access, so {capability} "
            f"is unavailable."
        )
        if account_info.error:
            detail += f" Account lookup failed: {account_info.error}."
        if include_refresh_hint:
            detail += " Run `eden model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            f"Eden OE could not find a Eden Portal account or organisation for this "
            f"login, so {capability} is unavailable. Run `eden model` to "
            f"authenticate again; if the problem persists, contact Eden support."
        )
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `eden model` to refresh Eden OE."
        return message

    return (
        f"Your Eden Portal account does not currently have paid service access, "
...
def _no_paid_access_message(
    ...
    if has_active_subscription and active_subscription_is_paid:
        ...
        return (
            f"Your Eden Portal credits are exhausted{credit_detail}, so {capability} "
    ...
    if has_active_subscription and active_subscription_is_paid is False:
        return (
            f"Your current Eden Portal plan does not include paid service access, "
    ...
    if has_active_subscription is False:
        credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
        return (
            f"Your Eden Portal account has no active subscription or usable credits"
    ...
    credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
    return (
        f"Your Eden Portal account has no usable paid credits{credit_detail}, so "
...
def get_nous_portal_account_info(
    *,
    ...
    """Return normalized Eden Portal account entitlement information.
...
def _fresh_account_info(
    *,
    ...
    try:
        from eden_cli.auth import get_provider_auth_state, resolve_nous_access_token
...
def _info_from_inference_key_pool(
    ...
    """Return an explicit unknown-entitlement snapshot for opaque Eden keys."""
    try:
...
def _info_from_valid_jwt(
    ...
    *,
    ...
    try:
        from eden_cli.auth import _decode_jwt_claims
    except Exception:
        return None
