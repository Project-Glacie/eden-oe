"""
Unified tool configuration for Eden OE.

`eden tools` and `eden setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.eden/config.yaml under
...
from eden_cli.eden_subscription import (
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from eden_cli.eden_account import format_nous_portal_entitlement_message
...
from eden_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _EDEN_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `eden tools` → Video Generation, which walks
...
# `eden tools` → X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from eden_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `eden tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``eden tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from eden_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``eden tools`` checklist actually offers
    for ``platform``.
    ...
    so ``eden tools`` never claims to add or remove a toolset the user was
...
from eden_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "web": {
        ...
        #   - "Eden Subscription" — managed Firecrawl billed via Eden
    ...
    "image_gen": {
        ...
        #   - "Eden Subscription" — managed FAL billed via the Eden
        #     subscription (requires_nous_auth + override_env_vars).
    ...
    "video_gen": {
        ...
        "icon": "🎬",
        # "Eden Subscription" row mirrors the image_gen pattern — managed
        # FAL video generation billed via the Eden Portal.  Plugin-backed
        ...
        "providers": [
            {
                ...
                # "Eden Subscription" we set video_gen.provider = "fal"
    ...
    "x_search": {
        ...
        "setup_note": (
            "Eden OE routes X searches through xAI's built-in x_search "
    ...
    "browser": {
        ...
        # backend, never on the paid Eden Subscription gateway row:
        #   - "Local Browser" — non-cloud option, no CloudBrowserProvider.
        #   - "Eden Subscription (Browser Use cloud)" — managed Browser Use
        #     billed via Eden subscription (requires_nous_auth +
        ...
        "providers": [
            ...
            {
                "name": "Eden Subscription (Browser Use cloud)",
                "badge": "subscription",
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set EDEN_OE_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "EDEN_OE_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "EDEN_OE_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "EDEN_OE_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "EDEN_OE_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "EDEN_OE_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
# provider+model picker like `eden model` — NOT this single-key prompt, so
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("EDEN_OE_CUA_DRIVER_CMD", "").strip() or "cua-driver"


def _cua_driver_env() -> dict:
    """cua-driver child env with the Eden OE telemetry policy applied.
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``eden update`` and
      by ``eden computer-use install --upgrade``.
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `eden update` calls this
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `eden computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
...
def _run_cua_driver_installer(label: str = "Installing", verbose: bool = True) -> bool:
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    try:
        # When not verbose (e.g. `eden update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `eden update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.eden/logs/update.log — write straight to it so
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / Eden OE process.")
            return True
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_eden_home()}/eden-agent && npm install --workspaces=false")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in eden-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/projectglacie/eden-agent:latest"
            )
            return
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.eden/config.yaml")
        ...
        # Run the full `eden auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.eden/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from eden_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: eden auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `eden auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: eden auth spotify")
        except Exception as exc:
        ...
        try:
            from eden_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: eden plugins enable observability/langfuse")
        _print_info("    Restart Eden OE for tracing to take effect.")
        _print_info("    Verify: eden plugins list")
        ...
        try:
            ...
            _print_info("    Run later: eden auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `eden auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: eden auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: eden auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``eden tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``eden tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: eden tools post-setup <key>")
        return 2
...
def _exempt_explicit_platform_native(
    ...
    the composite they chose (e.g. ``eden-discord``, which contains those
...
def _get_platform_tools(
    ...
    *,
    ...
    # ``eden-discord``) is an opt-in to the platform's native default-off
    ...
    # "eden-cli" (which include all _EDEN_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[eden-cli, spotify]`` after enabling Spotify via ``eden
        ...
        # (e.g. "eden-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``eden tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `eden tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"eden-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("eden-")}
    ...
    # shipped as a bundled plugin but user must opt in via `eden tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `eden tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by eden tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # is invalid (e.g. a migration or hand-edit left `eden` instead of
    # `eden-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    # at runtime, not only during the next `eden update`/`eden doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `eden tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (eden-cli, eden-telegram, etc.)
    ...
    # Opening `eden tools` is the user's opt-in to reconfigure tools, so treat
...
# ("Eden Subscription" managed-gateway entry, "Firecrawl Self-Hosted")
...
# ("Eden Subscription", "Local Browser", "Camofox") — see the comment block
...
def _visible_providers(
    ...
    *,
    ...
    Eden-managed Tool Gateway rows (``managed_nous_feature``) are always
    shown — even to logged-out / unentitled users — so the picker advertises
    that the capability exists.  Selecting one drives an inline Eden Portal
    ...
    *activates* the gateway once paid access is confirmed.
    ...
    for provider in cat.get("providers", []):
        # Eden-managed Tool Gateway rows stay visible regardless of auth —
    ...
    # later) so the picker lists them alongside FAL / Eden Subscription.
    ...
    # remaining hardcoded rows ("Eden Subscription", "Firecrawl
    ...
    # the hardcoded "Eden Subscription" / "Local Browser" / "Camofox" rows
...
def _hidden_nous_gateway_message(
    ...
    *,
    ...
    """Deprecated: Eden Tool Gateway rows are no longer hidden.

    Previously this returned a "log in / upgrade" banner shown above a
    category when its Eden-managed rows were filtered out for unentitled
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `eden tools` gets a silent no-op
...
def _configure_tool_category(
    ...
    *,
    ...
    hidden_nous_message = _hidden_nous_gateway_message(
        cat,
        config,
        f"the Eden Subscription provider for {name}",
    ...
    if len(providers) == 1:
        ...
        # When the user is logged into Eden, surface a marker on providers
        # whose access is included in their subscription so it's visually
        # obvious which options cost extra vs. cost nothing on top of Eden.
        try:
        ...
        for p in providers:
            ...
            # Mark Eden-managed entries. Logged-in paid subscribers get the
            # "included" star; everyone else gets a "via Eden Portal" hint so
            ...
            if p.get("managed_nous_feature"):
                if _nous_logged_in:
                    sub_marker = "  ★ Included with your Eden subscription"
                else:
                    sub_marker = "  ★ via Eden Portal (login on select)"
...
def _write_provider_config(provider: dict, config: dict, *, managed_feature) -> None:
    ...
    NOT prompt for env vars, run post-setup hooks, gate on Eden auth, or run
...
def apply_provider_selection(ts_key: str, provider_name: str, config: dict) -> None:
    ...
    post-setup hooks, gate on Eden Portal auth, or run interactive model
...
def _configure_provider(
    ...
    *,
    ...
    # Eden-managed Tool Gateway backends are always listed (see
    # _visible_providers), but only *activate* once the user has paid Eden
    ...
    # "enable all tools" prompt (that lives in `eden model`).
    if managed_feature:
        from eden_cli.eden_subscription import (
        ...
        if not ensure_nous_portal_access(
            capability=f"{provider.get('name', 'the Eden Tool Gateway')}",
            coverage_category=MANAGED_FEATURE_COVERAGE_CATEGORY.get(managed_feature),
        ):
            _print_warning(
                "  Not enabled — Eden Portal access is required for this backend."
            )
            return
    ...
    if provider.get("requires_nous_auth") and not managed_feature:
        ...
        if not features.nous_auth_present or not entitled:
            message = format_nous_portal_entitlement_message(
                ...
                capability=f"{provider.get('name', 'Eden Subscription')}",
            )
            _print_warning(
                f"  {message or 'Eden Subscription is only available after logging into Eden Portal.'}"
            )
            return
    ...
    if not env_vars:
        ...
        if managed_feature:
            _print_info("  Requests for this tool will be billed to your Eden subscription.")
    ...
    # Eden-managed sibling, show a single dim hint so users know
    # they can avoid the key entirely via a Portal subscription.
    # Suppressed when the user is already authed to Eden.
    ...
    if _show_portal_hint:
        _print_info("  Available through Eden Portal subscription.")
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``eden model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    if not providers:
        _print_warning(
            ...
            "with `eden model`, then re-run this."
        )
        return
...
def _reconfigure_provider(
    ...
    *,
    ...
    # Same inline Eden Portal login + entitlement gate as _configure_provider:
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `eden model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `eden tools` and `eden setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("❤️ Eden OE Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://eden-agent.projectglacie.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    if first_install:
        for pkey in enabled_platforms:
            ...
            for ts_key in sorted(auto_configured):
                ...
                print(color(f"  ✓ {label}: using your Eden subscription defaults", Colors.GREEN))
    ...
    print()
    from eden_constants import display_eden_home
    print(color(f"  Tool configuration saved to {display_eden_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'eden' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, eden mcp
