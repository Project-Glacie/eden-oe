"""`eden checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.eden/checkpoints/``.  Actions:

    eden checkpoints               # same as `status`
    eden checkpoints status        # total size, project count, breakdown
    eden checkpoints list          # per-project checkpoint counts + workdir
    eden checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    eden checkpoints clear [-f]    # nuke the entire base (asks first)
    eden checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    eden checkpoints
    eden checkpoints prune --retention-days 3 --max-size-mb 200
    eden checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: eden checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``eden checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `eden checkpoints` → status
