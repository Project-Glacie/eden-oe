"""
Configuration management for Eden OE.

Config files are stored in ~/.eden/ for easy access:
- ~/.eden/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.eden/.env         - API keys and secrets

This module provides:
- eden config          - Show current configuration
- eden config edit     - Open config in editor
- eden config set      - Set a specific value
- eden config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``eden config set`` (which rewrites
    ...
    ``config.yaml`` in place: eden never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.eden/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``eden logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  eden config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Eden OE makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Eden OE itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``eden update``, the TUI build.
...
#   on every plugin install / ``eden update``.
...
# * ``EDEN_HOME`` / ``EDEN_OE_PROFILE`` / ``EDEN_OE_CONFIG`` /
#   ``EDEN_OE_ENV`` — Eden OE runtime location flags. Writing these into
...
#
# IMPORTANT: ``EDEN_OE_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (EDEN_OE_LANGFUSE_PUBLIC_KEY,
# EDEN_OE_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Eden OE runtime location — never via dashboard env writer.
    # NOT a EDEN_OE_* blanket: integration credentials (EDEN_OE_GEMINI_*,
    # EDEN_OE_LANGFUSE_*, EDEN_OE_SPOTIFY_*, ...) ARE allowed.
    "EDEN_HOME", "EDEN_OE_PROFILE", "EDEN_OE_CONFIG", "EDEN_OE_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Eden OE runtime location "
            "(EDEN_HOME, EDEN_OE_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "EDEN_OE_TOOL_PROGRESS", "EDEN_OE_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `eden plugins enable
    # observability/langfuse` or `eden tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "EDEN_OE_LANGFUSE_ENV",
    "EDEN_OE_LANGFUSE_RELEASE",
    "EDEN_OE_LANGFUSE_SAMPLE_RATE",
    "EDEN_OE_LANGFUSE_MAX_CHARS",
    "EDEN_OE_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from eden_cli.colors import Colors, color
from eden_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("EDEN_OE_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_eden_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Eden OE is running in package-manager-managed mode.

    Two signals: the EDEN_OE_MANAGED env var (set by the systemd service),
    or a .managed marker file in EDEN_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade eden-agent"
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``eden_cli/`` — i.e. the git checkout for source
    installs, ``/opt/eden`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$EDEN_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Eden OE was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$EDEN_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. EDEN_OE_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.eden``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$EDEN_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.eden:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$EDEN_HOME``, a
    ...
    ``docker`` and ``eden update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``projectglacie/eden-agent`` image bakes a ``docker``
        stamp into ``/opt/eden`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $EDEN_HOME.
    try:
    ...
    #    host install whose shared $EDEN_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``eden update``.
    try:
        method = (
            (get_eden_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``eden_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$EDEN_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/eden`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Eden OE lives in a ``uv tool`` layout.

    ``uv tool install eden-agent`` places the install at
    ``.../uv/tools/eden-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``eden-agent`` happens to be uv-tool-installed on the machine while
    the *active* Eden OE is a regular pip/venv install, causing
    ``eden update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/eden-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade eden-agent"
    if method == "docker":
        return "docker pull projectglacie/eden-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade eden-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade eden-agent"
        return "pip install --upgrade eden-agent"
    return "eden update"
...
# banner, the TUI/desktop session info panel, and ``eden update``. NixOS
# stays fully supported (Tier 2) and must never hit this path.

PLATFORM_SUPPORT_DOCS_URL = "https://eden-agent.projectglacie.com/docs/getting-started/platform-support"
...
def format_unsupported_install_warning(method: str) -> str:
    ...
    ``eden update`` / ``eden update --check`` so the wording — and the
...
# Long-form text for ``eden update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# eden_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Eden OE, it doesn't update
...
✗ ``eden update`` doesn't apply inside the Docker container.

Eden OE runs as a published image (projectglacie/eden-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull projectglacie/eden-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate eden-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm projectglacie/eden-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/projectglacie/eden-agent/tags
  • Your config and session history live under ``$EDEN_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``eden update`` inside Docker.
...
def format_managed_message(action: str = "modify this Eden OE installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("EDEN_OE_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Eden OE installation is managed by NixOS "
            f"(EDEN_OE_MANAGED={env_hint}).\n"
            "Edit services.eden-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Eden OE installation is managed by Homebrew "
            f"(EDEN_OE_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade eden-agent"
        )

    return (
        f"Cannot {action}: this Eden OE installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Eden OE."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from EDEN_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, eden_bin
    or None if container mode is not active, we're already inside the
    container, or EDEN_OE_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("EDEN_OE_DEV") == "1":
        return None
    ...
    container_mode_file = get_eden_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "eden-agent")
    exec_user = info.get("exec_user", "eden")
    eden_bin = info.get("eden_bin", "/data/current-package/bin/eden")
...
# Re-export from eden_constants — canonical definition lives there.
from eden_constants import get_eden_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_eden_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_eden_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_eden_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the EDEN_OE_UID / EDEN_OE_GID env vars set by Docker deployments.

    Docker containers running Eden OE commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level EDEN_HOME once, but
    subdirectories created at runtime by ``ensure_eden_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("EDEN_OE_UID", "").strip()
    gid_str = os.environ.get("EDEN_OE_GID", "").strip()
    try:
...
def _chown_to_eden_uid(path) -> None:
    """Chown ``path`` to ``EDEN_OE_UID:EDEN_OE_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_eden_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_eden_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the eden group can
    share state with the gateway service.

    The mode can be overridden via the EDEN_HOME_MODE environment variable
    (e.g. EDEN_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse EDEN_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``EDEN_OE_UID``/``EDEN_OE_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("EDEN_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_eden_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Eden OE runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("EDEN_OE_CONTAINER") or os.environ.get("EDEN_OE_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set EDEN_OE_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into EDEN_HOME, upgrading legacy empty templates.
...
def ensure_eden_home():
    """Ensure ~/.eden directory structure exists with secure permissions.
    ...
    """
    home = get_eden_home()
    # Named profiles must be created explicitly (e.g. ``eden profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old EDEN_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_eden_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_eden_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"EDEN_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Eden-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Eden OE
        ...
        # identity slot (SOUL.md). Empty by default. The EDEN_OE_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Eden OE adds a coding operating brief
    ...
    "terminal": {
        ...
        #             EDEN_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force EDEN_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Eden OE auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Eden OE sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Eden OE
        ...
        #  "/home/user/.eden/cache/documents:/output"]
        ...
        # bundled Eden OE image, which drops to the `eden` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Eden OE sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Eden OE should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``eden chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.eden/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: eden sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .eden.md, or .cursorrules before Eden OE applies
    ...
    # single read_file page exceeds these limits, Eden OE truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Eden OE' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); eden =
                                      # Eden OE' compression threshold triggers
    ...
    # See tools/kanban_tools.py and eden_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``eden dashboard`` -> Notifications).
    "kanban": {
    ...
    "bedrock": {
        ...
        "guardrail": {
            ...
            "stream_processing_mode": "async",  # default: streaming ON — "sync" or "async"
    ...
    "auxiliary": {
        ...
        "transient_retries": 2,
        "vision": {
            "provider": "auto",    # auto | openrouter | eden | codex | custom
        ...
        # Invoked by ``eden kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``eden kanban decompose`` and the kanban
        ...
        # ``eden profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `eden model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `eden` (and `eden chat`) launches by default:
        ...
        # REPL and `--tui` (or EDEN_OE_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `eden --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `eden -c` muscle memory.  Default off so existing
        # users aren't surprised.  EDEN_OE_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `eden --tui` drops a one-time hint
        ...
        # Eden credits status-bar notices (usage bands, grant-spent, depleted /
        ...
        # can set `path`/`paths` to EDEN_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/eden`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `eden pets`. Disabled until
        ...
        "pet": {
            ...
            # get_eden_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``--insecure`` is not). The bundled Eden Portal plugin reads
        # both keys at startup; they are the canonical surface for these
        # settings. Each can be overridden by an environment variable —
        # ``EDEN_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``EDEN_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # ``portal_url`` defaults to https://portal.projectglacie.com).
        "oauth": {
        ...
        # (``EDEN_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # eden-account-service at deploy time via the
        # ``EDEN_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``EDEN_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/eden``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Eden OE appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.eden/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.eden/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # configured providers (OpenRouter, Eden, Z.ai, Kimi, etc.) are supported.
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.eden/cache/delegation/
    ...
    # response. If not, Eden OE feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Eden OE auto-pauses the goal and
    ...
    "moa": {
        ...
        # <eden_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.eden/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${EDEN_OE_SKILL_DIR} and ${EDEN_OE_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `eden curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `eden curator run --consolidate` overrides
        ...
        # normally restored on every `eden update`, so pruning them only
        ...
        # skipped), snapshot ~/.eden/skills/ into
        # ~/.eden/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `eden curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for eden-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "❤️ *Eden OE*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (EDEN_OE_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Eden OE' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.eden/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or EDEN_OE_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `eden doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``eden_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Eden OE to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $EDEN_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # agent's existing Eden Portal token — there is no token key here.
        "chronos": {
            ...
            "portal_url": "https://portal.projectglacie.com",
        ...
        # Also overridable via EDEN_OE_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `eden -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `eden` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `eden kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     eden-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Eden OE tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.eden/logs/.
    ...
    # curated model lists for OpenRouter and Eden Portal from this URL,
    # falling back to the in-repo snapshot on network failure.  Lets us
    # update model picker lists without shipping a eden-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://eden-agent.projectglacie.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `eden model` invocation; network failures
    ...
    "gateway": {
        ...
        # internal EDEN_OE_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.eden/sessions/sessions.json entirely.
        ...
        # the EDEN_OE_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.eden.gateway` that defenses 1-2 don't
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.eden/.env,
        ...
        # behavior: files must live under the Eden OE cache, under
        ...
        # user. Bridged to EDEN_OE_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Eden OE
        # cache (~/.eden/cache/{documents,images,audio,video,screenshots})
        ...
        # to EDEN_OE_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # EDEN_OE_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to EDEN_OE_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.eden/state.db.
    ...
    "sessions": {
        ...
        # default of ``eden sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.eden/sessions/session_{sid}.json`` on every turn
    ...
    # ``eden update`` behaviour.
    "updates": {
        # Run a full ``eden backup``-style zip of EDEN_HOME before every
        # ``eden update``.  Backups land in ``<EDEN_HOME>/backups/`` and
        # can be restored with ``eden import <path>``.  Off by default:
        # zipping a large EDEN_HOME (sessions DB, caches, skills) can add
        # minutes to every update.  The #48200 incident — a ``eden update
        ...
        "backup_keep": 5,
        # What `eden update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<EDEN_HOME>/lsp/bin/`` on first use.
    ...
    # `eden tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.eden/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.eden/.env (or your shell) and never in config.yaml.
            ...
            # ~/.eden/bin/ on first use.  When False you must install
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.eden/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Eden OE disables it for our users unless they
        # explicitly opt in here. When false (default), Eden OE sets
    ...
    # Eden OE Desktop (Electron app) launch options. These only affect
    # `eden desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the EDEN_OE_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
    },
...
# selection step (Eden Portal / OpenRouter / Custom endpoint), so this
...
OPTIONAL_ENV_VARS = {
    ...
    "NOUS_BASE_URL": {
        "description": "Eden Portal base URL override",
        "prompt": "Eden Portal base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Eden OE mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'eden model' for endpoint-specific config)",
    ...
    "FIRECRAWL_GATEWAY_URL": {
        "description": "Exact Firecrawl tool-gateway origin override for Eden Subscribers only (optional)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Eden Subscribers only, used to derive vendor hosts, e.g. projectglacie.com -> firecrawl-gateway.projectglacie.com",
    ...
    "TOOL_GATEWAY_SCHEME": {
        "description": "Shared tool-gateway URL scheme for Eden Subscribers only, used to derive vendor hosts (`https` by default, set `http` for local gateway testing)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Eden Subscriber access token for tool-gateway requests (optional; otherwise read from the Eden OE auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Eden OE, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @eden:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. EDEN_OE_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #eden)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: eden-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'eden-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Eden OE API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Eden OE API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Eden OE API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # EDEN_OE_TOOL_PROGRESS and EDEN_OE_TOOL_PROGRESS_MODE are deprecated —
    ...
    "EDEN_OE_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.eden.config`` entries, then checks
    ...
    try:
        ...
        # should never break `eden update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Eden OE' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Eden OE versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``eden_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'eden setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Eden OE won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'eden doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("EDEN_HOME", "~/.eden")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "eden update / eden -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("EDEN_OE_TOOL_PROGRESS")
            old_mode = get_env_value("EDEN_OE_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from EDEN_OE_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all", "verbose"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from EDEN_OE_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("EDEN_OE_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from EDEN_OE_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$EDEN_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_eden_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `eden plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `eden curator status` has no
    ...
    #   3. Creates `~/.eden/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_eden_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_eden_home() / "logs" / "curator"
            ...
            if added_curator:
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_curator)}) — edit via `eden config set`"
                    )
            if added_aux:
                results["config_added"].append(
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_aux)}) — edit via `eden config set`"
                    )
    ...
    # behavior set it to true explicitly via `eden config set`.
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from eden_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        ...
        from eden_cli.toolset_validation import validate_platform_toolsets
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: eden config set <key> <value>")
    ...
    # `eden update`, but only the version bump is persisted.
    ...
    # metadata.eden.config in their SKILL.md frontmatter.
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_eden_dotenv()`` populated the process env, or an env var
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``eden config set``
    ...
    backends) — while display paths (``eden status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _eden_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.eden/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.eden/config.yaml.
    ...
    (which change ``EDEN_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/eden/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_eden_dotenv()
        ...
        # ── Phase 2b: Load config from EdenDB (primary, YAML overrides) ──────
        try:
            from eden.db import EdenDB

            _db = EdenDB()
            _rows = _db.query(
                "SELECT section, key, value FROM system_config"
            )
            if _rows:
                ...
                for _r in _rows:
                    ...
                    _key = _r["key"]
                    _val: Any = _r["value"]
                    # Try JSON parse for structured values (lists, dicts,
                    # numbers); keep raw string on failure.
                    if isinstance(_val, str):
                        try:
                            _parsed = json.loads(_val)
                            if isinstance(_parsed, (dict, list, bool, int, float)):
                                _val = _parsed
                        except (json.JSONDecodeError, TypeError):
                            pass
                    if _sec not in _db_cfg:
                        _db_cfg[_sec] = {}
                    _db_cfg[_sec][_key] = _val
                config = _deep_merge(config, _db_cfg)
        except Exception:
            pass
        # ── End Phase 2b ─────────────────────────────────────────────────────
...
#   openai-codex (OAuth — eden auth) — OpenAI Codex
#   eden         (OAuth — eden auth) — Eden Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.eden/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Eden OE writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.eden/.env.
    ...
    interactive menu render (`eden tools`, `eden setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `eden tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Eden OE env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.eden/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.eden/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.eden/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.eden/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Eden OE — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Eden OE vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.eden/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.eden/.env`` over ``os.environ``.

    Used for Eden-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              Eden OE Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from eden_cli.auth import get_anthropic_key
    ...
    # Warn on stale EDEN_OE_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("EDEN_OE_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale EDEN_OE_MAX_ITERATIONS={_env_ghost} "
                f"(run 'eden doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  eden config edit     # Edit config file", Colors.DIM))
    print(color("  eden config set <key> <value>", Colors.DIM))
    print(color("  eden setup           # Run setup wizard", Colors.DIM))
    print()
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `eden config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `eden config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: eden config set <key> <value>")
            print()
            print("Examples:")
            print("  eden config set model anthropic/claude-sonnet-4")
            print("  eden config set terminal.backend docker")
            print("  eden config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'eden config migrate' to add them")
        ...
        print("  eden config           Show current configuration")
        print("  eden config edit      Open config in editor")
        print("  eden config set <key> <value>   Set a config value")
        print("  eden config check     Check for missing/outdated config")
        print("  eden config migrate   Update config with new options")
        print("  eden config path      Show config file path")
        print("  eden config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``eden config`` UI so users
