"""``eden mcp`` subcommand parser.

Extracted from ``eden_cli/main.py:main()`` (god-file Phase 2 follow-up).
...
def build_mcp_parser(subparsers, *, cmd_mcp: Callable) -> None:
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Eden OE as an MCP server",
        description=(
            "Manage MCP server connections and run Eden OE as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'eden mcp add' to connect to a new server, or\n"
            "'eden mcp serve' to expose Eden OE conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Eden OE as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `eden mcp add ...` to fall through to
    # interactive chat.
    ...
    # ── Catalog (Eden-approved MCPs shipped with the repo) ─────────────────
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `eden mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
        help="List Eden-approved MCPs available for one-click install",
    )
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `eden mcp install n8n`)",
    )
