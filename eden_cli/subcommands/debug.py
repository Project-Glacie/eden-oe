...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Eden OE. Use 'eden debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    eden debug share              Upload debug report (asks for confirmation)
    eden debug share --yes        Skip confirmation (for scripts/CI)
    eden debug share --lines 500  Include more log lines
    eden debug share --expire 30  Keep paste for 30 days
    eden debug share --local      Print report locally (no upload)
    eden debug share --no-redact  Disable upload-time secret redaction
    eden debug share --eden       Upload to Eden-internal storage (private)
    eden debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    share_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Upload the debug bundle to Eden-internal storage (AWS S3) instead "
            "of a public paste service. The bundle is private — viewable only "
            "by Eden staff (and allowlisted Discord mods) via a Google-login-"
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'eden debug share'",
    )
