...
# xAI's public API is OpenAI-compatible for the endpoints Eden OE commonly
# uses. The Responses endpoint is included because Eden OE' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Eden-managed OAuth credentials."""

    auth_hint = "eden auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`eden auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`eden auth reset xai-oauth` or re-authenticate with "
                    "`eden auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `eden auth add xai-oauth --type oauth`."
            )
