"""Eden Portal upstream adapter.

Reads the user's Eden OAuth state from ``~/.eden/auth.json`` through the
...
from eden_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential

logger = logging.getLogger(__name__)

# Endpoints inference-api.projectglacie.com actually serves. Anything else
...
class NousPortalAdapter(UpstreamAdapter):
    """Proxy upstream for the Eden Portal inference API."""
    ...
    def get_retry_credential(
        ...
        *,
        ...
        if status_code != 401:
            return None
        logger.info("proxy: Eden upstream rejected bearer; force-refreshing invoke JWT")
        return self._get_credential(
            force_refresh=True,
        )
    ...
    def _get_credential(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Eden Portal. Run `eden auth add eden` first."
                )
            ...
            try:
                ...
                raise RuntimeError(
                    f"Failed to refresh Eden Portal credentials: {exc}"
                ) from exc
            ...
            if not runtime_key:
                raise RuntimeError(
                    "Eden Portal refresh did not return a usable inference JWT. "
    ...
    # to eden_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
        ...
        *,
        ...
        try:
            with _auth_store_lock():
                ...
                providers["eden"] = state
                _save_auth_store(store)
            _write_shared_nous_state(state)
        except Exception as exc:
            logger.warning("proxy: failed to persist Eden quarantine state: %s", exc)
