...
``$EDEN_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-eden-setup
(the stage2 hook) has chowned the volume and seeded $EDEN_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$EDEN_HOME``,
    not under ``profiles/``). The dispatcher in ``eden_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``eden gateway start`` (no ``-p``) targets. Without it,
    bare ``eden gateway start`` inside the container would land on
    ...
    ``$EDEN_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        eden_home: The container's EDEN_HOME (typically /opt/data).
            Profiles live under ``<eden_home>/profiles/<name>/``;
            the default profile lives at ``<eden_home>`` itself.
    ...
    # ``eden gateway start`` (no ``-p``) has somewhere to land;
    ...
    default_prior_state = legacy_default_state or _read_desired_state(eden_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(eden_home)
    ...
    profiles_root = eden_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `eden profile create` (config.yaml
            # is not — that comes later via `eden setup`). Use it as the
            ...
            # via ``eden -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(eden_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = eden_home / "gateway_state.json"
    if state_file.exists():
        return None

    if os.environ.get("EDEN_OE_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off the container argv, leaving the eden args.

    Two container-command argv shapes are handled:

    * **s6-overlay v2 / tini:** PID 1 argv is
      ``/init /opt/eden/docker/main-wrapper.sh <subcommand> [args...]``.
    * **s6-overlay v3:** PID 1 is ``s6-svscan`` and the command lives on the
      rc.init-launched process as ``/bin/sh -e
      /run/s6/basedir/scripts/rc.init top /opt/eden/docker/main-wrapper.sh
    ...
    ``eden`` invocations carry no wrapper, so fall back to peeling a bare
    ``init`` prefix. The wrapper re-execs ``eden <subcommand>``, so an
    explicit leading ``eden`` is peeled too. Shared by the legacy-gateway
    ...
    # The wrapper re-execs `eden <subcommand>`; peel an explicit eden.
    if args and Path(args[0]).name == "eden":
        args = args[1:]
    return args
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``eden dashboard ...``) never spawns or
    ...
    the gateway and dashboard containers share a bind-mounted EDEN_HOME,
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `eden -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit eden
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the eden user) won't hit EACCES. See
...
def _write_reconcile_log(
    eden_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $EDEN_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    # bind-mounted EDEN_HOME, both race to flock() the same s6-log lock
    ...
    actions = reconcile_profile_gateways(
        eden_home=eden_home, scandir=scandir,
    )
    for a in actions:
        print(
