"""``eden gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
``docs/connector-gateway-auth-design.md``:
  ...
  1. Resolve a fresh Eden Portal access token from the existing login
     (``~/.eden/auth.json``) — the same path ``eden dashboard register``
     uses (``resolve_nous_access_token``). This proves *which Eden org (tenant)*
  ...
  4. Persist ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` /
     ...
     ``~/.eden/.env``. The per-gateway secret authenticates the WS upgrade;
...
def _resolve_identity_token() -> str:
    """Resolve the caller-identity bearer token (generic-OIDC or Eden Portal).
    ...
    self-hosted-IdP path; otherwise Eden Portal). Raises RuntimeError on failure.
    """
...
def _post_enroll(
    *,
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                "Connector rejected the caller identity (401). Your Eden Portal "
                "token could not be verified — try `eden auth add eden` and retry."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def cmd_gateway_enroll(args) -> None:
    ...
    if is_managed():
        print(
            "✗ `eden gateway enroll` is not available in a managed/hosted install.\n"
    ...
    #    gapped / self-hosted-IdP, NO Eden Portal); otherwise the Eden Portal token.
    try:
    ...
    #    save_env_value writes them to ~/.eden/.env (0600 dir) and never logs.
    to_write = {
