"""
Eden OE — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m eden_cli.main web          # Start on http://127.0.0.1:9119
    python -m eden_cli.main web --port 8080
"""
...
from eden_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
try:
    ...
    # running `eden dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["EDEN_OE_WEB_DIST"]) if "EDEN_OE_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``eden gateway run`` — but the
    desktop app spawns a ``eden dashboard`` backend, not a gateway, so a cron
    ...
    real gateway on the same EDEN_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
def _warm_gateway_module() -> None:
    try:
        import eden_cli.gateway  # noqa: F401
    except Exception:
        pass
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from eden_cli.gateway import _get_restart_drain_timeout
...
async def _lifespan(app: "FastAPI"):
    ...
    # Fire eden_cli.gateway import into a background thread so the event
    # loop is not blocked and EDEN_DASHBOARD_READY fires without delay.
    ...
    # Desktop-spawned backends (EDEN_OE_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `eden
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("EDEN_OE_DESKTOP") == "1":
...
app = FastAPI(title="Eden OE", version=__version__, lifespan=_lifespan)

# Memory-provider OAuth connect routes live in the memory layer, not here.
from eden_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
# EDEN_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("EDEN_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
_SESSION_HEADER_NAME = "X-Eden-Session-Token"
...
#
# This list is defined in ``eden_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from eden_cli.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Eden-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    unauthenticated-public-dashboard hole behind the June 2026 ``eden-0day``
    ...
    if path.startswith("/api/plugins/"):
        ...
        if _authed:
            ...
            if len(parts) >= 4:
                ...
                if plugin_name:
                    try:
                        from eden_cli.plugins_cmd import (
                            _get_enabled_set,
                            _get_disabled_set,
                        )
...
async def _dashboard_auth_gate(request: Request, call_next):
    from eden_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from eden_cli.dashboard_auth.token_auth import token_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Eden OE (no terminal prompt), "
    ...
    "updates.refresh_cua_driver": {
        "type": "bool",
        "description": (
            "Refresh an already-installed cua-driver during eden update. "
...
class ModelAssignment(BaseModel):
    ...
    # ``eden model`` custom flow collects. Honored only on the main slot for
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Eden OE provider slug — fine.
    ...
    1. Vendor-name → Eden-provider mapping: when the provider string is
       not a known Eden OE provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    """
    from eden_cli.config import get_compatible_custom_providers
    from eden_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from eden_cli.model_normalize import normalize_model_for_provider
    from eden_cli.providers import resolve_custom_provider, resolve_user_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from eden_cli.models import _AGGREGATOR_PROVIDERS
...
def _count_status_active_sessions() -> int:
    ...
    while another Eden OE process is writing to it.
    """
    from eden_state import DEFAULT_DB_PATH, SessionDB
...
_MANAGED_FILES_ROOT_ENV = "EDEN_DASHBOARD_FILES_ROOT"
...
# EDEN_HOME itself, at which point every one of these basenames is a live
...
# without needing to resolve them relative to EDEN_HOME.
...
def _is_sensitive_filename(name: str) -> bool:
    ...
    canonical Eden OE credential-store basenames (see
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no EDEN_HOME resolution, so it blocks these trees wherever
...
def _default_eden_root_is_opt_data() -> bool:
    raw = os.environ.get("EDEN_HOME", "").strip()
    if not raw:
        return False
    try:
        from eden_constants import get_default_eden_root

        root = get_default_eden_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``eden update``.
    ...
    a container — e.g. the eden-webui image sharing the Eden OE source tree),
    the dashboard's ``eden update`` button is the correct update path and
    ...
    """
    if _default_eden_root_is_opt_data():
        return True
    try:
        from eden_constants import is_container
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Eden OE root is actually /opt/data
    # (the container/hosted layout) or when EDEN_DASHBOARD_FILES_ROOT is set.
    if _default_eden_root_is_opt_data():
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    The dashboard /chat page runs Eden OE inside an xterm.js PTY. Browser
...
async def fs_write_text(payload: FsWriteText):
    ...
    Mirrors the local Electron ``eden:fs:writeText`` hardening: the path is
    ...
    tmp = target.with_name(f".{target.name}.eden-tmp-{os.getpid()}")
    try:
...
# gate + path hardening as /api/fs). Logic lives in ``eden_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from eden_cli import web_git as _web_git  # noqa: E402
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``gateway_mode`` — ``"multiplex"`` when the default gateway serves
    ...
    """
    try:
        from eden_cli.profiles import _check_gateway_running, profiles_to_serve
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_eden_home() at call time
    ...
    try:
        ...
        # Windows install the first import of eden_cli.gateway blocks the
        ...
        # and which providers are registered so ``eden status`` and the
        # SPA's StatusPage can show "OAuth gate ON via Project Glacie" or
        # "loopback only — no auth gate" with no extra round trips.
        auth_required = bool(getattr(app.state, "auth_required", False))
        auth_providers: list[str] = []
        try:
            from eden_cli.dashboard_auth import list_providers as _list_providers
            auth_providers = [p.name for p in _list_providers()]
        except Exception:
            # Module not importable yet (early startup) — leave as [].
            pass

        # Eden bootstrap-session validity for the NAS health sweep. A hosted
        # agent whose Eden auth dies terminally (invalid_grant / quarantine)
        ...
        try:
            from eden_cli.auth import get_nous_session_validity
        ...
        status = {
            ...
            "can_update_eden": not _dashboard_local_update_managed_externally(),
            "gateway_running": gateway_running,
            "gateway_state": gateway_state,
        ...
        # ``gateway_mode`` are low-sensitivity PRODUCT surface — Eden OE Cloud
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the eden home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_eden_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_eden_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
# ---------------------------------------------------------------------------
# Portal endpoint — Eden Portal auth + Tool Gateway routing status (read-only).
# ---------------------------------------------------------------------------
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from eden_cli.auth import get_nous_auth_status
    ...
    return {
        ...
        "subscription_url": "https://portal.projectglacie.com/manage-subscription",
        "features": features,
    }
...
async def run_prompt_size():
    try:
        proc = _spawn_eden_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_eden_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_eden_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from eden_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.eden/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_eden_home() / "logs"
...
def _spawn_eden_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``eden <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``eden_cli.main`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "eden_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "EDEN_OE_NONINTERACTIVE": "1"},
    }
    if sys.platform == "win32":
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["eden", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``eden gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``eden gateway restart`` children race each other on the
    ...
    return _spawn_eden_action(subcommand, "gateway-restart"), False
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``eden gateway restart`` in the background."""
    try:
...
async def gateway_drain(request: Request):
    ...
    active (no ``EDEN_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
...
@app.post("/api/eden/update")
async def update_eden():
    """Kick off ``eden update`` in the background."""
    if _dashboard_local_update_managed_externally():
        message = (
            "Eden OE updates are managed outside this dashboard in "
            "containerized environments. The built-in local updater is "
            "disabled here."
        )
        _record_completed_action("eden-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "eden-update",
    ...
    try:
        proc = _spawn_eden_action(["update"], "eden-update")
    except Exception as exc:
        _log.exception("Failed to spawn eden update")
...
@app.get("/api/eden/update/check")
async def check_eden_update(force: bool = False):
    """Report whether a Eden OE update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/eden/update`` actually runs ``eden update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed Eden OE version string
    ...
    try:
        ...
        if force:
            try:
                (get_eden_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.eden/config.yaml`` under ``tts.``.
    """
...
def get_profiles_sessions(
    ...
    from eden_cli import profiles as profiles_mod
...
async def search_sessions(q: str = "", limit: int = 20, profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            ...
            # logs, or another Eden OE surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Eden OE supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _read_memory_provider_existing_values(name: str) -> Dict[str, Any]:
    ...
    for path in (
        eden_home / f"{name}.json",
        eden_home / name / "config.json",
    ):
...
def _save_memory_provider_native_config(name: str, provider: Any, values: Dict[str, Any]) -> None:
    if provider is not None and hasattr(provider, "save_config"):
        try:
            ...
            provider.save_config(values, str(get_eden_home()))
            return
...
# in eden_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_model_options(
    ...
    """
    try:
        from eden_cli.inventory import build_models_payload, load_picker_context
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `eden model` does so GUI onboarding lands on a
    sensible default instead of blindly taking the first curated entry. For
    Eden this honors the user's free/paid tier: free users get a free model,
    ...
    where free_tier is True/False for Eden and None otherwise. `model` may be
    ...
    if slug == "eden":
        try:
            ...
            from eden_cli.auth import get_provider_auth_state

            model_ids = get_curated_nous_model_ids()
            pricing = get_pricing_for_provider("eden") or {}
            free_tier = check_nous_free_tier(force_fresh=True)

            portal_url = ""
            try:
                state = get_provider_auth_state("eden") or {}
            ...
            return {"provider": "eden", "model": model, "free_tier": bool(free_tier)}
        except Exception:
            _log.exception("GET /api/model/recommended-default (eden) failed")
            return {"provider": "eden", "model": "", "free_tier": None}

    # Non-Eden: first curated model for the provider, matching prior behaviour.
    try:
        from eden_cli.inventory import build_models_payload, load_picker_context
...
def get_moa_models(profile: Optional[str] = None):
    ...
    try:
        from eden_cli.moa_config import normalize_moa_config
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.eden/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if model and not body.confirm_expensive_model:
            try:
                from eden_cli.model_cost_guard import expensive_model_warning
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # When switching the main provider to Eden, mirror the CLI's
        # post-model-selection behaviour (eden_cli/main.py
        # prompt_enable_tool_gateway / tools_config apply_nous_managed_defaults):
        # auto-route any *unconfigured* tools through the Eden Tool Gateway.
        ...
        if provider.strip().lower() == "eden":
            try:
                from eden_cli.eden_subscription import apply_nous_managed_defaults
                from eden_cli.tools_config import _get_platform_tools
                ...
                # Portal lookup hiccups / non-subscriber / non-eden gating
        ...
        # endpoint, mirroring the ``eden model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from eden_cli.main import _auto_provider_name, _save_custom_provider
        ...
        # a now-unpaid provider (e.g. eden with $0 balance) keeps paying 402s
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``eden model`` universe). This is what lets the desktop Keys tab
    ...
    """
    try:
        from eden_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from eden_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `eden model` provider — would be
        ...
        # Vertex — a `eden model` provider — would be invisible in the desktop
...
async def get_env_vars(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # CLI `eden model` picker uses (not desktop-only prefix guesses).
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Eden OE from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Eden OE to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Eden OE from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Eden OE to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Eden OE in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Eden OE through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Eden OE via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Eden OE through an IMAP/SMTP mailbox.",
        "docs_url": "https://eden-agent.projectglacie.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Eden OE to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Eden OE inside Feishu / Lark.",
    ...
    "google_chat": {
        "name": "Google Chat",
        "description": "Connect Eden OE to Google Chat via Cloud Pub/Sub.",
        "docs_url": "https://eden-agent.projectglacie.com/docs/user-guide/messaging/google_chat",
    },
    "wecom": {
    ...
    "weixin": {
        ...
        "docs_url": "https://eden-agent.projectglacie.com/docs/user-guide/messaging/weixin/",
        "env_vars": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN", "WEIXIN_BASE_URL"),
        "required_env": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN"),
    },
    "bluebubbles": {
        "name": "BlueBubbles (iMessage)",
        "description": "Use Eden OE through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Eden OE to a QQ Bot from the QQ Open Platform.",
    ...
    "teams": {
        "docs_url": "https://eden-agent.projectglacie.com/docs/user-guide/messaging/teams",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Eden OE to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Eden OE as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://eden-agent.projectglacie.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://eden-agent.projectglacie.com/docs/user-guide/messaging/webhooks/",
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in eden gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in eden gateway setup",
...
def _whatsapp_session_path() -> Path:
    from eden_constants import get_eden_dir

    return get_eden_dir("platforms/whatsapp/session", "whatsapp/session")
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.eden-agent.projectglacie.com"
_TELEGRAM_ONBOARDING_USER_AGENT = f"EdenDashboard/{__version__}"
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Eden OE").strip() or "Eden OE"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Eden OE appears
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # all resolve against the requested profile's EDEN_HOME.
...
# Anthropic, device-code for Eden/Codex) still runs in the CLI for now;
# Phase 2 will add in-browser flows. For unconnected providers we return
# the canonical ``eden auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.eden/.anthropic_oauth.json`` — Eden-managed PKCE flow (what
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_eden_dotenv()``, so the same check covers them)
    ...
    try:
        from agent.anthropic_adapter import (
            read_eden_oauth_credentials,
            _get_eden_oauth_file,
        )
    except ImportError:
        read_eden_oauth_credentials = None  # type: ignore
        _get_eden_oauth_file = None  # type: ignore

    eden_creds = None
    if read_eden_oauth_credentials:
        try:
            eden_creds = read_eden_oauth_credentials()
        except Exception:
            eden_creds = None
    if eden_creds and eden_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "eden_pkce",
            "source_label": f"Eden OE PKCE ({_get_eden_oauth_file() if _get_eden_oauth_file else None})",
            "token_preview": _truncate_token(eden_creds.get("accessToken")),
            "expires_at": eden_creds.get("expiresAt"),
            "has_refresh_token": bool(eden_creds.get("refreshToken")),
        }
    ...
    try:
        ...
        pass
    try:
        from eden_cli.config import get_env_value
    except ImportError:
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Eden OE even
    when they also have a separate Eden-managed PKCE login.
    """
    try:
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    Eden OE never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Eden Portal",
        "flow": "device_code",
        "cli_command": "eden auth add eden",
        "docs_url": "https://portal.projectglacie.com",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "eden auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "eden auth add qwen-oauth",
    ...
    },
    {
        ...
        # as Eden's device-code flow; the PKCE bit is a security
        # extension that doesn't change the operator experience.
        "flow": "device_code",
        "cli_command": "eden auth add minimax-oauth",
    ...
    },
    {
        ...
        # 127.0.0.1 callback.
        "flow": "device_code",
        "cli_command": "eden auth add xai-oauth",
        "docs_url": "https://eden-agent.projectglacie.com/docs/guides/xai-grok-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "eden auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from eden_cli import auth as hauth
        if provider_id == "eden":
            raw = hauth.get_nous_auth_status()
            return {
                "logged_in": bool(raw.get("logged_in")),
                "source": "nous_portal",
                "source_label": raw.get("portal_base_url") or "Eden Portal",
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Eden OE, so the disconnect
    ...
    user sees exactly what executes, and Eden OE then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Eden OE — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``eden model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `eden model` universe (zero-edit for new plugins).
    try:
        from eden_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"eden auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("eden_pkce", "claude_code", ...)
    ...
    sync with the `eden model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        # Anthropic clears only the Eden-managed PKCE file and auth-store entry.
        ...
        if provider_id == "anthropic":
            cleared = False
            try:
                from agent.anthropic_adapter import _get_eden_oauth_file
                oauth_file = _get_eden_oauth_file()
            ...
            try:
                from eden_cli.auth import clear_provider_auth
        ...
        try:
            from eden_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
#          → persists to ~/.eden/.anthropic_oauth.json AND credential pool
#          → returns { ok: true, status: "approved" }
#
#   Device code (Eden, OpenAI Codex):
#     1. POST /api/providers/oauth/{eden|openai-codex}/start
...
# Guarded so eden web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Eden OE file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``eden auth add anthropic``.
    """
    from agent.anthropic_adapter import _get_eden_oauth_file
    oauth_file = _get_eden_oauth_file()
    payload = {
...
async def _start_device_code_flow(
    ...
    """Initiate a device-code flow (Eden, OpenAI Codex, MiniMax, or xAI).
    ...
    if provider_id == "eden":
        ...
        import httpx
        pconfig = PROVIDER_REGISTRY["eden"]
        portal_base_url = (
            os.getenv("EDEN_OE_PORTAL_BASE_URL")
        ...
        sid, sess = _new_oauth_session("eden", "device_code", profile=profile)
    ...
    if provider_id == "minimax-oauth":
        ...
        # operator's perspective it's identical to Eden's device-code
    ...
    if provider_id == "xai-oauth":
        from eden_cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _nous_poller(session_id: str) -> None:
    """Background poller that drives a Eden device-code flow to completion."""
    from eden_cli.auth import (
        _poll_for_token,
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from eden_cli.auth import persist_nous_credentials
            persist_nous_credentials(full_state)
        with _oauth_sessions_lock:
            sess["status"] = "approved"
        _log.info("oauth/device: eden login completed (session=%s)", session_id)
    except Exception as e:
        _log.warning("eden device-code poll failed (session=%s): %s", session_id, e)
        with _oauth_sessions_lock:
            sess["status"] = "error"
...
def _minimax_poller(session_id: str) -> None:
    ...
    the ``device_code`` field used by Eden. On success, builds the same
    auth_state dict that ``_minimax_oauth_login`` (the CLI flow) builds
    and persists via ``_minimax_save_auth_state`` — so the dashboard
    path leaves the system in the same state as
    ``eden auth add minimax-oauth``.
    """
    from eden_cli.auth import (
        _minimax_poll_token,
...
def _xai_device_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            # prior ``eden auth remove xai-oauth`` (mirrors auth_add_command
            # and the ``eden model`` re-login path in _login_xai_oauth).
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before Eden OE "
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from eden_cli.auth import _save_codex_tokens
...
async def poll_oauth_session(
    ...
    Shared by the device-code flows (Eden, OpenAI Codex, MiniMax, xAI).
...
async def import_sessions_endpoint(request: Request):
    ...
    restores a whole Eden OE backup archive, while this endpoint is scoped to
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `eden sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions matching filters (mirrors `eden sessions prune`)."""
    has_window = (
    ...
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_eden_home()
    db = _open_session_db_for_profile(body.profile)
    try:
        filters = dict(
...
async def get_logs(
    ...
    from eden_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_eden_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from eden_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, EDEN_HOME)."""
    from eden_cli import profiles as profiles_mod
...
def _call_cron_for_profile(target_profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from eden_constants import (
        reset_eden_home_override,
        set_eden_home_override,
    )

    token = set_eden_home_override(str(home))
    try:
        with cron_jobs.use_cron_store(home):
            result = getattr(cron_jobs, func_name)(*args, **kwargs)
    finally:
        reset_eden_home_override(token)
...
def _fire_cron_job_for_profile(profile: str, job_id: str) -> bool:
    ...
    Scope both cron storage and the runtime Eden OE home so the job's store,
...
#
# Wraps the same config data layer the CLI uses (eden_cli.mcp_config), so
# servers managed here show up under `eden mcp list` and vice versa.  Secrets
...
async def list_mcp_servers(profile: Optional[str] = None):
    from eden_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from eden_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def replace_mcp_servers(body: MCPServersReplace, profile: Optional[str] = None):
    ...
    """
    from eden_cli.mcp_config import _replace_mcp_servers
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from eden_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from eden_cli.mcp_config import (
        _get_mcp_servers,
    ...
    def _probe_scoped():
        ...
        # The probe touches no skills globals; it only needs the EDEN_HOME
...
async def auth_mcp_server(name: str, profile: Optional[str] = None):
    ...
    Mirrors ``eden mcp login``: wipe cached OAuth state so the probe forces
    ...
    def _run():
        from tools.mcp_oauth import EdenTokenStorage, force_interactive_oauth
        ...
        # resolve EDEN_HOME via the contextvar override, which is all they need.
        with _config_profile_scope(profile), force_interactive_oauth():
            storage = EdenTokenStorage(name)
...
async def list_mcp_catalog(profile: Optional[str] = None):
    """Browse the Eden-approved MCP catalog (the optional-mcps/ manifests).
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds EDEN_HOME-derived paths in the child.
...
#
# Wraps the same JSON store the CLI uses (eden_cli.webhook); the webhook
...
# spawn the real `eden gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_eden_action(_gateway_subcommand(profile, "start"), "gateway-start")
    except HTTPException:
        raise
    except Exception as exc:
...
async def stop_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_eden_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    except HTTPException:
        raise
    except Exception as exc:
...
async def list_credential_pool():
    ...
    from eden_cli.auth import read_credential_pool
...
async def run_doctor():
    try:
        proc = _spawn_eden_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_eden_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
def _dashboard_backup_dir() -> Path:
    return get_eden_home() / "backups"


def _new_dashboard_backup_path() -> Path:
    stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
    return _dashboard_backup_dir() / f"eden-backup-{stamp}-{secrets.token_hex(4)}.zip"
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_eden_action(args, "backup")
    except Exception as exc:
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `eden import`. The spawned action runs with
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_eden_action(args, "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    form can offer them.
    """
    from eden_cli.config import load_config as _load_config
    from agent import shell_hooks

    try:
        from eden_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <eden_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_eden_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_eden_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``eden`` subprocess, and
...
def _hub_action_name(verb: str, key: str) -> str:
    ...
    ``_spawn_eden_action`` tracks one process/log per name, so a shared
...
# Human-readable labels for each hub source id (matches `eden skills search`
# provenance).  Keep in sync with create_source_router()'s source list.
_SKILL_HUB_SOURCE_LABELS = {
    "official": "Official (Eden)",
    "eden-index": "Eden OE Index",
    "skills-sh": "skills.sh",
...
async def preview_skill_hub(identifier: str = "", profile: Optional[str] = None):
    ...
    def _run():
        from eden_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`eden -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the EDEN_HOME
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_eden_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "eden setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local EDEN_HOME override so the write lands in the target
    ...
    """
    from eden_constants import set_eden_home_override, reset_eden_home_override

    token = set_eden_home_override(str(profile_dir))
    try:
        provider, model = _normalize_main_model_assignment(provider, model)
        cfg = load_config()
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local EDEN_HOME override (same mechanism as
    ...
    """
    from eden_constants import set_eden_home_override, reset_eden_home_override
    from eden_cli.mcp_security import validate_mcp_server_entry
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the EDEN_HOME override. Returns the
    number of skills newly disabled.
    """
    from eden_constants import set_eden_home_override, reset_eden_home_override
    from eden_cli.skills_config import get_disabled_skills, save_disabled_skills
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's EDEN_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``eden profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from EDEN_HOME).
    """
    from eden_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``eden profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the EDEN_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``eden profile describe
    <name> --auto``.
    ...
    _resolve_profile_dir(name)
    try:
        from eden_cli import profile_describer
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_eden_home()`` at call
       time — the context-local override from ``set_eden_home_override``
    ...
    retargeted to the *current* ``get_eden_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a EDEN_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_eden_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_eden_home()`` at call time
...
async def get_skills(profile: Optional[str] = None):
    ...
    from eden_cli.skills_config import get_disabled_skills
...
async def get_toolsets(profile: Optional[str] = None):
    from eden_cli.tools_config import (
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``eden tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``eden tools`` picker shows
...
def _resolve_toolset_model_plugin(ts_key: str, provider_row: dict) -> Optional[str]:
    ...
    the managed "Eden Subscription" image row instead carries the legacy
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `eden tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``eden tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.eden/.env`` via ``save_env_value`` —
    the same store ``eden tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``eden tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``eden -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same EDEN_HOME the rest of the
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not Eden OE,
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``eden computer-use permissions grant`` as a background action.
...
#
# The endpoint spawns the same ``eden --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from eden_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from eden_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
from eden_cli.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from eden_cli.dashboard_auth.audit import AuditEvent, audit_log
        from eden_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Default: whatever ``eden --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``EDEN_OE_TUI_RESUME`` env var —
    matching what ``eden_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``EDEN_OE_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``EDEN_OE_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `active_session_file` (when set) is forwarded as
    ``EDEN_OE_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    ``EDEN_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_eden_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``eden -p <name>`` performs. The in-process
    ``EDEN_OE_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from eden_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("EDEN_OE_TUI_DISABLE_MOUSE", "1")
    env.setdefault("EDEN_OE_TUI_INLINE", "1")
    ...
    env["EDEN_OE_TUI_DASHBOARD"] = "1"

    if profile_dir is not None:
        env["EDEN_HOME"] = str(profile_dir)
    ...
    if resume:
        ...
        env["EDEN_OE_TUI_RESUME"] = resume

    if sidecar_url:
        env["EDEN_OE_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # inherits the profile EDEN_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["EDEN_OE_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _resolve_client_ws_host() -> Optional[str]:
    ...
    1. Explicit ``EDEN_DASHBOARD_WS_HOST`` env var — wins always. Operators
    ...
    """
    explicit = os.environ.get("EDEN_DASHBOARD_WS_HOST", "").strip()
    if explicit:
        return explicit
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from eden_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    fd, raw_path = tempfile.mkstemp(prefix="eden-pty-active-", suffix=".json")
    os.close(fd)
    path = Path(raw_path)
    files[channel] = path
    return path
...
# ---------------------------------------------------------------------------
# /api/console — safe Eden OE Console command WebSocket.
#
# Unlike /api/pty, this endpoint never spawns a PTY, shell, or full Eden OE CLI
        ...
        with _console_executor_lock:
            if _console_executor is None:
                _console_executor = concurrent.futures.ThreadPoolExecutor(
                    ...
                    thread_name_prefix="eden-console",
                )
...
def _dashboard_console_context() -> str:
    ...
    return "hosted" if _default_eden_root_is_opt_data() else "local"
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from eden_cli.console_engine import EdenConsoleEngine

        engine = EdenConsoleEngine(
            ...
            if command_id == command_generation:
                ...
                await _console_send(
                    ...
                    send_lock,
                    {
                        ...
                        "id": command_id,
                        "message": (
                            "Command timed out. Eden OE Console returned to the prompt."
                        ),
                        "command": line,
                    },
                )
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Eden OE inside WSL2 to use the dashboard's /chat "
...
# EDEN_OE_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`eden_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from eden_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/eden/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /eden`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__EDEN_OE_BASE_PATH__`` honour that prefix
    without rebuilding the bundle.
    """
    # `eden serve` is the headless backend: it must NEVER serve the browser
    ...
    _headless = os.environ.get("EDEN_OE_SERVE_HEADLESS") == "1"
    if _headless or not WEB_DIST.exists():
        _msg = (
            "Headless backend (eden serve): web UI disabled — use "
            "`eden dashboard` for the browser UI."
            if _headless
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/eden``)
        ...
        ``__EDEN_OE_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__EDEN_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__EDEN_OE_BASE_PATH__="{prefix}";'
                f"window.__EDEN_OE_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__EDEN_OE_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__EDEN_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__EDEN_OE_BASE_PATH__="{prefix}";'
                f"window.__EDEN_OE_AUTH_REQUIRED__={gated_js};"
                f"</script>"
    ...
    # under ``/eden`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Eden OE backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Eden OE Teal",         "description": "Classic dark teal — the canonical Eden OE look"},
    {"name": "default-large", "label": "Eden OE Teal (Large)", "description": "Eden OE Teal with bigger fonts and roomier spacing"},
    {"name": "eden-blue",     "label": "Eden Blue",           "description": "Light mode — vivid Eden-blue accents on cream canvas"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.eden/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.eden/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_eden_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.eden/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as eden_cli.plugins):
    1. User plugins:    ~/.eden/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.eden/plugins/  (only if EDEN_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from eden_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_eden_home() / "plugins", "user"),
    ...
    # ``eden_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("EDEN_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".eden" / "plugins", "project"))
...
async def get_dashboard_plugins():
    ...
    try:
        from eden_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
        enabled_set = _get_enabled_set()
        disabled_set = _get_disabled_set()
    except Exception:
        enabled_set = set()
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    plugins_root_resolved = (get_eden_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from eden_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from eden_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from eden_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from eden_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.eden/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"eden_dashboard_plugin_{plugin['name']}"
...
from eden_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    so Electron passes ``EDEN_OE_DESKTOP_READY_FILE`` and waits for this JSON.
    Normal CLI/dashboard launches still use the stdout READY line below.
    """
    target = os.environ.get("EDEN_OE_DESKTOP_READY_FILE")
    if not target:
        return
...
def start_server(
    ...
    build and no SPA mount (mount_spa() honours ``EDEN_OE_SERVE_HEADLESS``), so
    the banner announces the bind rather than a browser URL.
    """
    import uvicorn

    try:
        from eden_cli.eden_auth_keepalive import start_nous_auth_keepalive

        start_nous_auth_keepalive()
    except Exception as exc:
        _log.debug("Eden auth keepalive did not start: %s", exc)
    ...
    # the eden-0day MCP-persistence campaign abused unauthenticated public
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing EDEN_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Eden OE exposes a
            # module-level ``LAST_SKIP_REASON`` string for this purpose;
            # without it the operator would only see "no providers" which
            # is misleading when the provider IS installed but unconfigured.
            skip_reasons: list[str] = []
            try:
                from plugins.dashboard_auth import eden as _nous_plugin

                if _nous_plugin.LAST_SKIP_REASON:
                    skip_reasons.append(
                        f"  • eden: {_nous_plugin.LAST_SKIP_REASON}"
                    )
            except Exception:
                pass
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `eden dashboard register` (Eden Portal) or "
    ...
    # EDEN_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            ready_token = "EDEN_OE_BACKEND_READY" if headless else "EDEN_DASHBOARD_READY"
            print(f"{ready_token} port={actual_port}", flush=True)
            if headless:
                # No SPA, and the JSON-RPC/WS endpoints are auth-gated — don't
                # advertise a paste-and-connect URL, just announce the bind.
                print(f"  Eden OE backend listening on {host}:{actual_port}")
            else:
                print(f"  Eden OE Web UI → http://{host}:{actual_port}")
