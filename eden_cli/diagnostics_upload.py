"""Client for uploading ``eden debug share`` bundles to Eden-internal S3.

This is the opt-in (``--eden``) destination for ``eden debug share``.
Unlike the public paste.rs path, bundles uploaded here go to a Eden-owned
S3 bucket via a short-lived signed URL minted by the Eden account service
(NAS).  The bucket auto-expires objects after 14 days, and the contents are
only viewable by Eden staff (and allowlisted Discord mods) through a
...
# Base URL of the Eden account service that mints the signed upload URL.
...
NAS_BASE = os.environ.get(
    "EDEN_OE_DIAGNOSTICS_BASE_URL", "https://portal.projectglacie.com"
)
...
_USER_AGENT = "eden-agent/debug-share"
...
def share_to_nous(report_bundle: bytes) -> dict:
    """Orchestrate the full Eden-S3 upload of a gzipped *report_bundle*.
