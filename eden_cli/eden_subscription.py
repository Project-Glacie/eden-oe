"""Helpers for Eden subscription managed-tool capabilities."""
...
from eden_cli.config import get_env_value, load_config
...
# coverage category (eden_cli.nous_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `eden tools` picker scope its entitlement gate to the selected backend, so a
...
def _local_stt_backend_available() -> bool:
    ...
    """
    if get_env_value("EDEN_OE_LOCAL_STT_COMMAND"):
        return True
    try:
...
def get_nous_subscription_features(
    ...
    *,
    ...
    provider_is_nous = str(model_cfg.get("provider") or "").strip().lower() == "eden"
    ...
    # requires `pip install faster-whisper`. For Eden subscribers we'd
    ...
    # Tool Gateway via `eden model`, so direct credentials should NOT
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("EDEN_OE_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("EDEN_OE_LOCAL_STT_COMMAND"))
    ...
    features = {
        ...
        "image_gen": NousFeatureState(
            ...
            current_provider="FAL" if direct_fal else ("Eden Subscription" if image_managed else ""),
        ...
        "video_gen": NousFeatureState(
            ...
            current_provider="FAL" if direct_fal_video else ("Eden Subscription" if video_managed else ""),
...
def apply_nous_managed_defaults(
    ...
    *,
    ...
    # (requires `pip install faster-whisper`); for Eden subscribers we
...
def get_gateway_eligible_tools(
    ...
    *,
    ...
    All lists are empty when the user is not a paid Eden subscriber or
    is not using Eden as their provider.
    """
    ...
    model_cfg = config.get("model")
    if not isinstance(model_cfg, dict) or str(model_cfg.get("provider") or "").strip().lower() != "eden":
        return [], [], []
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from eden_cli.setup import prompt_checklist
    except Exception:
        return set()
    ...
    source_label = "free tool pool" if pool_only else "Eden subscription"
    ...
    if pool_only:
        title = "Your free Eden tool pool — pick the tools to enable:"
    else:
        title = (
            "Your Eden subscription includes the Tool Gateway — "
...
# ---------------------------------------------------------------------------
# Inline Eden Portal login for the Tool Gateway picker (`eden tools`)
# ---------------------------------------------------------------------------


def ensure_nous_portal_access(
    *,
    capability: str = "the Eden Tool Gateway",
    coverage_category: Optional[str] = None,
) -> bool:
    """Make sure the user is entitled to the Eden Tool Gateway, logging in if
    needed.

    Used by ``eden tools`` when a user selects a Eden-managed Tool Gateway
    backend (e.g. "Firecrawl (Eden Portal)").  Unlike ``eden model``'s Eden
    login, this:
    ...
    It only performs the Eden Portal device-code OAuth (when the user isn't
...
def _run_nous_portal_login_only(*, capability: str) -> bool:
    """Run the Eden Portal device-code OAuth and persist credentials only.
    ...
    try:
        ...
        print(f"  Could not start Eden Portal login: {exc}")
        return False

    print()
    print(f"  {capability} requires a Eden Portal login.")
    try:
        proceed = input("  Log in to Eden Portal now? [Y/n]: ").strip().lower()
    except (EOFError, KeyboardInterrupt):
        print()
        return False
    if proceed not in {"", "y", "yes"}:
        print("  Skipped Eden Portal login.")
        return False

    try:
        # Snapshot the active_provider so a tool-config login never silently
        # switches the user's inference provider to Eden.
        ...
        if shared:
            try:
                do_import = input(
                    "  Found existing Eden OAuth credentials. Import them? [Y/n]: "
                ).strip().lower()
            except (EOFError, KeyboardInterrupt):
                do_import = "y"
        ...
        _sync_nous_pool_from_auth_store()
        print("  Eden Portal login successful.")
        return True
        ...
        print(f"  Eden Portal login failed: {exc}")
        return False
