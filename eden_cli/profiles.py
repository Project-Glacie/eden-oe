"""
Profile management for multiple isolated Eden OE instances.

Each profile is a fully independent EDEN_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.eden/profiles/<name>/`` by default.

The "default" profile is ``~/.eden`` itself — backward compatible,
zero migration needed.

Usage::

    eden profile create coder          # fresh profile + bundled skills
    eden profile create coder --clone  # also copy config, .env, SOUL.md, skills
    eden profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    eden -p coder chat                 # or via flag
    eden profile use coder             # set as sticky default
    eden profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See eden_constants.get_subprocess_home().
    "home",
]
...
# Marker file written by `eden profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `eden
...
# `eden skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Eden OE infrastructure directories that only the default profile
         (``~/.eden``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_eden_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.eden) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "eden-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when EDEN_HOME equals the
...
# into the export. The set below identifies the *known Eden OE profile
# artifacts* at the root of EDEN_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``eden_constants``.
...
_RESERVED_NAMES = frozenset({
    "eden", "default", "test", "tmp", "root", "sudo",
})

# Eden OE subcommands that cannot be used as profile names/aliases
_EDEN_OE_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the eden root, NOT to the current EDEN_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where EDEN_HOME points outside
    ``~/.eden``, profiles live under ``EDEN_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_eden_home() / "profiles"


def _get_default_eden_home() -> Path:
    """Return the default (pre-profile) EDEN_HOME path.

    In standard deployments this is ``~/.eden``.
    In Docker/custom deployments where EDEN_HOME is outside ``~/.eden``
    (e.g. ``/opt/data``), returns EDEN_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_eden_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Eden OE installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its EDEN_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_eden_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, eden subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _EDEN_OE_SUBCOMMANDS:
        return f"'{canon}' conflicts with a eden subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            eden_exe = shutil.which("eden") or "eden"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(eden_exe)} -p {profile} "$@"\n')
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Eden OE. If we
    ...
    try:
        ...
        # not be migrated. The next `eden doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``eden -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``eden profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``eden -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``eden profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``EDEN_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Eden OE config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_eden_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, eden_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own EDEN_HOME.
    ...
    The returned ``eden_home`` is the path to pass to
    ``set_eden_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_eden_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``eden update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.eden)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.eden/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `eden -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Eden OE profile.\n"
    ...
    if not soul_path.exists():
        try:
            from eden_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `eden update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`eden profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `eden update`.\n",
                encoding="utf-8",
            )
        except OSError:
            pass  # best-effort — the feature still works via the empty skills/ dir

    # Cloned configs can be older than the running Eden OE (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `eden profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `eden -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches EDEN_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``eden profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "EDEN_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_eden_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Eden OE *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Eden OE backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``EDEN_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `eden -p <canon> profile
    ...
    eden_markers = ("eden_cli.main", "eden-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Eden OE process: either an entrypoint marker in argv, or
            # a resolved executable named `eden`.
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_eden = (
                any(marker in joined for marker in eden_markers)
                or exe_name == "eden"
                or exe_name.startswith("eden")
            )
            if not is_eden:
                continue
            ...
            # ...or by EDEN_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("EDEN_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.eden).\n"
            "To remove everything, use: eden uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_eden_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``EDEN_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from eden_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set EDEN_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("EDEN_HOME")
    try:
        os.environ["EDEN_HOME"] = str(profile_dir)
        from eden_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["EDEN_HOME"] = old_home
        elif "EDEN_HOME" in os.environ:
            del os.environ["EDEN_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.eden/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: eden profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from EDEN_HOME.

    Returns ``"default"`` if EDEN_HOME is not set or points to ``~/.eden``.
    Returns the profile name if EDEN_HOME points into ``~/.eden/profiles/<name>``.
    Returns ``"custom"`` if EDEN_HOME is set to an unrecognized path.
    """
    from eden_constants import get_eden_home
    eden_home = get_eden_home()
    resolved = eden_home.resolve()

    default_resolved = _get_default_eden_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      EDEN_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Eden file the
      user may have lying alongside EDEN_HOME (#58394).
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.eden itself — its parent is ~/ and its
        # directory name is ".eden", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: eden profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.eden itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.eden). "
            "Specify a different name: eden profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="eden_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"eden_{old_name}"
    legacy_old_host = f"eden.{old_name}"
    new_host = f"eden_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_eden_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("eden_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a EDEN_HOME path string.

    Called early in the CLI entry point, before any eden modules
    are imported, to set the EDEN_HOME environment variable.
    """
