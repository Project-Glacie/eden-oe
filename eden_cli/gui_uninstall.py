"""
Eden Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``eden desktop`` / ``eden gui``)
     Built inside the agent checkout under ``$EDEN_HOME/eden-agent/``:
       ...
       - ``$EDEN_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Eden.app`` or ``~/Applications/Eden.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Eden`` (NSIS per-user)
...
the app name ("Eden"), separate from ``$EDEN_HOME``:
  - macOS:   ``~/Library/Application Support/Eden``
  - Windows: ``%APPDATA%\\Eden``
  - Linux:   ``$XDG_CONFIG_HOME/Eden`` (default ``~/.config/Eden``)
...
``eden uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def desktop_userdata_dir() -> Path:
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Eden"
...
def source_built_gui_artifacts(eden_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``eden desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``eden-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(eden_home)
    ...
    return [
        ...
        eden_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Eden" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Eden.app"),
            home / "Applications" / "Eden OE.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Eden).
            local_base / "Programs" / "Eden",
            # Older / alternate layout some builds used.
            local_base / "eden-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Eden")
    else:
        ...
        paths += [
            data_base / "applications" / "eden.desktop",
            data_base / "applications" / "Eden OE.desktop",
        ]
    return paths


def agent_is_installed(eden_home: Path) -> bool:
    """Return True when a usable Python agent install exists under EDEN_HOME.
...
def gui_is_installed(eden_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(eden_home):
        if p.exists():
            return True
...
def gui_install_summary(eden_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = eden_home if eden_home is not None else get_eden_home()
...
def uninstall_gui(eden_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``eden-agent/eden_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$EDEN_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove eden' or "
            "'sudo dnf remove eden'). AppImage builds are a single file you "
