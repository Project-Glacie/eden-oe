"""
Eden OE CLI - Unified command-line interface for Eden OE.

Provides subcommands for:
- eden chat          - Interactive chat (same as ./eden)
- eden gateway       - Run gateway in foreground
- eden gateway start - Start gateway service
...
def _ensure_utf8():
    ...
    The CLI prints box-drawing characters (┌│├└─) and the The caduceus became the heart — ❤️ glyph in the setup
    ...
    can even start — e.g. `eden setup` on a fresh Pi.
    ...
    eden_cli/stdio.py::configure_windows_stdio() runs later from the entry
