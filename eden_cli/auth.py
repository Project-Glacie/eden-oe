"""
Multi-provider authentication system for Eden OE.

Supports OAuth device code flows (Eden Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.eden/auth.json with cross-process file locking.
...
from eden_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
# Eden Portal defaults
DEFAULT_NOUS_PORTAL_URL = "https://portal.projectglacie.com"
DEFAULT_NOUS_INFERENCE_URL = "https://inference-api.projectglacie.com/v1"
DEFAULT_NOUS_CLIENT_ID = "eden-cli"
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from eden_cli import __version__ as _EDEN_OE_CLI_VERSION
except Exception:  # pragma: no cover - version import should always succeed
    _EDEN_OE_CLI_VERSION = "unknown"
CODEX_OAUTH_USER_AGENT = f"eden-cli/{_EDEN_OE_CLI_VERSION}"
...
SPOTIFY_DOCS_URL = "https://eden-agent.projectglacie.com/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

OAUTH_OVER_SSH_DOCS_URL = "https://eden-agent.projectglacie.com/docs/guides/oauth-over-ssh"
...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    ...
    "eden": ProviderConfig(
        id="eden",
        name="Eden OE (Local 4B)",
        auth_type="api_key",
        inference_base_url="http://127.0.0.1:9093/v1",
        api_key_env_vars=("EDEN_LOCAL_KEY",),
        base_url_env_var="EDEN_BASE_URL",
    ),
...
def get_anthropic_key() -> str:
    ...
    ``~/.eden/.env`` so a deliberate key rotation isn't shadowed by a stale
    ...
    """
    from eden_cli.config import get_env_value_prefer_dotenv
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from eden_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Prefer ~/.eden/.env over os.environ so a deliberate key rotation
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    instead of prompting the operator to run ``eden auth``.
    """
    return (
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `eden model` to re-authenticate."

    if error.code == "subscription_required":
        if error.provider == "eden":
...
def _format_nous_entitlement_auth_error(error: AuthError) -> str:
    try:
        from eden_cli.eden_account import (
        ...
        message = format_nous_portal_entitlement_message(
            account_info,
            capability="Eden model access",
        )
        if message:
            return message
    except Exception:
        pass
    return f"{error} Check credits or billing in Eden Portal, then retry."
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("EDEN_OE_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.eden/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_eden_home() / "auth.json"
    # Seat belt: if pytest is running and EDEN_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch EDEN_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".eden" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set EDEN_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom EDEN_HOME that is not a profile).
    ...
    try:
        ...
        global_root = get_default_eden_root()
    except Exception:
        return None
    profile_home = get_eden_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.eden/auth.json`` even when EDEN_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_eden_root()`` for a profile-shaped EDEN_HOME can
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".eden" / "auth.json"
            try:
...
def _file_lock(
    ...
    (e.g. profile auth.json vs shared Eden store) don't share reentrancy
...
def _auth_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    (outer) and the shared Eden lock SECOND (inner). All runtime
...
def _load_auth_store(auth_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    if isinstance(raw, dict) and isinstance(raw.get("systems"), dict):
        ...
        if "nous_portal" in systems:
            providers["eden"] = systems["nous_portal"]
        return {"version": AUTH_STORE_VERSION, "providers": providers,
                "active_provider": "eden" if providers else None}
...
def _load_provider_state_with_source(
    ...
    global-root fallback state; persisting a rotated Eden refresh token only to
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    profile can see providers (e.g. ``eden``) that were only authenticated at
    global scope. Once the user runs ``eden auth login <provider>`` inside
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    ...
    Used by ``eden auth add`` OAuth paths that create credential-pool
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``eden auth add <provider>`` inside the profile, profile entries
...
def get_provider_auth_state(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    OAuth or Eden device-code session). See issue #18594 follow-up.
    """
    auth_store = _load_auth_store()
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in Eden OE.
    ...
    # the user initiated inside Eden OE (manual add / device-code / PKCE), plus
    ...
    try:
        for entry in read_credential_pool(normalized):
            ...
            if source.startswith("env:"):
                ...
                continue
            if (
                source in {"device_code", "loopback_pkce", "eden_pkce", "manual"}
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `eden logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from eden_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected — run 'eden doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'eden model' for available providers, or run 'eden doctor' to diagnose config issues."
    ...
    # Auto-detect an OpenRouter credential added via `eden auth add openrouter`
    ...
    # `eden auth list` showing the credential while requests go out with no
    ...
    for pid, pconfig in PROVIDER_REGISTRY.items():
        ...
        for env_var in pconfig.api_key_env_vars:
            if has_usable_secret(os.getenv(env_var, "")):
                ...
                # OAuth but has a stale key in ~/.eden/.env isn't silently
    ...
    raise AuthError(
        "No inference provider configured. Run 'eden model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.eden/.env.",
...
_NOUS_STALE_PORTAL_HOSTS: FrozenSet[str] = frozenset({
    "api.projectglacie.com",
})

# Allowlist of valid Eden Portal hosts. A portal_base_url outside this
...
def _migrate_stale_nous_portal_url(providers: Dict[str, Any]) -> None:
    eden = providers.get("eden")
    if not isinstance(eden, dict):
        return
    stored = (eden.get("portal_base_url") or "").strip()
    if stored:
        parsed = urlparse(stored)
        if parsed.hostname in _NOUS_STALE_PORTAL_HOSTS:
            logger.warning(
                "auth: migrating stale eden portal_base_url %s -> %s",
                stored, DEFAULT_NOUS_PORTAL_URL,
            )
            eden["portal_base_url"] = DEFAULT_NOUS_PORTAL_URL


# Allowlist of hosts the Eden Portal proxy is willing to forward inference
...
_ALLOWED_NOUS_INFERENCE_HOSTS: FrozenSet[str] = frozenset({
    "inference-api.projectglacie.com",
})
...
def _validate_nous_inference_url_from_network(url: Optional[str]) -> Optional[str]:
    ...
    if parsed.scheme != "https":
        logger.warning(
            "eden: refusing non-https inference URL scheme %r from Portal response",
            parsed.scheme,
        )
        return None
    if parsed.hostname not in _ALLOWED_NOUS_INFERENCE_HOSTS:
        logger.warning(
            "eden: refusing inference URL host %r from Portal response "
...
def _nous_portal_env_override() -> Optional[str]:
    ...
    Mirrors ``_nous_inference_env_override()``: ``EDEN_OE_PORTAL_BASE_URL`` /
    ``NOUS_PORTAL_BASE_URL`` are the documented dev/staging escape hatch for
    pointing Eden OE at a non-production Eden Portal (e.g. a hosted agent
    provisioned on eden-account-service's `staging` environment, which stamps
    ``EDEN_OE_PORTAL_BASE_URL=https://portal.staging-projectglacie.com`` into
    ...
    return _optional_base_url(
        os.getenv("EDEN_OE_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL")
    )
...
def _assert_nous_inference_jwt_usable(
    ...
    *,
    ...
    if reason is None:
        return
    raise AuthError(
        "Eden Portal access token is not a usable inference JWT "
        f"({reason}). Re-authenticate with: eden auth add eden",
        provider="eden",
        code=reason,
...
def _log_nous_invoke_jwt_selected(
    *,
    ...
    logger.info("Eden inference auth: using NAS invoke JWT")
    _oauth_trace(
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("EDEN_OE_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
def get_qwen_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        # and `eden model` walks users into a broken Qwen setup flow.
...
# =============================================================================
# Spotify auth — PKCE tokens stored in ~/.eden/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("EDEN_OE_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set EDEN_OE_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("EDEN_OE_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("EDEN_OE_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("EDEN_OE_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `eden auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `eden auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `eden auth spotify` first.",
                provider="spotify",
        ...
        if should_refresh:
            try:
                ...
                if exc.relogin_required and state.get("refresh_token"):
                    ...
                    # Mirrors the Eden / xAI-OAuth / Codex-OAuth / MiniMax pattern.
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `eden auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.eden/.env, and return it.
    ...
    print("       App name:     anything (e.g. eden-agent)")
    ...
    # Persist so subsequent `eden auth spotify` runs skip the wizard.
    save_env_value("EDEN_OE_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("EDEN_OE_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved EDEN_OE_SPOTIFY_CLIENT_ID to ~/.eden/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "EDEN_OE_SPOTIFY_CLIENT_ID is required".
    ...
    print()
    print("Open this URL to authorize Eden OE:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"Eden OE is waiting for the OAuth callback on {redirect_uri}")
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.eden/auth.json (not ~/.codex/)
#
# Eden OE maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Eden OE auth store (~/.eden/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `eden auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `eden auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `eden auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `eden auth` to re-authenticate.",
            provider="openai-codex",
...
def _sync_codex_pool_entries(
    ...
    * ``device_code`` — the singleton-seeded entry written by the device-code
      OAuth flow when the user logged in via ``eden setup`` / the model
      picker.  Always synced with the fresh tokens.
    * ``manual:device_code`` — entries created by ``eden auth add openai-codex``
      ...
      ``eden auth add openai-codex`` and must not be overwritten by a
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to Eden OE auth store (~/.eden/auth.json)."""
    ...
    with _auth_store_lock():
        ...
        # ``eden auth add openai-codex`` created (which must not be
...
def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    """Adopt a valid Codex CLI token pair into Eden OE auth, if available."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Eden OE auth state."""
    ...
    if response.status_code == 429:
        ...
        # "run eden auth" prompt (see issue #32790).
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `eden auth` to re-authenticate."
            )
            relogin_required = True
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Eden OE auth store automatically.
    """
    try:
        ...
        # Self-heal cross-store refresh_token rotation. Eden OE keeps its OWN
        ...
        # the Codex CLI (or another Eden OE process) rotates the shared token,
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Eden OE's own Codex token store.
    ...
    if data is None:
        ...
        if pool_token:
            base_url = (
                os.getenv("EDEN_OE_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("EDEN_OE_CODEX_REFRESH_TIMEOUT_SECONDS", 20)

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other Eden OE processes
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.eden/auth.json
# =============================================================================
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok / Premium+) in `eden model`.",
            provider="xai-oauth",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "xAI OAuth state is missing tokens. Re-authenticate with `eden model`.",
            provider="xai-oauth",
    ...
    if not access_token:
        raise AuthError(
            "xAI OAuth state is missing access_token. Re-authenticate with `eden model`.",
            provider="xai-oauth",
    ...
    if not refresh_token:
        raise AuthError(
            "xAI OAuth state is missing refresh_token. Re-authenticate with `eden model`.",
            provider="xai-oauth",
...
def _write_through_xai_oauth_to_global_root(state: Dict[str, Any]) -> None:
    ...
    # ~/.eden/auth.json even when EDEN_HOME points at a profile path
...
def _xai_proactive_refresh_skew_seconds(access_token: str) -> int:
    ...
    resolution (chat turn, Imagine tool call, ``eden auth status``, …),
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.eden/auth.json``. A single MITM during initial
    ...
    if host != "x.ai" and not host.endswith(".x.ai"):
        raise AuthError(
            ...
            f"discovery; re-authenticate with `eden model` to re-fetch.",
            provider="xai-oauth",
            code="xai_discovery_invalid",
        )
    return url
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``EDEN_OE_XAI_BASE_URL``
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `eden model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    # written by an older Eden OE (or hand-edited) may carry a non-xAI
    ...
    # with a clear error so the user can re-run `eden model` to refetch.
    ...
    if response.status_code != 200:
        ...
        # on the allowlist for API access).  Re-running ``eden model``
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("EDEN_OE_XAI_REFRESH_TIMEOUT_SECONDS", 20)
    ...
    base_url = _xai_validate_inference_base_url(
        os.getenv("EDEN_OE_XAI_BASE_URL", "").strip().rstrip("/")
...
def _resolve_verify(
    *,
    ...
    effective_ca = (
        ...
        or os.getenv("EDEN_OE_CA_BUNDLE")
...
# =============================================================================
# Eden Portal — token refresh and model discovery
# =============================================================================

# -----------------------------------------------------------------------------
# Shared Eden token store — lets OAuth credentials persist across profiles
# so a new `eden --profile <name> auth add eden --type oauth` can one-tap
# import instead of running the full device-code flow every time.
#
# File lives at ${EDEN_OE_SHARED_AUTH_DIR}/nous_auth.json, defaulting to
# ``<eden-root>/shared/nous_auth.json`` where ``<eden-root>`` is what
# ``get_default_eden_root()`` returns — ``~/.eden`` on Linux/macOS,
# ``%LOCALAPPDATA%\eden`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's EDEN_HOME so named profiles (which
# typically live under ``<eden-root>/profiles/<name>/``) all see the
# same file.
#
...
def _nous_shared_auth_dir() -> Path:
    """Resolve the directory that holds the shared Eden token store.

    Honors ``EDEN_OE_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<eden-root>/shared/``, where ``<eden-root>`` is what
    :func:`eden_constants.get_default_eden_root` returns — so
    Linux/macOS classic installs land at ``~/.eden/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\eden\\shared\\``, and
    Docker / custom ``EDEN_HOME`` deployments at
    ``<EDEN_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("EDEN_OE_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from eden_constants import get_default_eden_root
    return get_default_eden_root() / "shared"


def _nous_shared_store_path() -> Path:
    path = _nous_shared_auth_dir() / NOUS_SHARED_STORE_FILENAME
    # Seat belt: if pytest is running and this resolves to a path under the
    # real user's Eden OE root, refuse rather than silently corrupt cross-profile
    # state. Tests must set EDEN_OE_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from eden_constants import get_default_eden_root
        real_home_shared = (
            get_default_eden_root() / "shared" / NOUS_SHARED_STORE_FILENAME
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                f"Refusing to touch real user shared Eden auth store during test run: "
                f"{path}. Set EDEN_OE_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    """Cross-profile lock for the shared Eden OAuth store.
    ...
    try:
        ...
        # No EDEN_HOME yet (pre-setup): fall through without locking.
        yield
        return
    ...
    with _file_lock(
        ...
        timeout_seconds,
        "Timed out waiting for shared Eden auth lock",
    ):
        yield


def _merge_shared_nous_oauth_state(state: Dict[str, Any]) -> bool:
    """Copy fresher shared OAuth tokens into a profile-local Eden state."""
...
def _write_shared_nous_state(state: Dict[str, Any]) -> None:
    """Persist a minimal copy of the Eden OAuth state to the shared store.
    ...
    try:
        with _nous_shared_store_lock():
            ...
            # Create with 0o600 atomically via os.open(O_EXCL) — closes the TOCTOU
            # window where write_text() + post-write chmod briefly exposed Eden
        ...
        logger.debug("Failed to write shared Eden auth store: %s", exc)


def _read_shared_nous_state() -> Optional[Dict[str, Any]]:
    """Return the shared Eden OAuth state if present and well-formed.
    ...
    try:
        ...
        logger.debug("Shared Eden auth store at %s is unreadable: %s", path, exc)
        return None
    if not isinstance(payload, dict):
        return None
...
def _clear_shared_nous_state(reason: str) -> None:
    """Remove the shared Eden OAuth store after a terminal token failure."""
    try:
        with _nous_shared_store_lock():
            path = _nous_shared_store_path()
            try:
        ...
        logger.debug("Failed to clear shared Eden auth store: %s", exc)


def _is_terminal_nous_refresh_error(exc: Exception) -> bool:
    """True when retrying the same Eden refresh token cannot succeed."""
    return (
        isinstance(exc, AuthError)
        and exc.provider == "eden"
...
def _quarantine_nous_oauth_state(
    ...
    *,
    ...
    # sizes/booleans. NEVER pass a raw token/agent_key into the log call — Eden OE
    # has a known bug class where credential-shaped literals get corrupted in logs.
    forensic: Dict[str, Any] = {
        "reason": reason,
        "error_code": error.code,
        # No session_id field exists on Eden state; provenance is client_id +
    ...
    logger.warning(
        "Eden OAuth state quarantined (terminal auth death): %s",
...
def _quarantine_nous_pool_entries(
    ...
    *,
    ...
    """Remove singleton-seeded Eden pool entries that contain dead OAuth state."""
    pool = auth_store.get("credential_pool")
    if not isinstance(pool, dict):
        return False
    entries = pool.get("eden")
    if not isinstance(entries, list):
        return False
...
def _try_import_shared_nous_state(
    *,
    ...
    """Attempt to rehydrate Eden OAuth state from the shared store.
    ...
    try:
        ...
        logger.debug("Shared Eden import failed: %s", exc)
        return None
    except Exception as exc:
        _oauth_trace(
            "nous_shared_import_failed",
...
def _refresh_access_token(
    *,
    ...
    response = client.post(
        f"{portal_base_url}/api/oauth/token",
        headers={"x-eden-refresh-token": refresh_token},
        data={
            "grant_type": "refresh_token",
            "client_id": client_id,
        },
    ...
    if response.status_code == 200:
        ...
        if "access_token" not in payload:
            raise AuthError("Refresh response missing access_token",
                            provider="eden", code="invalid_token", relogin_required=True)
        return payload
    ...
    try:
        ...
        raise AuthError("Refresh token exchange failed",
                        provider="eden", relogin_required=True) from exc

    code = str(error_payload.get("error", "invalid_grant"))
    description = str(error_payload.get("error_description") or "Refresh token exchange failed")
    relogin = code in {"invalid_grant", "invalid_token", "refresh_token_reused"}

    # Detect the OAuth 2.1 "refresh token reuse" signal from the Eden portal
    ...
    # called POST /api/oauth/token with Eden OE's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, Eden OE's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            "Eden Portal detected refresh-token reuse and revoked this session.\n"
            "This usually means an external process (monitoring script, "
            "custom self-heal hook, or another Eden OE install sharing "
            "~/.eden/auth.json) called POST /api/oauth/token with Eden OE's "
            "refresh token without persisting the rotated token back.\n"
            "Eden refresh tokens are single-use — only Eden OE may call the "
            "refresh endpoint. For health checks, use `eden auth status` "
            "instead.\n"
            "Re-authenticate with: eden auth add eden"
        )
        relogin = True

    raise AuthError(description, provider="eden", code=code, relogin_required=relogin)
...
def fetch_nous_models(
    *,
    ...
    """Fetch available model IDs from the Eden inference API."""
    ...
    if response.status_code != 200:
        ...
        raise AuthError(description, provider="eden", code="models_fetch_failed")
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Eden OE models — they're not reliable for agentic tool-calling
            if "eden" in mid.lower():
                continue
...
def resolve_nous_access_token(
    *,
    ...
    """Resolve a refresh-aware Eden Portal access token for managed tool gateways."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
        state, state_source_path = _load_provider_state_with_source(auth_store, "eden")

        if not state:
            raise AuthError(
                "Eden OE is not logged into Eden Portal.",
                provider="eden",
                relogin_required=True,
            )

        # EDEN_OE_PORTAL_BASE_URL / NOUS_PORTAL_BASE_URL is the trusted
        ...
        with _nous_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            ...
            if not isinstance(access_token, str) or not access_token:
                raise AuthError(
                    "No access token found for Eden Portal login.",
                    provider="eden",
                    relogin_required=True,
                )
            ...
            if not _is_expiring(state.get("expires_at"), refresh_skew_seconds):
                if merged_shared:
                    _save_provider_state_to_source(auth_store, "eden", state, state_source_path)
                return access_token
...
def refresh_nous_oauth_pure(
    ...
    *,
    ...
    """Refresh Eden OAuth state without mutating auth.json directly.
    ...
    with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
        ...
        if force_refresh or current_invoke_jwt_status is not None:
            ...
            if not isinstance(refresh_token_value, str) or not refresh_token_value:
                ...
                raise AuthError(
                    "No refresh token is available for Eden Portal.",
                    provider="eden",
                    relogin_required=True,
                )
            refreshed = _refresh_access_token(
...
def refresh_nous_oauth_from_state(
    ...
    *,
    ...
    """Refresh Eden OAuth from a state dict. Thin wrapper around refresh_nous_oauth_pure."""
    tls = state.get("tls") or {}
    return refresh_nous_oauth_pure(
        state.get("access_token", ""),
        state.get("refresh_token", ""),
        state.get("client_id", "eden-cli"),
...
def persist_nous_credentials(
    ...
    *,
    ...
    """Persist Eden OAuth credentials as the singleton provider state
    and ensure the credential pool is in sync.

    Eden credentials are read at runtime from two independent locations:

    - ``providers.eden``: singleton state read by
      ``resolve_nous_runtime_credentials()`` during 401 recovery and by
      ``_seed_from_singletons()`` during pool load.
    - ``credential_pool.eden``: used by the runtime ``pool.select()`` path.

    Historically ``eden auth add eden`` wrote a ``manual:device_code`` pool
    entry only, skipping ``providers.eden``. When the runtime credential
    expired, the recovery path read the empty singleton state and raised
    ``AuthError`` silently (``logger.debug`` at INFO level).

    This helper writes ``providers.eden`` then calls ``load_pool("eden")`` so
    ...
    ``eden auth add eden --label <name>``).  It gets embedded in the
    singleton state so that ``_seed_from_singletons`` uses it as the pool
    entry's label on every subsequent ``load_pool("eden")`` instead of the
    ...
    with _auth_store_lock():
        auth_store = _load_auth_store()
        _save_provider_state(auth_store, "eden", state)
        _save_auth_store(auth_store)

    # Mirror to the shared store so a new profile can one-tap import
    # these credentials via `eden auth add eden --type oauth`. Best-
...
def _sync_nous_pool_from_auth_store() -> None:
    """Best-effort pool reseed after providers.eden changes; never fail login."""
    try:
        from agent.credential_pool import load_pool

        load_pool("eden")
    except Exception as exc:
        logger.debug("Failed to sync Eden credential pool from auth store: %s", exc)
...
def resolve_nous_runtime_credentials(
    *,
    ...
    """
    Resolve Eden inference credentials for runtime use.
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("Eden OE is not logged into Eden Portal.",
                            provider="eden", relogin_required=True)
        ...
        def _resolve_effective_routing_metadata() -> tuple[str, str, str, str]:
            ...
            portal_url = (
                _optional_base_url(state.get("portal_base_url"))
                or os.getenv("EDEN_OE_PORTAL_BASE_URL")
        ...
        def _persist_state(reason: str) -> None:
            ...
            # the mtime-keyed Eden auth-status cache warm during read paths.
            if (
        ...
        with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
            ...
            if not isinstance(access_token, str) or not access_token:
                raise AuthError("No access token found for Eden Portal login.",
                                provider="eden", relogin_required=True)
    ...
    if not isinstance(api_key, str) or not api_key:
        raise AuthError("Failed to resolve a Eden inference API key",
                        provider="eden", code="server_error")
...
# which does a synchronous OAuth refresh POST to portal.projectglacie.com. That can take
# ~350ms even on the failure path, and read-only UI surfaces (`eden tools`, status panels,
# subscription-feature checks) call it many times per render — `eden tools` → "All Platforms"
...
# `eden auth login/logout/add/remove` invalidate naturally on the next call.
...
def invalidate_nous_auth_status_cache() -> None:
    ...
    Call this from any code path that mutates Eden auth state without going
...
def _compute_nous_auth_status() -> Dict[str, Any]:
    ...
    state = get_provider_auth_state("eden")
    if state:
        base_status = {
        ...
        try:
            ...
            refreshed_state = get_provider_auth_state("eden") or state
            base_status.update(
                {
                    "logged_in": True,
...
def get_nous_session_validity() -> str:
    """Classify the Eden bootstrap session for the dashboard /api/status probe.

    Returns one of:
      - ``"valid"``    — a usable Eden credential is present (login healthy).
      - ``"terminal"`` — the Eden session has taken a terminal auth failure
      ...
      - ``"unknown"``  — indeterminate (no Eden provider state, or a transient/
    ...
    # into the Eden provider state when it clears dead tokens (the exact path
    ...
    # No Eden provider state at all, or a non-terminal not-logged-in condition
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `eden auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `eden auth` and
    # `eden model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("EDEN_OE_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("EDEN_OE_COPILOT_ACP_ARGS", "").strip()
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``eden doctor`` runs the live
    ...
    try:
        from eden_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on Eden OE' "
                    "lazy-install at first use)."
                )
            else:
                info["hint"] = (
                    "azure-identity is installed; live credential validation "
                    "is skipped here. Run `eden doctor` to verify token acquisition."
                )
            return info
        except Exception as exc:
...
def resolve_api_key_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id in {"kimi-coding", "kimi-coding-cn"}:
        ...
        try:
            from eden_cli.copilot_auth import (
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set EDEN_OE_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _update_config_for_provider(
    ...
    # model.api_key.
    from eden_cli.config import clear_model_endpoint_credentials
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `eden logout` historically keyed off auth.json.active_provider only.
    ...
    if provider in {"eden", "openai-codex", "xai-oauth"}:
        return provider
    return None
...
def _prompt_model_selection(
    ...
    """
    from eden_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from eden_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'eden model' or 'eden setup' instead."""
    print("The 'eden login' command has been removed.")
    print("Use 'eden auth' to manage credentials,")
    print("'eden model' to select a provider, or 'eden setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.eden/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing Eden-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in Eden OE auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Eden OE will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("EDEN_OE_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
                config_path = _update_config_for_provider("openai-codex", base_url)
                print()
                print("Credentials imported. Note: if Codex CLI refreshes its token,")
                print("Eden OE will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — Eden OE gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Eden OE creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Eden OE auth store
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in Eden OE auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(Eden OE creates its own local OAuth session)")
    print()
    ...
    # xAI credential re-enabled. ``eden auth remove xai-oauth`` leaves a
    # ``device_code`` suppression marker that otherwise stops the singleton
    # seed from re-creating the pool entry, so ``eden auth list`` would show
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Eden OE auth store (~/.eden/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Eden OE login via MiniMax ({region}) OAuth...")
...
def _minimax_oauth_quarantine_on_terminal_refresh(state: Dict[str, Any], exc: AuthError) -> None:
    ...
    provider. Mirrors the Eden / xAI-OAuth / Codex-OAuth quarantine pattern
    so subsequent calls fail fast without a network retry.
    """
...
def build_minimax_oauth_token_provider() -> Callable[[], str]:
    ...
    def _provide() -> str:
        ...
        if not state or not state.get("access_token"):
            raise AuthError(
                "Not logged into MiniMax OAuth. Run `eden model` and select "
                "MiniMax (OAuth).",
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    diagnostic call sites like ``eden status`` that just want to know
...
def _nous_device_code_login(
    *,
    ...
    """Run the Eden device-code flow and return full OAuth state without persisting."""
    ...
    print(f"Starting Eden OE login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `eden model` again to finish setup.")
            raise SystemExit(1)
        raise


def nous_token_has_billing_scope() -> bool:
    """Return True if the currently-held Eden token carries ``billing:manage``.
...
def step_up_nous_billing_scope(
    *,
    ...
    targets the same deployment (incl. a preview via ``EDEN_OE_PORTAL_BASE_URL`` set
    ...
    """
    prior = get_provider_auth_state("eden") or {}
    pconfig = PROVIDER_REGISTRY["eden"]
    ...
    with _auth_store_lock():
        auth_store = _load_auth_store()
        _save_provider_state(auth_store, "eden", auth_state)
        _save_auth_store(auth_store)
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    """Eden Portal device authorization flow."""
    ...
    try:
        ...
        # flow, check the shared store for an existing Eden credential
        ...
        if shared:
            ...
            print()
            if shared_path:
                print(f"Found existing Eden OAuth credentials at {shared_path}")
            else:
                print("Found existing shared Eden OAuth credentials")
            try:
                do_import = input("Import these credentials? [Y/n]: ").strip().lower()
            except (EOFError, KeyboardInterrupt):
                do_import = "y"
            if do_import in {"", "y", "yes"}:
                print("Rehydrating Eden session from shared credentials...")
        ...
        # overwrites it to "eden".  If the user picks "Skip (keep current)"
        ...
        # leave the config in a half-updated state (provider=eden but model
        ...
        try:
            ...
            unavailable_message = ""
            if model_ids:
                pricing = get_pricing_for_provider("eden")
                ...
                if free_tier:
                    ...
                    # as free so users on older Eden OE builds still see
            ...
            if model_ids:
                ...
                print("No curated models available for Eden Portal.")
        except Exception as exc:
        ...
        # them to Eden with a mismatched model.  The Eden OAuth tokens
        # stay saved for future use.
        if not selected_model:
            # Restore the prior active_provider that _save_provider_state
            # overwrote to "eden".  config.yaml model.provider is left
            ...
            with _auth_store_lock():
                ...
                _save_auth_store(auth_store)
            print()
            print("No provider change. Eden credentials saved for future use.")
            print("  Run `eden model` again to switch to Eden Portal.")
            return

        config_path = _update_config_for_provider(
            "eden", inference_base_url, default_model=selected_model,
        )
        if selected_model:
            _save_model_choice(selected_model)
            print(f"Default model set to: {selected_model}")
        print(f"  Config updated: {config_path} (model.provider=eden)")
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("Eden OE will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `eden model` or configure an API key to use Eden OE.")
        else:
