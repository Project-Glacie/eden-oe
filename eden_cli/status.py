"""
Status command for eden CLI.

Shows the status of all Eden OE components.
"""
...
from eden_cli.auth import AuthError, resolve_provider
from eden_cli.colors import Colors, color
from eden_cli.config import get_env_path, get_env_value, get_eden_home, load_config
from eden_cli.models import provider_label
from eden_cli.eden_account import (
    format_nous_portal_entitlement_message,
    get_nous_portal_account_info,
)
from eden_cli.eden_subscription import get_nous_subscription_features
from eden_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``eden config``'s
...
def show_status(args):
    """Show status of all Eden OE components."""
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ❤️ Eden OE Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (Eden inference key configured)"
    else:
        nous_label = "not logged in (run: eden portal)"
    print(
        f"  {'Eden Portal':<12}  {check_mark(nous_logged_in)} "
        f"{nous_label}"
    )
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: eden model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: eden auth add minimax-oauth)'}"
    )
    ...
    # disrupt the already-printed Eden/Codex/Qwen/MiniMax rows above.
    try:
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: eden auth add xai-oauth)'}"
    )
    ...
    # =========================================================================
    # Eden Subscription Features
    # =========================================================================
    if managed_nous_tools_enabled():
        features = get_nous_subscription_features(config)
        print()
        print(color("◆ Eden Tool Gateway", Colors.CYAN, Colors.BOLD))
        if not features.nous_auth_present:
            print("  Eden Portal   ✗ not logged in")
        else:
            print("  Eden Portal   ✓ managed tools available")
        for feature in features.items():
            if feature.managed_by_nous:
                state = "active via Eden subscription"
            elif feature.active:
        ...
        # Eden OAuth without entitlement, or an opaque inference key without
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: eden model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from eden_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from eden_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    jobs_file = get_eden_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    if _session_count is not None and _session_count > 0:
        ...
        sessions_file = get_eden_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'eden doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'eden setup' to configure", Colors.DIM))
    print()
