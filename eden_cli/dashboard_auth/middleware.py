...
from eden_cli.dashboard_auth import list_session_providers
from eden_cli.dashboard_auth.audit import AuditEvent, audit_log
from eden_cli.dashboard_auth.base import ProviderError, RefreshExpiredError
from eden_cli.dashboard_auth.cookies import (
    clear_sso_attempt_cookie,
    read_session_cookies,
    read_sso_attempt_cookie,
    set_sso_attempt_cookie,
)
from eden_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /eden``, the
    ``login_url`` is prefixed (``/eden/login?next=...``) so the
    ...
    """
    from eden_cli.dashboard_auth.prefix import prefix_from_request
...
def _auto_sso_response(request: Request) -> Response | None:
    ...
    from eden_cli.dashboard_auth.cookies import detect_https
    set_sso_attempt_cookie(
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["eden.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # sends only ``eden_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if at:
        ...
        # a self-hosted-OIDC session hits the `eden` provider first, which
        # tries to reach Eden Portal's JWKS; if that's unreachable it raises,
    ...
    if session is None:
        ...
        from eden_cli.dashboard_auth.cookies import clear_session_cookies
        from eden_cli.dashboard_auth.prefix import prefix_from_request
