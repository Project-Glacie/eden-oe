...
Profile-aware location: ``$EDEN_HOME/logs/dashboard-auth.log``.
...
from ``eden_constants`` or other eden_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$EDEN_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``eden_constants.get_eden_home`` semantics: env var wins,
    else ``~/.eden``. A local copy avoids an import cycle with the
    middleware which lives below ``eden_cli``.
    """
    home = os.environ.get("EDEN_HOME") or str(Path.home() / ".eden")
