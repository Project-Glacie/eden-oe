"""eden webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    eden webhook subscribe <name> [options]
    eden webhook list
    eden webhook remove <name>
    eden webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.eden/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _eden_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: eden gateway run
"""
...
def webhook_command(args):
    """Entry point for 'eden webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: eden webhook {subscribe|list|remove|test}")
        print("Run 'eden webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (eden gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: eden webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from eden webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (eden gateway run)")
