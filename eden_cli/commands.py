"""Slash command definitions and autocomplete for the Eden OE CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Eden OE config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Eden OE works on across turns until achieved", "Session",
    ...
    CommandDef("secret", "Store a secret with masked input (API keys, tokens — never echoed)",
               "Configuration", cli_only=True,
               args_hint="<name> [value]", subcommands=("list", "delete")),
    ...
    CommandDef("busy", "Control what Enter does while Eden OE is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.eden/skills/ for newly installed or removed skills",
    ...
    CommandDef("credits", "Show Eden credit balance and top up", "Info"),
    CommandDef("billing", "Manage Eden terminal billing — buy credits, auto-reload, limits", "Info",
               cli_only=True),
    ...
    CommandDef("update", "Update Eden OE to the latest version", "Info"),
    CommandDef("version", "Show Eden OE version", "Info", aliases=("v",)),
    CommandDef("debug", "Upload debug report (system info + logs) and get shareable links", "Info",
               args_hint="[eden|local]"),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`eden_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/eden`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Eden OE ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Eden OE
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``eden skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``eden skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``eden skills list`` and
...
# unrelated command landed. These claim their slots right after /eden,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /eden <command>).
...
# native slot, the alias spelling stays reachable via /eden reset).
...
# ``/eden <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/eden" decision, not a silent clamp.
#   - credits: the billing/top-up surface; reached via /eden credits on Slack.
#   - billing: the terminal-billing surface (buy/auto-reload/limit); /eden billing.
#   - moa: high-cost slash mode, available through /eden moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /eden debug on Slack.
_SLACK_VIA_EDEN_OE_ONLY = frozenset({"credits", "billing", "moa", "debug"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/eden <verb>`` subcommand.
    ...
    can still reach them via ``/eden <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/eden`` is always reserved as the first entry so the
    legacy ``/eden <subcommand>`` form keeps working for anything that
    ...
    # Reserve /eden as the catch-all top-level command.
    entries.append(("eden", "Talk to Eden OE or run a subcommand", "[subcommand] [args]"))
    seen.add("eden")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_EDEN_OE_ONLY:
            # Intentionally Slack-via-/eden only (see _SLACK_VIA_EDEN_OE_ONLY).
            return
    ...
    # everything except /eden, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://eden-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /eden handler.

    Maps both canonical names and aliases so /eden bg do stuff works
    the same as /eden background do stuff.

    Plugin-registered slash commands are included so ``/eden <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from eden_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
