"""
eden fallback — manage the fallback provider chain.
...
Subcommands:
  eden fallback [list]   Show the current fallback chain (default when no subcommand)
  eden fallback add      Pick provider + model via the same picker as `eden model`,
                           then append the selection to the chain
  eden fallback remove   Pick an entry to delete from the chain
  eden fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.eden/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from eden_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `eden model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  eden fallback add")
        print()
        return
    ...
    print("  Docs: https://eden-agent.projectglacie.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `eden model`, then append the selection to the chain."""
    from eden_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `eden model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `eden fallback list` to view, or `eden fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``eden fallback [subcommand]``."""
