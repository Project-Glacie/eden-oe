...
``eden gateway restart`` (or ``launchctl kickstart ai.eden.gateway``
or ``systemctl restart eden-gateway``).  When the cron fires, the
...
``eden cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``eden gateway``, ``launchctl ... eden-gateway``,
``systemctl ... eden-gateway``, ``pkill`` against the gateway) so it
...
blocks these commands at *execution* time when ``_EDEN_OE_GATEWAY=1``, and
``eden gateway stop|restart`` refuse to self-target from inside the
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `eden gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:eden\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a eden-gateway label. macOS launchd
    # labels look like `ai.eden.gateway` / `eden-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated eden services (e.g.
    # `launchctl unload ai.eden.update-checker.plist`).
    r"|(?:launchctl\s+(?:kickstart|unload|load|stop|restart)\b[^\n]*\beden[.\-]?gateway)"
    # Branch C: systemctl ops on a eden-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\beden[.\-]?gateway)"
    # Branch D: pkill / kill targeting the eden gateway process. Both
...
def _resolve_script_path(script_path: str) -> Path:
    ...
    under ``<EDEN_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.eden/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    return get_eden_home() / "scripts" / raw
...
def check_gateway_lifecycle(
    ...
    if contains_gateway_lifecycle_command(combined):
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `eden gateway restart` from a shell outside "
