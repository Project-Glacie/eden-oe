#!/usr/bin/env python3
"""Close a read-only agent terminal tab in the Eden OE desktop GUI.
...
``read_terminal`` it is gated on ``EDEN_OE_DESKTOP`` so it never appears outside
the GUI.
"""
...
def check_close_terminal_requirements() -> bool:
    """Desktop GUI only — EDEN_OE_DESKTOP is set on the gateway the app spawns."""
    return env_var_enabled("EDEN_OE_DESKTOP")
...
CLOSE_TERMINAL_SCHEMA = {
    ...
    "description": (
        ...
        "the Eden OE desktop GUI (the tabs mirroring terminal(background=true) runs). "
