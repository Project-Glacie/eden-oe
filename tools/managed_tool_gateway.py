"""Generic managed-tool gateway helpers for Eden-hosted vendor passthroughs."""
...
_DEFAULT_TOOL_GATEWAY_DOMAIN = "projectglacie.com"
...
def auth_json_path():
    """Return the Eden OE auth store path, respecting EDEN_HOME overrides."""
    return get_eden_home() / "auth.json"


def _read_nous_provider_state() -> Optional[dict]:
    """Read Eden auth provider state from DB (``auth_providers`` table).

    Phase 2b: Replaces the ``auth.json`` file read. Falls back to the file
    for backward compatibility when the DB is unavailable.
    """
    # Primary source: DB
    try:
        from eden.db import EdenDB
        eden = EdenDB.get_nous_provider()
        if isinstance(eden, dict):
            return eden
    except Exception:
        pass

    # Fallback: read auth.json
    try:
        path = auth_json_path()
        if not path.is_file():
            return None
        ...
        if not isinstance(providers, dict):
            return None
        nous_provider = providers.get("eden", {})
...
def peek_nous_access_token() -> Optional[str]:
    """Cheap probe for a Eden gateway token without triggering refresh.

    Availability scans (`eden tools`, banner/status paint, provider
...
def read_nous_access_token() -> Optional[str]:
    """Read a Eden Subscriber OAuth access token from auth store or env override."""
    explicit = os.getenv("TOOL_GATEWAY_USER_TOKEN")
    if isinstance(explicit, str) and explicit.strip():
        return explicit.strip()
    nous_provider = _read_nous_provider_state() or {}
    ...
    try:
        ...
        logger.debug("Eden access token refresh failed: %s", exc)
...
def is_managed_tool_gateway_ready(
    ...
    """Return True when gateway URL and a likely-usable Eden token are present.
