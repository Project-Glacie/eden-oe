...
# Eden-internal env vars that should NOT leak into terminal subprocesses.
_EDEN_OE_PROVIDER_ENV_FORCE_PREFIX = "_EDEN_OE_FORCE_"

# Eden-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a Eden OE inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    try:
        from eden_cli.config import OPTIONAL_ENV_VARS
    ...
    # Eden-managed inference credential — Claude subscription auth is not a
    # working Eden OE provider path.  Stripping it broke agent-spawned
...
_EDEN_OE_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()
...
# project's dependencies into the Eden OE venv path instead of the project's own
# ``.venv`` — silently clobbering the Eden OE environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# Eden OE venv stays reachable via PATH (its bin dir is first), so stripping
# these markers is safe and only prevents the cross-project clobber (#23473).
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX")


def _is_eden_internal_secret(key: str) -> bool:
    """Return True for Eden-internal secrets injected under *dynamic* names.

    ``_EDEN_OE_PROVIDER_ENV_BLOCKLIST`` is name-based and derived from the
    ...
    This is the single source of truth for "Eden-internal dynamic secret"
    across every spawn path — the terminal ``_make_run_env`` /
    ``_sanitize_subprocess_env`` filters, the Docker passthrough filter, and the
    non-terminal :func:`eden_subprocess_env` helper all call it, so the
...
def _inject_context_eden_home(env: dict) -> None:
    """Bridge the context-local Eden OE home override into subprocess env."""
    try:
        from eden_constants import get_eden_home_override

        value = get_eden_home_override()
        if value:
            env["EDEN_HOME"] = value
    except Exception:
        pass
...
def _inject_session_context_env(env: dict) -> None:
    ...
    (EDEN_OE_SESSION_*) are bridged onto the child env here.
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Eden-managed secrets from a subprocess environment."""
    try:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_EDEN_OE_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if _is_eden_internal_secret(key):
            continue
        if key not in _EDEN_OE_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_EDEN_OE_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_EDEN_OE_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_eden_internal_secret(real_key):
                continue
            sanitized[real_key] = value
        elif _is_eden_internal_secret(key):
            continue
        elif key not in _EDEN_OE_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    _inject_context_eden_home(sanitized)

    from eden_constants import apply_subprocess_home_env
...
# legitimate child Eden OE spawns needs them, and they are the highest-value
...
# narrow subset of _EDEN_OE_PROVIDER_ENV_BLOCKLIST; provider keys are handled by
# the conditional Tier-2 strip in eden_subprocess_env().
_ALWAYS_STRIP_KEYS: frozenset[str] = frozenset({
    ...
    # _is_eden_internal_secret, but _ID has no secret suffix, so it must be
...
def eden_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    single source of truth (``_EDEN_OE_PROVIDER_ENV_BLOCKLIST``).  The terminal
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child Eden OE spawns legitimately needs them.
    * **Tier 2 (conditional):** the rest of ``_EDEN_OE_PROVIDER_ENV_BLOCKLIST``
    ...
    for key in _ALWAYS_STRIP_KEYS:
        env.pop(key, None)
    # Internal routing hints and Eden-internal dynamic secrets
    ...
    # legitimate use for them. See :func:`_is_eden_internal_secret`.
    for key in list(env):
        if key.startswith(_EDEN_OE_PROVIDER_ENV_FORCE_PREFIX):
            env.pop(key, None)
        elif _is_eden_internal_secret(key):
    ...
    if not inherit_credentials:
        ...
        for key in _EDEN_OE_PROVIDER_ENV_BLOCKLIST:
            env.pop(key, None)

    # Windows UTF-8 safety for spawned processes (#31420).
    env.setdefault("PYTHONUTF8", "1")

    _inject_context_eden_home(env)
    from eden_constants import apply_subprocess_home_env
    ...
    # copies os.environ, whose EDEN_OE_SESSION_* mirror is a last-writer-wins
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("EDEN_OE_GIT_BASH_PATH")
    ...
    #   PortableGit: %LOCALAPPDATA%\eden\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\eden\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _eden_portable_git = os.path.join(_local_appdata, "eden", "git") if _local_appdata else ""
    if _eden_portable_git:
        for candidate in (
            os.path.join(_eden_portable_git, "bin", "bash.exe"),        # PortableGit (primary)
            os.path.join(_eden_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
    ...
    raise RuntimeError(
        "Git Bash not found. Eden OE requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set EDEN_OE_GIT_BASH_PATH to your bash.exe location."
    )
...
# Cached directory containing the ``eden`` console-script.
# ``_SENTINEL`` distinguishes "not resolved yet" from a resolved ``None``.
_SENTINEL = object()
_EDEN_OE_BIN_DIR: "str | None | object" = _SENTINEL


def _resolve_eden_bin_dir() -> str | None:
    """Return the directory holding the ``eden`` console-script, or None.
    ...
    eden install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
    nix) is absent from that PATH, so plugins shelling out to bare ``eden``
    via the terminal tool hit ``command not found`` (exit 127) even though
    ``eden`` works fine in the user's own interactive terminal.

    We resolve the install dir once (it never changes within a process) and
    prepend-if-missing it to the subshell PATH so bare ``eden`` resolves
    regardless of how the gateway was started.

    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("eden")`` — normal PATH-installed shim.
      2. The directory of ``sys.argv[0]`` when it's an absolute path to a
         real ``eden`` executable (covers nix-store / venv wrappers).
      3. The directory of ``sys.executable`` — the running interpreter's
         venv ``bin``/``Scripts`` is where its console-scripts live.
    """
    global _EDEN_OE_BIN_DIR
    if _EDEN_OE_BIN_DIR is not _SENTINEL:
        return _EDEN_OE_BIN_DIR  # type: ignore[return-value]

    candidate: str | None = None

    which = shutil.which("eden")
    if which:
    ...
    if candidate is None:
        ...
        if (
            os.path.isabs(argv0)
            and (base == "eden" or base.startswith("eden."))
    ...
    if candidate is None:
        ...
        if exe_dir:
            shim = "eden.exe" if _IS_WINDOWS else "eden"
    ...
    _EDEN_OE_BIN_DIR = candidate
    return candidate


def _prepend_eden_bin_dir(existing_path: str) -> str:
    """Prepend the eden install dir to ``existing_path`` if it's missing.
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    Windows commands such as ``tasklist``, ``schtasks``, and ``wmic``.  Eden OE
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_EDEN_OE_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_EDEN_OE_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_eden_internal_secret(real_key):
                continue
            run_env[real_key] = v
        elif _is_eden_internal_secret(k):
            continue
        elif k not in _EDEN_OE_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
    path_key = _path_env_key(run_env)
    if path_key is not None:
        new_path = _append_missing_sane_path_entries(run_env.get(path_key, ""))
        # Ensure the eden install dir is reachable so plugins can shell out
        # to bare ``eden`` via the terminal tool even when the gateway was
        # launched without it on PATH (systemd, service managers, cron, etc.).
        run_env[path_key] = _prepend_eden_bin_dir(new_path)

    _inject_context_eden_home(run_env)
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Eden OE trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``EDEN_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under EDEN_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_eden_home() / "cache" / "terminal"
            except Exception:
                cache_dir = Path(tempfile.gettempdir()) / "eden_terminal"
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._eden_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_eden_pgid", None)
                    if pgid is None:
                        raise
