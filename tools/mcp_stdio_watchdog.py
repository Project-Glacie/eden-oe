...
<url>``) is spawned as a direct child of the Eden OE process. Eden OE's own
...
Eden process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Eden OE startup's opt-in
