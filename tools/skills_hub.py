#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Eden OE Skills Hub.
...
Used by eden_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _eden_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            eden_meta = metadata.get("eden", {})
            if isinstance(eden_meta, dict):
                tags = eden_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Project Glacie) but not activated
    by default — they don't appear in the system prompt and aren't copied to
    ~/.eden/skills/ during setup.  They are discoverable via the Skills Hub
    (search / install / inspect) and labelled "official" with "builtin" trust.
    """

    OFFICIAL_REPO = "ProjectGlacie/eden-agent"
...
# ---------------------------------------------------------------------------
# Eden OE centralized index source
# ---------------------------------------------------------------------------

EDEN_OE_INDEX_URL = "https://eden-agent.projectglacie.com/docs/api/skills-index.json"
EDEN_OE_INDEX_TTL = 6 * 3600  # 6 hours


def _eden_index_cache_file() -> Path:
    return _index_cache_dir() / "eden-index.json"


def _load_eden_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for EDEN_OE_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    eden_index_cache_file = _eden_index_cache_file()
    if eden_index_cache_file.exists():
        try:
            age = time.time() - eden_index_cache_file.stat().st_mtime
            if age < EDEN_OE_INDEX_TTL:
                return json.loads(eden_index_cache_file.read_text())
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Eden OE index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Eden OE index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Eden OE index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        eden_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        eden_index_cache_file.write_text(json.dumps(data))
    except OSError:
        pass
...
class EdenIndexSource(SkillSource):
    """Skill source backed by the centralized Eden OE Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "eden-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "eden-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        EdenIndexSource(auth=auth), # Centralized index (search + resolved install paths)
