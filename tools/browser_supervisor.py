...
One ``CDPSupervisor`` runs per Eden OE ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Eden OE logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "eden-dialog-bridge.invalid"
...
(() => {
  if (window.__edenDialogBridgeInstalled) return;
  window.__edenDialogBridgeInstalled = true;
  const ENDPOINT = "http://eden-dialog-bridge.invalid/";
