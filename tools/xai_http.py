...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``eden tools`` repaint, tool-registration scans,
    ...
    2. ``~/.eden/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_eden_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.eden/.env`` first, then ``os.environ``.

    Wraps :func:`eden_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from eden_cli.config import get_env_value as _eden_get_env_value

        value = _eden_get_env_value(name)
...
def eden_xai_user_agent() -> str:
    """Return a stable Eden-specific User-Agent for xAI HTTP calls."""
    try:
        from eden_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Eden-OE/{__version__}"


def _load_config_section(section_name: str) -> Dict[str, Any]:
    """Return a top-level Eden OE config section as a dict, or empty."""
    try:
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Eden OE home, then mark it seen."""
    ...
    try:
        ...
        marker_dir = get_eden_home() / "state"
...
def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    ...
    Prefers Eden-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``eden_cli.config.get_env_value`` so keys
    stored in ``~/.eden/.env`` (the standard Eden OE location) are honored —
