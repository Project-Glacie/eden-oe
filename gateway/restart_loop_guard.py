...
Defenses 1 and 2 (the ``_EDEN_OE_GATEWAY`` guard on ``eden gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.eden.gateway")``,
...
State lives in ``<EDEN_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_eden_home() / "gateway" / "restart_loop.json"
