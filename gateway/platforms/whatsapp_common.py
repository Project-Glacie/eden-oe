...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "❤️ *Eden OE*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to EDEN_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/eden), this function
    mirrors the bridge source to a writable EDEN_HOME location and returns that
    ...
    # Try EDEN_HOME location first
    eden_home = get_eden_home()
    eden_home_bridge = eden_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to EDEN_HOME if needed
    if eden_home_bridge.exists():
        return eden_home_bridge

    # Mirror the bridge source to EDEN_HOME
    try:
        eden_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            eden_home_bridge,
            dirs_exist_ok=False,
