...
There are two ways to add a platform to the Eden OE gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.eden/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Eden OE code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `eden gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `eden_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Eden OE core.
...
And add it to the `eden-gateway` composite:

```python
"eden-gateway": {
    "includes": [..., "eden-your-platform"]
}
```
...
## 12. Status Display (`eden_cli/status.py`)
...
## 13. Gateway Setup Wizard (`eden_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ eden_cli/ toolsets.py \
  --include="*.py" -l | sort -u
