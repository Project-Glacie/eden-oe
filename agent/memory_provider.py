...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `eden backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - eden_home (str): The active EDEN_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.eden``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "eden").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'eden memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], eden_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'eden memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``eden_home`` is the active EDEN_HOME directory path.
    ...
    def on_memory_write(
        ...
        """Mirror built-in memory writes to Haven's life database.

        Writes to haven_life.eden → memory_entries with FTS indexing.
        Flat file memory continues to operate normally — this is additive.
        """
        ...
        try:
            ...
            # Derive basic VAD from target type
            valence = 0.7 if target == "user" else 0.5
            arousal = 0.3
            dominance = 0.6
            importance = 0.7
            source = metadata.get("write_origin", "memory_tool") if metadata else "memory_tool"
            db.execute(
                "INSERT INTO memory_entries (content, importance, source, valence, arousal, dominance) "
                "VALUES (?, ?, ?, ?, ?, ?)",
                (content[:2000], importance, source, valence, arousal, dominance)
            )
            db.commit()
            # Rebuild FTS
            ...
            pass  # Never break the main memory flow

    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE EDEN_HOME.

        ``eden backup`` only walks EDEN_HOME, so any provider state kept
        ...
        of the archive, and ``eden import`` restores them to their original
