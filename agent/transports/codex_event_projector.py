"""Projects codex app-server events into Eden OE' messages list.

The translator that lets Eden OE' memory/skill review keep working under the
...
Eden' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape Eden OE
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way Eden OE' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but Eden OE stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way Eden OE already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — Eden OE' messages store text only.
        text_parts: list[str] = []
