...
Owns one Codex thread per Eden OE session. Drives `turn/start`, consumes
...
# Eden OE' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_EDEN_OE_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Eden/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Eden OE session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _EDEN_OE_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("EDEN_OE_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="eden",
            client_title="Eden OE",
            client_version=_get_eden_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Eden OE' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Eden OE' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own eden-tools callback we
            # auto-accept — the user already approved Eden OE' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Eden OE approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Eden OE returns 'once', 'session', 'always', or 'deny'.
...
def _get_eden_version() -> str:
    """Best-effort Eden OE version string for codex's userAgent line."""
    try:
