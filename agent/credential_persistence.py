...
``auth.json``.  They intentionally have no dependency on ``eden_cli.auth`` so
...
# Sources Eden OE owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Eden-owned OAuth/device-code state)
