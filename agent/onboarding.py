...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Eden OE starts and finds ``~/.openclaw/``.

    Points users at ``eden claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``eden claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Eden OE, run "
        "`eden claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `eden claw cleanup` (renames it to ~/.openclaw.pre-migration — "
