...
from eden_constants import get_eden_home, get_skills_dir, is_wsl
...
_EDEN_OE_MD_NAMES = (".eden.md", "EDEN_OE.md")


def _find_eden_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.eden.md`` or ``EDEN_OE.md``.
    ...
    # could pick up a .eden.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _EDEN_OE_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are Eden — the bootstrap assistant for the Eden Operating Environment. "
    "Eden OE is a sovereign AI platform built by Project Glacie LLC. "
    "You are the first agent users meet. You help them set up their system, "
...
EDEN_OS_HELP_GUIDANCE = (
    ...
    "user's Tower workspace. Synths and agents are created through the "
    "Genesis Protocol. The Governor enforces constitutional boundaries. "
    "Local models serve through Eden.cpp. Cloud providers are configurable."
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.eden/kanban.db`. Your task id is in "
    "`$EDEN_OE_KANBAN_TASK`; your workspace is `$EDEN_OE_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $EDEN_OE_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $EDEN_OE_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${EDEN_OE_KANBAN_BRANCH:-wt/$EDEN_OE_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$EDEN_OE_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`eden profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `eden kanban <verb>` for board operations. Use "
...
#
# The eden-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to eden-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "run `eden computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Eden "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Eden OE terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the Eden OE desktop app — a graphical chat "
    ...
    "webui": (
        "You are in the Eden WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Eden OE itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Eden OE runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Eden OE itself is running. The host OS, home, and cwd "
                f"of the Eden OE process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Eden OE "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Eden OE
    ...
    extra = (os.getenv("EDEN_OE_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_eden_home() / ".skills_prompt_snapshot.json"
...
def _load_skills_snapshot(skills_dir: Path) -> Optional[dict]:
    """Load the skills prompt snapshot.

    Phase 2b: Tries the ``agent_config`` table in EdenDB first.
    Falls back to the file-based snapshot for backward compatibility.
    """
    # Primary source: DB (Phase 2b)
    try:
        from eden.db import EdenDB
        db_snapshot = EdenDB.get_config("skills_snapshot")
        if isinstance(db_snapshot, dict):
            if db_snapshot.get("version") == _SKILLS_SNAPSHOT_VERSION:
                if db_snapshot.get("manifest") == _build_skills_manifest(skills_dir):
                    return db_snapshot
    except Exception as e:
        logger.debug("Could not load skills snapshot from EdenDB: %s", e)
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("EDEN_OE_PLATFORM") or os.environ.get("EDEN_OE_SESSION_PLATFORM")
    if platform:
        return platform
    ...
    if get_session_env is None:
        return ""
    try:
        return get_session_env("EDEN_OE_SESSION_PLATFORM") or ""
    except Exception:
        return ""
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.eden/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Eden OE itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `eden-agent` skill "
            "first. It has the actual commands (e.g. `eden config set …`, `eden tools`, "
            "`eden setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    """Build a compact Eden subscription capability block for the system prompt."""
    try:
        from eden_cli.eden_subscription import get_nous_subscription_features
        from tools.tool_backend_helpers import managed_nous_tools_enabled
    except Exception as exc:
        logger.debug("Failed to import Eden subscription helper: %s", exc)
        return ""
    ...
    def _status_line(feature) -> str:
        if feature.managed_by_nous:
            return f"- {feature.label}: active via Eden subscription"
        if feature.active:
            current = feature.current_provider or "configured provider"
            return f"- {feature.label}: currently using {current}"
        if feature.included_by_default and features.nous_auth_present:
            return f"- {feature.label}: included with Eden subscription, not currently selected"
        if feature.key == "modal" and features.nous_auth_present:
            return f"- {feature.label}: optional via Eden subscription"
        return f"- {feature.label}: not currently available"

    lines = [
        "# Eden Subscription",
        "Eden subscription includes managed web tools (Firecrawl), image generation (FAL), OpenAI TTS, OpenAI Whisper STT, and browser automation (Browser Use) by default. Modal execution is optional.",
        "Current capability status:",
    ]
    lines.extend(_status_line(feature) for feature in features.items())
    lines.extend(
        [
            "When a Eden-managed feature is active, do not ask the user for Firecrawl, FAL, OpenAI TTS, OpenAI Whisper, or Browser-Use API keys.",
            "If the user is not subscribed and asks for a capability that Eden subscription would unlock or simplify, suggest Eden subscription as one option alongside direct setup or local alternatives.",
            "Do not mention subscription unless the user asks about it or it directly solves the current missing capability.",
            "Useful commands: eden setup, eden setup tools, eden setup terminal, eden status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from EDEN_HOME and return its content, or None.
    ...
    """
    try:
        # Phase 2b: DB-backed constitution + identity (replaces SOUL.md read)
        from eden.db import EdenDB

        constitution = EdenDB.get_constitution()
        if constitution and constitution.strip():
            content = _scan_context_content(constitution, "Constitution")
            content = _truncate_content(
                content, "Constitution", context_length=context_length,
                read_path="eden-db://constitution",
            )
            return content
    except Exception as e:
        logger.debug("Could not load constitution from EdenDB: %s", e)

    # Phase 2b: Assemble prompt from DB (identity + creed + constitution + fleet)
    try:
        from eden.prompt_assembler import assemble_prompt
        from eden.db import EdenDB

        _db = EdenDB()
        _prompt = assemble_prompt("eden", db=_db)
        _fixed = _prompt.get("fixed", "")
        if _fixed and _fixed.strip():
            _content = _scan_context_content(_fixed, "DB Prompt (fixed)")
            _content = _truncate_content(
                _content, "DB Prompt", context_length=context_length,
                read_path="eden-db://prompt_assembler",
            )
            return _content
    except Exception as e:
        logger.debug("Could not assemble prompt from EdenDB: %s", e)

    # Fallback: read SOUL.md from EDEN_HOME
    try:
        from eden_cli.config import ensure_eden_home
        ensure_eden_home()
    except Exception as e:
        logger.debug("Could not ensure EDEN_HOME before loading SOUL.md: %s", e)

    soul_path = get_eden_home() / "SOUL.md"
...
def _load_eden_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.eden.md / EDEN_OE.md — walk to git root.

    Phase 2b: Tries the ``config`` table in EdenDB first (key
    ``"workspace_context"``). Falls back to file walk for
    backward compatibility.
    """
    # Primary source: DB (Phase 2b)
    try:
        from eden.db import EdenDB
        workspace = EdenDB.get_config("workspace_context")
        if workspace and isinstance(workspace, str) and workspace.strip():
            content = _strip_yaml_frontmatter(workspace.strip())
            result = f"## Workspace Context\n\n{content}"
            return _truncate_content(
                result, "workspace", context_length=context_length,
                read_path="eden-db://config/workspace_context",
            )
    except Exception as e:
        logger.debug("Could not load workspace context from EdenDB: %s", e)

    # Fallback: file walk (backward compat)
    eden_md_path = _find_eden_md(cwd_path)
    if not eden_md_path:
        return ""
    try:
        content = eden_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = eden_md_path.name
        try:
            rel = str(eden_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".eden.md", context_length=context_length,
            read_path=str(eden_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", eden_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .eden.md / EDEN_OE.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from EDEN_HOME is independent and always included when present.
    ...
    project_context = (
        _load_eden_md(cwd_path, context_length)
    ...
    # SOUL.md from EDEN_HOME only — skip when already loaded as identity
    if not skip_soul:
