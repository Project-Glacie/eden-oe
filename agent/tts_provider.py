...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Eden OE with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.eden/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Eden OE
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``eden tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``eden setup`` for
