...
from eden_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``EDEN_OE_KANBAN_TASK`` / ``EDEN_OE_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("EDEN_OE_KANBAN_TASK") or os.getenv("EDEN_OE_KANBAN_BOARD"):
            result = True
        else:
            try:
        ...
        # The Eden OE Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``eden_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``EDEN_OE_PLATFORM`` or
            ``EDEN_OE_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        platform
        or os.getenv("EDEN_OE_PLATFORM")
        or get_session_env("EDEN_OE_SESSION_PLATFORM")
    )
...
# which becomes the dominant cost of ``eden`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.eden/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``eden`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against EDEN_HOME, not cwd
        if not p.is_absolute():
            p = (eden_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.eden/skills/`` first, then external.
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.eden/skills/`` (including via symlinks) or configured
    ...
    # ~/.eden/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Eden OE can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    eden = metadata.get("eden") or {}
    if not isinstance(eden, dict):
        eden = {}
    return {
        "fallback_for_toolsets": eden.get("fallback_for_toolsets", []),
        "requires_toolsets": eden.get("requires_toolsets", []),
        "fallback_for_tools": eden.get("fallback_for_tools", []),
        "requires_tools": eden.get("requires_tools", []),
    }
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Eden OE metadata, VCS, virtualenv/dependency, cache, and skill
