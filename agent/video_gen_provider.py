...
as ``kind: backend``) or ``~/.eden/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``eden tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$EDEN_HOME/cache/videos/``, creating parents as needed."""
    from eden_constants import get_eden_home

    path = get_eden_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$EDEN_HOME/cache/videos/``.
