...
row taxonomy is inferred from the atlas shape so Eden OE can render both legacy
...
# rows (1536x1872), while older Eden/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Eden OE' activity state names. They are not always identical to the
...
# Legacy Eden/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Eden OE activity names -> accepted row-name aliases in descending
