...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the EDEN_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Eden OE runtime / deployment
    "EDEN_HOME", "EDEN_OE_PROFILE", "EDEN_OE_GATEWAY_LOCK_DIR",
    "EDEN_OE_MAX_ITERATIONS", "EDEN_OE_MAX_TOKENS", "EDEN_OE_API_TIMEOUT",
    "EDEN_OE_REDACT_SECRETS", "EDEN_OE_NOUS_TIMEOUT_SECONDS",
    "_EDEN_OE_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "EDEN_OE_KANBAN_DB", "EDEN_OE_KANBAN_WORKSPACES_ROOT", "EDEN_OE_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "EDEN_OE_KANBAN_",
    "EDEN_OE_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def build_profile_secret_scope(eden_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(eden_home) / ".env")
