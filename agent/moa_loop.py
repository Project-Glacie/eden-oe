...
MoA-enabled; the normal Eden OE agent loop still owns tool calling and turn
...
def _slot_runtime(slot: dict[str, str]) -> dict[str, Any]:
    ...
    try:
        ...
        # bedrock SigV4 signing, eden Portal tags — still fire. Those branches
...
def _reference_messages(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    conversation flow, but flatten it into clean user/assistant *text* turns:
      ...
      - system prompt: dropped (8K of Eden OE boilerplate, not advisory signal).
...
def _preset_temperature(preset: dict[str, Any], key: str) -> float | None:
    ...
    single-model Eden OE agent (which never sends temperature unless
...
def aggregate_moa_context(
    *,
    ...
    synth_prompt = (
        ...
        "Eden OE agent. Focus on next steps, tool-use strategy, risks, and any "
    ...
    return (
        ...
        "normal Eden OE agent loop. You may call tools, continue reasoning, or "
        "finish normally.]\n"
