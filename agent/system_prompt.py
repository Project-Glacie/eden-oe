...
upstream prefix cache warm.  See ``eden-agent-dev``'s
...
* ``stable``   — identity (SOUL.md or DEFAULT_AGENT_IDENTITY), tool
  guidance, computer-use guidance, eden subscription block, tool-use
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``EDEN_OE_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("EDEN_OE_DESKTOP_TERMINAL")):
        return hint
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Eden OE never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want EDEN_HOME persona while keeping
    ...
    # EDEN_OE_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Eden, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Eden OE profile the agent is running
    # under so it doesn't conflate ~/.eden/skills/ (default profile) with
    # ~/.eden/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Eden profile: default. Other profiles (if any) live "
            "under ~/.eden/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Eden profile: {active_profile}. This session reads "
            f"and writes ~/.eden/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.eden/skills/, ~/.eden/plugins/, "
            f"~/.eden/cron/, ~/.eden/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Eden OE never rebuilds or reinjects parts of it
