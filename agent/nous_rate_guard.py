"""Cross-session rate limit guard for Eden Portal.

Writes rate limit state to a shared file so all sessions (CLI, gateway,
cron, auxiliary) can check whether Eden Portal is currently rate-limited
before making requests.  Prevents retry amplification when RPH is tapped.

Each 429 from Eden triggers up to 9 API calls per conversation turn
(3 SDK retries x 3 Eden OE retries), and every one of those calls counts
...
_STATE_FILENAME = "eden.json"


def _state_path() -> str:
    """Return the path to the Eden rate limit state file."""
    try:
        from eden_constants import get_eden_home
        base = get_eden_home()
    except ImportError:
        base = os.path.join(os.path.expanduser("~"), ".eden")
...
def record_nous_rate_limit(
    *,
    ...
    """Record that Eden Portal is rate-limited.
    ...
    try:
        ...
        logger.info(
            "Eden rate limit recorded: resets in %.0fs (at %.0f)",
            reset_at - now, reset_at,
        )
    except Exception as exc:
        logger.debug("Failed to write Eden rate limit state: %s", exc)


def nous_rate_limit_remaining() -> Optional[float]:
    """Check if Eden Portal is currently rate-limited.
...
def clear_nous_rate_limit() -> None:
    """Clear the rate limit state (e.g., after a successful Eden request)."""
    try:
        os.unlink(_state_path())
    except FileNotFoundError:
        pass
    except OSError as exc:
        logger.debug("Failed to clear Eden rate limit state: %s", exc)
...
def is_genuine_nous_rate_limit(
    *,
    ...
    """Decide whether a 429 from Eden Portal is a real account rate limit.

    Eden Portal multiplexes multiple upstream providers (DeepSeek, Kimi,
    MiMo, Eden OE, ...) behind one endpoint.  A 429 can mean either:

      (a) The caller's own RPM / RPH / TPM / TPH bucket on Eden is
      ...
      (b) The upstream provider is out of capacity for a specific model
          ...
          the caller's quota on Eden.

    Tripping the cross-session breaker on (b) blocks ALL Eden requests
    (and all models, since Eden is one provider key) for minutes even
    ...
    We tell the two apart by looking at:
      ...
      1. The 429 response's own ``x-ratelimit-*`` headers.  Eden emits
