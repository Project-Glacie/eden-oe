...
Resolution order for text tasks (auto mode):
  ...
  3. Eden Portal (~/.eden/auth.json active provider)
...
from eden_constants import OPENROUTER_BASE_URL
...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_ca_cert`` / ``ssl_verify`` config and the ``EDEN_OE_CA_BUNDLE`` /
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # Eden OE owns auxiliary retry + provider/model fallback policy (the
    ...
    # Eden OE sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let Eden OE control the budget; explicit callers can still
    # override via kwargs.
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before Eden OE triggers summarization.  Higher values delay
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://eden-agent.projectglacie.com",
    "X-Title": "Eden OE",
...
def _apply_user_default_headers(headers: dict | None) -> dict | None:
    ...
    """
    try:
        from eden_cli.config import cfg_get, load_config
        _cfg = load_config()
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``EDEN_OE_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``EDEN_OE_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("EDEN_OE_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("EDEN_OE_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "EdenOS",
}
...
# Eden Portal extra_body for product attribution.
# Callers should pass this as extra_body in chat.completions.create()
# when the auxiliary client is backed by Eden Portal.
#
# The tags are computed from agent.portal_tags so the client= marker stays
# in lockstep with eden_cli.__version__ across every Portal call site
...
def _nous_extra_body() -> dict:
    """Return a fresh Eden Portal ``extra_body`` dict.

    Computed at call time so a hot-reloaded ``eden_cli.__version__`` is
...
# Set at resolve time — True if the auxiliary client points to Eden Portal
auxiliary_is_nous: bool = False

# Default auxiliary models per provider
_OPENROUTER_MODEL = "google/gemini-3-flash-preview"
_NOUS_MODEL = "google/gemini-3-flash-preview"
_NOUS_DEFAULT_BASE_URL = "https://inference-api.projectglacie.com/v1"
_ANTHROPIC_DEFAULT_BASE_URL = "https://api.anthropic.com"
_AUTH_JSON_PATH = get_eden_home() / "auth.json"
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Eden OE)",
...
def _pool_runtime_api_key(entry: Any) -> str:
    ...
    # provider-specific fallback (e.g. agent_key for eden).
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    if getattr(entry, "provider", None) == "eden":
        # Funnel through the canonical auth-layer reader so the env override
        # shares one normalization path with the rest of the EDEN resolution.
        from eden_cli.auth import _nous_inference_env_override

        env_url = _nous_inference_env_override()
        if env_url:
            return env_url
    # runtime_base_url handles provider-specific logic (e.g. eden prefers inference_base_url).
...
def _nous_min_key_ttl_seconds() -> int:
    try:
        return max(60, int(os.getenv("EDEN_OE_NOUS_MIN_KEY_TTL_SECONDS", "1800")))
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``eden_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.eden/auth.json for an active Eden provider.

    Returns the provider state dict if Eden is active with tokens,
    otherwise None.
    """
    pool_present, entry = _select_pool_entry("eden")
    if pool_present:
        if entry is None:
            return None
        return {
    ...
    try:
        ...
        if data.get("active_provider") != "eden":
            return None
        provider = data.get("providers", {}).get("eden", {})
        # Must have at least an access_token or agent_key
        if not provider.get("agent_key") and not provider.get("access_token"):
            return None
        return provider
    except Exception as exc:
        logger.debug("Could not read Eden auth: %s", exc)
        return None


def _nous_api_key(provider: dict) -> str:
    """Extract a usable Eden inference JWT from stored auth state."""
    from eden_cli.auth import _nous_invoke_jwt_is_usable
...
def _nous_base_url() -> str:
    """Resolve the Eden inference base URL from env or default."""
    return os.getenv("NOUS_INFERENCE_BASE_URL", _NOUS_DEFAULT_BASE_URL)


def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    """Resolve Eden auxiliary credentials from the selected pool entry."""
    try:
        from eden_cli.auth import _agent_key_is_usable

        pool = load_pool("eden")
    except Exception as exc:
        logger.debug("Auxiliary Eden pool credential resolution failed: %s", exc)
        return None
    ...
    try:
        ...
        logger.debug("Auxiliary Eden pool selection failed: %s", exc)
        return None
    ...
    if force_refresh or not _agent_key_is_usable(state, _nous_min_key_ttl_seconds()):
        try:
            ...
            logger.debug("Auxiliary Eden pool refresh failed: %s", exc)
            refreshed = None
...
def _resolve_nous_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    """Return fresh Eden runtime credentials when available.
    ...
    try:
        from eden_cli.auth import resolve_nous_runtime_credentials

        creds = resolve_nous_runtime_credentials(
            timeout_seconds=env_float("EDEN_OE_NOUS_TIMEOUT_SECONDS", 15),
            force_refresh=force_refresh,
        )
    except Exception as exc:
        logger.debug("Auxiliary Eden runtime credential resolution failed: %s", exc)
        return None
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``eden auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``eden_cli.auth``'s singleton runtime resolver for older
    ...
    try:
        from eden_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Eden OE auth store.
    ...
    try:
        from eden_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from eden_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from eden_cli.auth import is_provider_explicitly_configured
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from eden_cli.models import copilot_default_headers
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    # Check cross-session rate limit guard before attempting Eden —
    # if another session already recorded a 429, skip Eden entirely
    ...
    try:
        ...
        if _remaining is not None and _remaining > 0:
            logger.debug(
                "Auxiliary: skipping Eden Portal (rate-limited, resets in %.0fs)",
                _remaining,
            )
            _mark_provider_unhealthy("eden", ttl=_remaining)
            return None, None
    except Exception:
        pass

    eden = _read_nous_auth()
    runtime = _resolve_nous_runtime_api(force_refresh=False)
    if runtime is None and not eden:
        logger.warning(
            "Auxiliary Eden client unavailable: no Eden authentication found "
            "(run: eden auth)."
        )
        _mark_provider_unhealthy("eden", ttl=60)
        return None, None
    if runtime is None and eden:
        logger.debug(
            "Auxiliary Eden: runtime JWT refresh failed; checking stored "
            "auth.json token."
        )
    global auxiliary_is_nous
    auxiliary_is_nous = True
    logger.debug("Auxiliary client: Eden Portal")

    # Ask the Portal which model it currently recommends for this task type.
    # The /api/eden/recommended-models endpoint is the authoritative source:
    ...
    model = _NOUS_MODEL
    try:
        from eden_cli.models import get_nous_recommended_aux_model
    ...
    if runtime is not None:
        ...
        api_key = _nous_api_key(eden or {})
        if not api_key:
            logger.warning(
                "Auxiliary Eden client unavailable: no usable inference JWT found "
                "(run: eden auth add eden)."
            )
            _mark_provider_unhealthy("eden", ttl=60)
            return None, None
        base_url = str((eden or {}).get("inference_base_url") or _nous_base_url()).rstrip("/")
    return (
...
def _refresh_nous_recommended_model(
    *, vision: bool, stale_model: Optional[str]
    ...
    """Re-fetch the Eden Portal's recommended model after a stale-model 404.
    ...
    the Eden → OpenRouter catalog, every auxiliary call 404s with
    ...
    try:
        ...
        logger.debug(
            "Eden recommended-model refresh failed (%s); using default %s",
            exc, _NOUS_MODEL,
        )
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `eden setup` or `eden model` and enter a valid http(s) base URL."
        ) from exc
...
def _try_azure_foundry(
    *,
    ...
    :func:`eden_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from eden_cli.runtime_provider import _resolve_azure_foundry_runtime
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second eden
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``eden config aux reset``)."""
...
def _nous_portal_account_has_fresh_paid_access() -> bool:
    """Return True only when the fresh Eden account API says paid access is allowed."""
    try:
        from eden_cli.eden_account import get_nous_portal_account_info

        account_info = get_nous_portal_account_info(force_fresh=True)
        return account_info.paid_service_access is True
    except Exception as exc:
        logger.debug("Auxiliary Eden paid-entitlement refresh check failed: %s", exc)
        return False
...
def _is_model_not_found_error(exc: Exception) -> bool:
    ...
    that has since been dropped from the Eden → OpenRouter catalog. The Eden
...
def _recoverable_pool_provider(
    ...
    if base_url_host_matches(base, "openrouter.ai"):
        return "openrouter"
    if base_url_host_matches(base, "inference-api.projectglacie.com"):
        return "eden"
...
def _refresh_provider_credentials(provider: str) -> bool:
    ...
    try:
        ...
        if normalized == "openai-codex":
            from eden_cli.auth import resolve_codex_runtime_credentials
...
def _auth_refresh_provider_for_route(
    ...
    Copilot/Codex/Anthropic/Eden routes too. (#20832)
    """
    normalized = _normalize_aux_provider(resolved_provider)
    ...
    if base_url_host_matches(client_base_url, "api.anthropic.com"):
        return "anthropic"
    if base_url_host_matches(client_base_url, "inference-api.projectglacie.com"):
        return "eden"
    return normalized
...
def _try_payment_fallback(
    ...
    _alias_to_label = {"openrouter": "openrouter", "eden": "eden",
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into Eden OE' built-in discovery
...
def _resolve_auto(
    ...
    Priority:
      1. User's main provider + main model, regardless of provider type.
         ...
         chat.  Users on OpenRouter/Eden get their chosen chat model; users
      ...
      2. OpenRouter → Eden → custom → Codex → API-key providers (fallback
    ...
    #    scenario where a user switches providers via `eden model` but the
    #    old OPENAI_BASE_URL lingers in ~/.eden/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: eden model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.eden/.env",
    ...
    # on aggregators (OpenRouter, Eden) who previously got routed to a
    ...
    if main_provider == "moa":
        try:
            from eden_cli.config import load_config
            from eden_cli.moa_config import resolve_moa_preset
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        from eden_cli.copilot_auth import copilot_request_headers
    ...
    # See _create_openai_client: disable SDK-internal retries so Eden OE owns
...
def resolve_provider_client(
    ...
    Args:
        provider: Provider identifier.  One of:
            "openrouter", "eden", "openai-codex" (or "codex"),
    ...
    if provider == "eden":
        ...
        if client is None:
            logger.warning("resolve_provider_client: eden requested "
                           "but Eden Portal not configured (run: eden auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: eden model)")
                return None, None
    ...
    # OpenRouter / Eden bills for side tasks they thought were running on
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: eden model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    try:
        from eden_cli.runtime_provider import _get_named_custom_provider
        ...
        # entries that coincidentally match a canonical provider (e.g. ``eden``)
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: eden doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("eden_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from eden_cli.models import _should_use_copilot_responses_api
    ...
    if pconfig.auth_type == "external_process":
        ...
        if provider == "eden":
            return resolve_provider_client("eden", model, async_mode)
        if provider == "openai-codex":
...
def get_available_vision_backends() -> List[str]:
    ...
    Order: active provider → OpenRouter → Eden → stop.  This is the single
    ...
    if main_provider and main_provider not in {"auto", ""}:
        if main_provider in _VISION_AUTO_PROVIDER_ORDER:
            ...
            if client is not None:
                available.append(main_provider)
    # 2. OpenRouter, 3. Eden — skip if already covered by main provider.
...
def get_auxiliary_extra_body() -> dict:
    ...
    Includes Eden Portal product tags when the auxiliary client is backed
    by Eden Portal. Returns empty dict otherwise.
    """
...
def _refresh_nous_auxiliary_client(
    *,
    ...
    """Refresh Eden runtime creds, rebuild the client, and replace the cache entry."""
...
def _resolve_task_provider_model(
    ...
    def _preserve_provider_with_base_url(prov: Optional[str]) -> bool:
        ...
        if normalized in {"", "auto", "custom"} or normalized.startswith("custom:"):
            return False
        try:
            from eden_cli.providers import get_provider
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`eden_cli.plugins.PluginContext.register_auxiliary_task`) the
...
def call_llm(
    ...
    *,
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                ...
                if fb_client is not None:
                    ...
                    raise RuntimeError(
                        ...
                        f"variable, or switch to a different provider with `eden model`."
                    )
    ...
    try:
        ...
        # ── Stale-model self-heal (Eden Portal recommendation drift) ───
        # A long-lived process can pin a Portal-recommended model that has
        # since been dropped from the Eden → OpenRouter catalog, so every
        ...
        # known-good default). Only applies to Eden-routed calls.
        _heal_is_nous = (
            resolved_provider == "eden"
            or base_url_host_matches(_base_info, "inference-api.projectglacie.com")
        )
        if _is_model_not_found_error(first_err) and _heal_is_nous:
            ...
            if healed_model and healed_model != kwargs.get("model"):
                logger.warning(
                    "Auxiliary %s: model %r no longer in Eden catalog; "
        ...
        # ── Eden auth refresh parity with main agent ──────────────────
        client_is_nous = (
            resolved_provider == "eden"
            or base_url_host_matches(_base_info, "inference-api.projectglacie.com")
        )
        if (
            ...
            refreshed_client, refreshed_model = _refresh_nous_auxiliary_client(
                cache_provider=resolved_provider or "eden",
                model=final_model,
                async_mode=False,
                base_url=resolved_base_url,
                api_key=resolved_api_key,
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s: refreshed Eden runtime credentials after paid account check, retrying",
                    task or "call",
                )
        ...
        if _is_auth_error(first_err) and client_is_nous:
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s: refreshed Eden runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
        ...
        # When the resolved provider returns 401 and neither the Eden
...
async def async_call_llm(
    ...
    *,
    ...
    try:
        ...
        # the Eden → OpenRouter catalog, 404'ing every auxiliary call. Force a
        # fresh Portal fetch and retry once with the current recommendation.
        _heal_is_nous = (
            resolved_provider == "eden"
            or base_url_host_matches(_client_base, "inference-api.projectglacie.com")
        )
        if _is_model_not_found_error(first_err) and _heal_is_nous:
            healed_model = _refresh_nous_recommended_model(
                vision=(task == "vision"), stale_model=kwargs.get("model"))
            if healed_model and healed_model != kwargs.get("model"):
                logger.warning(
                    "Auxiliary %s (async): model %r no longer in Eden catalog; "
                    "retrying with refreshed recommendation %r",
                    task or "call", kwargs.get("model"), healed_model,
                )
                kwargs["model"] = healed_model
        ...
        if (
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s (async): refreshed Eden runtime credentials after paid account check, retrying",
                    task or "call",
                )
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
        ...
        if _is_auth_error(first_err) and client_is_nous:
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s (async): refreshed Eden runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
