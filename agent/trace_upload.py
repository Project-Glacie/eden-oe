"""Upload a Eden OE session transcript to Hugging Face as an agent trace.

Eden stores sessions in its own SQLite store (``eden_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``eden trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Eden OE' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "eden-traces"
_EDEN_OE_VERSION = "eden-agent"
...
# ---------------------------------------------------------------------------
# Conversion: Eden OE OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Eden OE' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Eden OE conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.eden/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `eden trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/eden-traces`` dataset. Returns a
