...
#
# When a user invokes a /skill (or /bundle), Eden OE expands the turn into a
...
# tests/openviking_plugin/test_openviking.py::test_skill_markers_match_eden_scaffolding.
# ---------------------------------------------------------------------------
...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``EDEN_OE_PLATFORM`` env var and
    ``EDEN_OE_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = (
            os.getenv("EDEN_OE_PLATFORM")
            or get_session_env("EDEN_OE_SESSION_PLATFORM")
        )
    except Exception:
        resolved_platform = os.getenv("EDEN_OE_PLATFORM")
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.eden.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_eden_home()}/config.yaml):"]
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.eden/skills/ and return a mapping of /command -> skill info.
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.eden/skills/`` and any ``skills.external_dirs`` so the
...
def build_preloaded_skills_prompt(
    ...
    bundle-invocation gate (#59156). Without this, ``eden -s <skill>`` or
    a deployment's ``EDEN_OE_TUI_SKILLS`` env var could force-load a skill an
