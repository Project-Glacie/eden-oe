...
Plugins built on Eden OE often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Eden OE
...
:class:`~eden_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Eden OE' main config
...
handles every provider, fallback chain, and per-task override Eden OE
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`eden_cli.plugins.PluginContext`
