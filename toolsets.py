...
_EDEN_CORE_TOOLS = [
    ...
    # read-only terminal tab (both gated on EDEN_OE_DESKTOP via check_fn —
    ...
    # spawned as a kanban worker (EDEN_OE_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `eden tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (EDEN_OE_KANBAN_TASK env "
    ...
    # Coding posture (base Eden — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in eden_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Eden OE toolsets (CLI + messaging platforms)
    #
    ...
    # the `eden send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "eden-cli": {
        ...
        "tools": _EDEN_CORE_TOOLS,
        "includes": []
    },

    "eden-cron": {
        # Mirrors eden-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `eden tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as eden-cli; gated by `eden tools`",
        "tools": _EDEN_CORE_TOOLS,
        "includes": []
    },
    ...
    "eden-email": {
        "description": "Email bot toolset - interact with Eden OE via email (IMAP/SMTP)",
        "tools": _EDEN_CORE_TOOLS,
        "includes": []
    },
    ...
    "eden-sms": {
        "description": "SMS bot toolset - interact with Eden OE via SMS (Twilio)",
        "tools": _EDEN_CORE_TOOLS,
        "includes": []
    },
    ...
    "eden-webhook": {
        ...
        "tools": _EDEN_OE_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "eden-gateway": {
        ...
        "tools": [],
        "includes": ["eden-telegram", "eden-discord", "eden-whatsapp", "eden-slack", "eden-signal", "eden-bluebubbles", "eden-homeassistant", "eden-email", "eden-sms", "eden-mattermost", "eden-matrix", "eden-dingtalk", "eden-feishu", "eden-wecom", "eden-wecom-callback", "eden-weixin", "eden-qqbot", "eden-webhook", "eden-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``eden-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_EDEN_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``eden-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (eden-<name>).
        # Gives them _EDEN_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("eden-"):
            platform_name = name[len("eden-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_EDEN_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
