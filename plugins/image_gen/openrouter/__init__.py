"""OpenRouter-compatible image generation backend (OpenRouter + Eden Portal).

Both OpenRouter and the Eden Portal inference endpoint speak the same
...
Eden Portal proxies OpenRouter, so one implementation services both — we only
swap the resolved ``(base_url, api_key)``. Credentials are resolved through the
agent's existing :func:`~eden_cli.runtime_provider.resolve_runtime_provider`,
which already understands OpenRouter's key pool and the Eden OAuth device-code
...
# image_gen.model from ``eden tools``): use exactly that model (no auto
...
class OpenRouterCompatImageProvider(ImageGenProvider):
    ...
    Instantiated once per backend (OpenRouter, Eden Portal). The two differ only
    ...
    def _resolve_model_chain(self, explicit: Optional[str] = None) -> list[str]:
        ...
        by ``eden tools``) → the quality-first default chain.
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key or not base_url:
            return error_response(
                error=(
                    ...
                    f"Configure {self._display} in `eden tools` → Image Generation."
                ),
                error_type="missing_api_key",
                provider=self._name,
                aspect_ratio=aspect_ratio,
        ...
        headers = {
            ...
            # OpenRouter attribution headers (harmless against Eden Portal).
            "HTTP-Referer": "https://github.com/ProjectGlacie/eden-agent",
            "X-Title": "Eden OE",
        }
...
def _build_providers() -> List[OpenRouterCompatImageProvider]:
    return [
        ...
        OpenRouterCompatImageProvider(
            provider_name="eden",
            display_name="Eden Portal",
            runtime_name="eden",
            config_key="eden",
            model_env_var="NOUS_IMAGE_MODEL",
            setup_schema={
                "name": "Eden Portal (image)",
                "badge": "subscription",
                "tag": "Reference-grounded image generation via Eden Portal (OpenRouter-backed)",
                "env_vars": [],
...
def register(ctx: Any) -> None:
    """Register the OpenRouter + Eden Portal image gen providers."""
