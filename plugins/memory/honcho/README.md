...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/eden>
...
```bash
eden memory setup honcho   # configure Honcho directly (works on a fresh install)
eden memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
eden config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.eden/.env
```

> `eden honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `eden memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$EDEN_HOME/honcho.json` | Profile-local (isolated Eden OE instances) |
| 2 | `~/.eden/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Eden OE profile: `eden` (default) or `eden_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `eden honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Eden OE session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`eden-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`eden`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-eden-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening eden in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Eden OE session ID (timestamp + hex). Every `eden` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Eden OE profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`eden`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Eden OE profile: `eden` (default) or `eden_<profile>` (e.g. `eden -p coder` -> host key `eden_coder`). Older `eden.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `EDEN_OE_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `eden-agent`) |
...
|---------|-------------|
| `eden memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `eden honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `eden memory setup`) |
| `eden honcho status` | Show resolved config for active profile |
| `eden honcho enable` / `disable` | Toggle Honcho for active profile |
| `eden honcho mode <mode>` | Change recall or observation mode |
| `eden honcho peer --user <name>` | Update user peer name |
| `eden honcho peer --ai <name>` | Update AI peer name |
| `eden honcho tokens --context <N>` | Set context token budget |
| `eden honcho tokens --dialectic <N>` | Set dialectic max chars |
| `eden honcho map <name>` | Map current directory to a session name |
| `eden honcho sync` | Create host blocks for all Eden OE profiles |
