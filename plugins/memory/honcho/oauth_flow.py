...
``eden://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Eden OE OAuth client. IP-literal so
...
def begin_authorization(
    ...
    *,
    ...
    (``eden-desktop`` / ``eden-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Eden OE.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local EDEN_HOME override can't reach.
