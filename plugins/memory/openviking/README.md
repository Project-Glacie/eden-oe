...
```bash
eden memory setup    # select "openviking"
```

The setup can link to an existing `~/.openviking/ovcli.conf`, copy its current
connection values into Eden OE, or create a minimal `ovcli.conf` when one does
not exist.

Or manually:
```bash
eden config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.eden/.env
```
...
| `OPENVIKING_AGENT` | `eden` | Eden OE peer ID in OpenViking, used for peer-scoped memories |

When `OPENVIKING_API_KEY` is set, Eden OE lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Eden sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
...
Eden built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Eden OE action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under the configured peer memory namespace |

Built-in `replace` and `remove` operations are not mirrored because Eden OE
...
file URIs, such as
`viking://user/peers/eden/memories/preferences/mem_abc123.md` or the canonical
`viking://user/default/peers/eden/memories/preferences/mem_abc123.md`. Files
