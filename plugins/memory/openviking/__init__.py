...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Eden OE peer ID in OpenViking (default: eden)
...
_DEFAULT_AGENT = "eden"
_AGENT_PROMPT_LABEL = "Eden OE peer ID in OpenViking"
...
_SYNC_TRACE_ENV = "EDEN_OE_OPENVIKING_SYNC_TRACE"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Eden OE slash-skill scaffolding before sending content to OpenViking.
...
def _load_eden_openviking_config() -> dict:
    try:
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("EDEN_HOME", "")).expanduser() if os.environ.get("EDEN_HOME") else Path.home() / ".eden"
...
def _runtime_openviking_timeout_message(endpoint: str) -> str:
    return (
        ...
        "OpenViking memory disabled for this Eden OE run."
    )
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Eden OE session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Eden OE only", "write values only to Eden OE .env"),
    ...
    _save_eden_only_config(
        config=config,
        provider_config=provider_config,
        env_path=env_path,
        values=values,
    )
    _print_openviking_ready("Connection saved to Eden OE .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See eden-agent#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_eden_openviking_config()
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Eden OE peer ID in OpenViking, sent as the actor peer and "
    ...
    def post_setup(self, eden_home: str, config: dict) -> None:
        ...
        from eden_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        eden_home_path = Path(eden_home)
        env_path = eden_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        settings = _resolve_connection_settings(_load_eden_openviking_config())
        ...
        try:
            ...
            if health_state == "unreachable":
                ...
                _emit_runtime_warning(
                    f"{health_message} OpenViking memory disabled for this Eden OE run.",
                    warning_callback,
                )
                self._client = None
        except ImportError:
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Eden OE' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Eden OE canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See eden-agent#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/eden/.overview.md).
