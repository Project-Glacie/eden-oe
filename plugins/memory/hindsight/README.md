...
```bash
eden memory setup    # select "hindsight"
```
...
```bash
eden config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.eden/.env
```
...
Eden spins up a local Hindsight daemon with built-in PostgreSQL. Requires an LLM API key for memory extraction and synthesis. The daemon starts automatically in the background on first use and stops after 5 minutes of inactivity.
...
Daemon startup logs: `~/.eden/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p eden ui start
```
...
Config file: `~/.eden/hindsight/config.json`
...
| Key | Default | Description |
|-----|---------|-------------|
| `bank_id` | `eden` | Memory bank name (static fallback used when `bank_id_template` is unset or resolves empty) |
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `eden-{profile}` isolates memory per active Eden OE profile. Empty placeholders collapse cleanly (e.g. `eden-{user}` with no user becomes `eden`). |
...
| `retain_context` | `conversation between Eden OE and the User` | Context label for retained memories |
...
The LLM API key is stored in `~/.eden/.env` as `HINDSIGHT_LLM_API_KEY`.
