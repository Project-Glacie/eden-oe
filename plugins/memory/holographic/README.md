...
```bash
eden memory setup    # select "holographic"
```

Or manually:
```bash
eden config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.eden-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$EDEN_HOME/memory_store.db` | SQLite database path |
