"""disk_cleanup — ephemeral file cleanup for Eden OE.
...
Rules:
  ...
  - empty dirs    → always delete (under EDEN_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly EDEN_HOME and /tmp/eden-*
Never touches: ~/.eden/logs/ or any system directory.
"""
...
try:
    ...
    def get_eden_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("EDEN_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".eden").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$EDEN_HOME/logs/``."""
    return get_eden_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$EDEN_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under EDEN_HOME or ``/tmp/eden-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    eden_home = get_eden_home()
    try:
        path.resolve().relative_to(eden_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/eden-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("eden-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "eden-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $EDEN_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so EDEN_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        eden_home = get_eden_home()
        for parent in ("cron", "cronjobs"):
            base = eden_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside EDEN_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under EDEN_HOME, but never recurse into known
    # durable state trees.  Some installs place the Eden OE checkout, venv,
    # and desktop build under EDEN_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    eden_home = get_eden_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in eden_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    eden_home = get_eden_home()
    try:
        rel = path.resolve().relative_to(eden_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "eden-agent",
        }:
            return None
        ...
        # Path isn't under EDEN_HOME (e.g. /tmp/eden-*) — fall through.
        pass
