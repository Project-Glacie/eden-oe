...
Auto-tracks and cleans up ephemeral files created during Eden OE
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$EDEN_HOME` and `/tmp/eden-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `EDEN_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under EDEN_HOME | always | Never |
...
- `is_safe_path()` rejects anything outside `EDEN_HOME` or `/tmp/eden-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$EDEN_HOME/disk-cleanup/` is itself excluded
- `$EDEN_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
