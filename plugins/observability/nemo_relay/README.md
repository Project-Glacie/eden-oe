...
Optional Eden OE observability plugin that maps Eden OE observer hooks to
...
not replace Eden OE's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Eden OE emit NeMo Relay lifecycle events for
the work Eden OE already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Eden OE can:

- Preserve Eden OE execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
eden plugins enable observability/nemo_relay
```

The `EDEN_OE_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `EDEN_HOME` that the
agent run will use:

```bash
env EDEN_HOME=/tmp/eden-nemo-relay-test \
  eden plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`EDEN_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`EDEN_HOME` is the Eden OE profile/config home used by both
`eden plugins enable ...` and the later `eden chat ...` run. If unset,
Eden uses the user's default home, usually `~/.eden`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export EDEN_HOME=/tmp/eden-nemo-relay-test
eden plugins enable observability/nemo_relay
eden chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `eden` command you run is built from the
...
uv run eden plugins enable observability/nemo_relay
uv run eden chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/eden_os-*.whl
...
The plugin can configure exporters directly from `EDEN_OE_NEMO_RELAY_*`
...
```bash
export EDEN_OE_NEMO_RELAY_ATOF_ENABLED=1
export EDEN_OE_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export EDEN_OE_NEMO_RELAY_ATIF_ENABLED=1
export EDEN_OE_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `EDEN_OE_NEMO_RELAY_ATOF_FILENAME`
- `EDEN_OE_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `EDEN_OE_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `EDEN_OE_NEMO_RELAY_ATIF_AGENT_NAME`
- `EDEN_OE_NEMO_RELAY_ATIF_AGENT_VERSION`
- `EDEN_OE_NEMO_RELAY_ATIF_MODEL_NAME`
- `EDEN_OE_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Eden OE at it:

```bash
export EDEN_OE_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
When `EDEN_OE_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`EDEN_OE_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`EDEN_OE_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Eden OE does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Eden keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Eden OE routes LLM and tool
...
For the full generic Eden OE middleware contract, see
...
export EDEN_HOME=/tmp/eden-nemo-relay-docs/eden-home
mkdir -p "$EDEN_HOME"

cat > "$EDEN_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Eden OE session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export EDEN_OE_NEMO_RELAY_ATOF_ENABLED=1
export EDEN_OE_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/eden-nemo-relay-docs/subagent/atof
export EDEN_OE_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export EDEN_OE_NEMO_RELAY_ATOF_MODE=overwrite
export EDEN_OE_NEMO_RELAY_ATIF_ENABLED=1
export EDEN_OE_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/eden-nemo-relay-docs/subagent/atif
export EDEN_OE_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export EDEN_OE_NEMO_RELAY_ATIF_AGENT_NAME='Eden OE E2E'
export EDEN_OE_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export EDEN_OE_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"eden.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Eden OE E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Eden OE dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/eden-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/eden-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/eden-nemo-relay-docs/workdir/beta.txt
cd /tmp/eden-nemo-relay-docs/workdir

export EDEN_OE_NEMO_RELAY_ATOF_ENABLED=1
export EDEN_OE_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/eden-nemo-relay-docs/parallel/atof
export EDEN_OE_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export EDEN_OE_NEMO_RELAY_ATOF_MODE=overwrite
export EDEN_OE_NEMO_RELAY_ATIF_ENABLED=1
export EDEN_OE_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/eden-nemo-relay-docs/parallel/atif
export EDEN_OE_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export EDEN_OE_NEMO_RELAY_ATIF_AGENT_NAME='Eden OE E2E'
export EDEN_OE_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Eden OE sessions map to `agent` scopes.
- Eden OE API request hooks map to `llm` scope start/end events.
- Eden OE tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Eden OE execution middleware to hand
...
Enable it for Eden OE:

```bash
export EDEN_OE_NEMO_RELAY_PLUGINS_TOML=/tmp/eden-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Eden OE routes execution
...
Eden provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Eden OE provider adapter next_call(...)

Eden tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Eden OE tool dispatcher next_call(...)
```
...
middleware for a local Eden OE run. This path requires a NeMo Relay runtime that
...
```bash
export EDEN_HOME=/tmp/eden-middleware-test/eden-home
mkdir -p "$EDEN_HOME" /tmp/eden-middleware-test/nemo-relay
...
cat > /tmp/eden-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/eden-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/eden-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Eden OE Middleware E2E"
agent_version = "local"
...
export EDEN_OE_NEMO_RELAY_PLUGINS_TOML=/tmp/eden-middleware-test/nemo-relay/plugins.toml
