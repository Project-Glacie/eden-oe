# Eden OE Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: eden-achievements plugin
...
### `GET /api/plugins/eden-achievements/achievements`
...
### `POST /api/plugins/eden-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/eden-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/eden-achievements/overview`
...
- Persist plugin data under:
  - `~/.eden/plugins/eden-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.eden/plugins/eden-achievements/scan_checkpoint.json`
...
- `~/.eden/plugins/eden-achievements/`
