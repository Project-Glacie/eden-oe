...
into the same ``DashboardAuthProvider`` framework as the Eden OAuth
...
mirroring the Eden provider's precedence convention:
  ...
  Environment overrides::
      ...
      EDEN_DASHBOARD_BASIC_AUTH_PASSWORD_HASH   # preferred
      EDEN_DASHBOARD_BASIC_AUTH_PASSWORD        # plaintext fallback
      EDEN_DASHBOARD_BASIC_AUTH_SECRET
      EDEN_DASHBOARD_BASIC_AUTH_TTL_SECONDS
...
Skip reasons:
  Like the Eden provider, this exposes a module-level ``LAST_SKIP_REASON``
...
def _resolve_secret(cfg_section: dict) -> bytes:
    ...
    """
    raw = _resolve(
        "EDEN_DASHBOARD_BASIC_AUTH_SECRET", cfg_section, "secret"
    )
    if not raw:
        logger.info(
            ...
            "secret (or EDEN_DASHBOARD_BASIC_AUTH_SECRET) for stable "
            "sessions."
        )
...
def register(ctx) -> None:
    ...
    username = _resolve(
        "EDEN_DASHBOARD_BASIC_AUTH_USERNAME", section, "username"
    )
    password_hash = _resolve(
        "EDEN_DASHBOARD_BASIC_AUTH_PASSWORD_HASH", section, "password_hash"
    )
    plaintext = _resolve(
        "EDEN_DASHBOARD_BASIC_AUTH_PASSWORD", section, "password"
    )
    ttl_raw = _resolve(
        "EDEN_DASHBOARD_BASIC_AUTH_TTL_SECONDS", section, "session_ttl_seconds"
    )

    if not username:
        LAST_SKIP_REASON = (
            "dashboard.basic_auth.username is not set (and "
            "EDEN_DASHBOARD_BASIC_AUTH_USERNAME is empty). Set a username "
    ...
    # EDEN_DASHBOARD_BASIC_AUTH_PASSWORD env var overrides a config.yaml
    ...
    plaintext_from_env = os.environ.get(
        "EDEN_DASHBOARD_BASIC_AUTH_PASSWORD", ""
    ).strip()
