"""
Email platform adapter for the Eden OE gateway.

Allows users to interact with Eden OE by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        try:
            from eden_cli import __version__ as _eden_version
        except Exception:  # noqa: BLE001 — keep ID best-effort if import fails
            _eden_version = "0"
        imap.xatom(
            "ID",
            f'("name" "eden-agent" "version" "{_eden_version}" '
            '"vendor" "ProjectGlacie" '
            '"support-email" "noreply@projectglacie.com")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `eden config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `eden gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Eden OE")
        ...
        msg_id = f"<eden-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# eden_cli/gateway.py, and the _send_email dispatch in
