"""
IRC Platform Adapter for Eden OE.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Eden OE agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "eden-bot")
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Eden OE")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Eden OE shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: eden_, eden_1, eden_2...
...
def check_requirements() -> bool:
    ...
    # eden setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `eden gateway setup` flow for the IRC platform.

    Lazy-imports ``eden_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Eden OE to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #eden — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.eden/.env")
    print_info("Restart the gateway for changes to take effect: eden gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "eden-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "eden-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Eden OE (cron)")
