...
# ``model_tools`` import time, so ``eden status``, ``eden chat``, etc.
...
# ``eden_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Eden OE is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a eden MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${EDEN_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _eden_home = _get_eden_home()
        except (ModuleNotFoundError, ImportError):
            _eden_home = _Path.home() / ".eden"
        self._thread_count_store = _ThreadCountStore(
            _eden_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Eden OE is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("EDEN_HOME", str(_Path.home() / ".eden"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${EDEN_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a eden MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Eden OE is thinking…' marker message.
        ...
        "Eden OE is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Eden OE is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Eden OE is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Eden OE is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Eden OE is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``eden setup``.

    The setup wizard at ``eden_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.eden/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. eden-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.eden/.env")
    print_info("Restart the gateway: eden gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``eden cron`` running as a
    separate process from ``eden gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Eden OE plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'eden-agent[google_chat]'",
        ...
        # the "Eden OE is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Eden OE is thinking…' marker message "
