"""SimpleX Chat platform adapter (Eden OE plugin).
...
This adapter ships as a Eden OE platform plugin under
``plugins/platforms/simplex/``. The Eden OE plugin loader scans the
...
Optional environment variables:
    ...
    EDEN_OE_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``eden setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Eden OE — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("EDEN_OE_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``eden setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.eden/.env`` via
    ``eden_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "eden_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.eden/.env"
        )
        return
