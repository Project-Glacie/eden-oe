...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `eden slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        tokens_file = get_eden_home() / "slack_tokens.json"
        ...
        try:
            ...
            # Eden OE does not currently need to route them into the agent loop.
            ...
            # /model work directly without /eden prefix). A single regex
            ...
            # manifest (see `eden slack manifest`). In Socket Mode, Slack
            ...
            from eden_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/eden$")

            @self._app.command(_slash_pattern)
            async def handle_eden_command(ack, command):
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Eden OE handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Eden OE session — matching the per-thread behavior channels already
        ...
        try:
            ...
            if not smart_denied:
                actions.append({
                    ...
                    "action_id": "eden_approve_session",
                    "value": session_key,
                })
                if allow_permanent:
                    actions.append({
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "eden_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "eden_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "eden_confirm_once": "once",
            "eden_confirm_always": "always",
            "eden_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "eden_approve_once": "once",
            "eden_approve_session": "session",
            "eden_approve_always": "always",
            "eden_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/eden <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/eden
        ...
        if slash_name in {"eden", ""}:
            # Legacy /eden <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from eden_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/eden   ``).
        ...
        # "Running /cmd…" ack shown by handle_eden_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/eden <question>" must produce public replies so
...
# ``eden_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``eden_cli/setup.py::_setup_slack``.
    """
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under EDEN_HOME, and print
        paste-into-Slack instructions. Failures are non-fatal."""
        try:
            from eden_cli.slack_cli import _build_full_manifest
            from eden_constants import get_eden_home
            import json as _json

            manifest = _build_full_manifest(
                bot_name="Eden",
                bot_description="Your Eden OE agent on Slack",
            )
            target = Path(get_eden_home()) / "slack-manifest.json"
            ...
            print_info(
                "   Re-run `eden slack manifest --write` anytime to refresh after "
                "Eden OE adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `eden update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://eden-agent.projectglacie.com/docs/user-guide/messaging/slack/")
    print()
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``eden_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'eden-agent[slack]'",
        # Interactive setup wizard — replaces eden_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in eden_cli/gateway.py.
