...
Small Node helper that bridges Eden OE to Photon's Spectrum SDK
(`spectrum-ts`).  Eden OE is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Eden-Sidecar-Token`)
...
The Eden OE plugin's `eden photon setup` command runs `npm install`
here automatically.
