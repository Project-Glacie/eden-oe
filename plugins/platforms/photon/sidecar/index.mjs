// Eden OS — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Eden OE): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Eden OE -> gRPC): `/send` drives `space.send(...)`; `/typing`
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Eden OE can restart the Photon adapter"
    );
    process.exit(75);
...
// instead of a cryptic module-resolution error during import. Apply Eden OE'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Eden OE allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Eden OE, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Eden OE supplied them so a known-good
