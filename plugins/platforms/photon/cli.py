"""
``eden photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Eden OE
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `eden photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Eden OE')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``eden photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Eden OE" project.
    ...
    # 3. Rotate the project secret and persist creds (runtime -> ~/.eden/.env,
    ...
    print("  Start the gateway:  eden gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `eden photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`eden photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.eden/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `eden photon telemetry on` / `eden photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.eden/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  eden gateway restart")
    return 0
...
#
# `eden gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``eden photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `eden gateway setup` wizard."""
